...
and define $b^k_{ij}$ similarly, except the rank index is transposed. \jv{note that I transpose the rank above, for symmetry of local correlations}
...
\begin{tabular}{c  c  c  c}
 $\delta_x$(44,45) & \hspace{1.8em} -1.45 \hspace{1.8em}  & \hspace{1.8em} -2.08 \hspace{1.8em} & \hspace{1.8em} -2.08 \hspace{1.8em}  \\ 
 $\delta_y$(44,45) & 0.83 & 1.43 & 0.00  \\ 
 $\delta_x \times \delta_y$ & -1.20 & -1.45 & 0.00  \\ 
 
\hline


 $\delta_x$(45,46) & -1.90 & -2.70 & -2.70  \\ 
 $\delta_y$(45,46) & -2.05 & -2.60 & -2.60  \\ 
 $\delta_x \times \delta_y$ & 3.90 & -1.90 & 7.01  \\ 
\end{tabular}
\end{center}
\caption{
...
% 
\textbf{Column 4:} Rank truncating yields $A^{k}$ and $B^{l}$, for a particular choice of $(k,l)=(4,6)$ (which is the optimal scale in this simulation), from which we can compute $C^{k,l}$, whose  sum is the \Mgc~statistic.  
...
Although single centering is equivalent to double centering for the global correlation, single centering is actually consistent in preserving ranks, e.g., the ranks by sorting $\tilde{A}$ within each row are exactly the same as the row ranks of $H\tilde{A}$, which are different from the ranks of $H\tilde{A}H$. This is why we define the rank-truncated comparisons a bit differently for $\mt{a}_{ij}^k$ and $\mt{b}_{ij}^l$: $\mt{a}_{ij}^k$ is defined based on ranks within each row, while $\mt{b}_{ij}^l$ is defined based on ranks within each column. This not only makes the ranks consistent in the $\tilde{Z}$ and $Z$ for $Z=A,B$, but also makes the resulting local correlations always symmetric even if $A$ and $B$ are not symmetric.
...
Furthermore, the row ranks of $\tilde{A}$ are preserved in $A$ under single centering but not double centering; similarly the column ranks of $\tilde{B}$ are preserved in $B$ under single centering.
\end{lem}
...
\Linefor{$Z:=A,B$}{$R^{Z}=\textsc{Sort}(\tilde{Z})$} \Comment{row-wise sorting  assuming no ties}
\State $R^{B}=\textsc{Transpose}(R^{B})$ \Comment{column-wise ranks are used for second data}
