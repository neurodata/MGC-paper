...
\node (X1) [draw=none,node2, rectangle split part fill={black!25,green},text width=3.5cm,minimum width=3.5cm,minimum height=2.65cm, above of=bot, yshift=1.19cm,label=west:(3),label=south: ] {\small Compute the p-value via permutation test. \nodepart{second} Then determine the informative scales (those with small p-values).};
...
\node (in1) [draw=none, node, fill=black!25,text width=3.5cm,minimum width=3.5cm,minimum height=2.65cm,above of=D1,yshift=3.01cm,label=west:(0)] {\small Obtain $n$ samples of cloud density on day $i$ ($x_i$) and grass wetness on day $i$ ($y_i$).};
\draw [arrow] (in1) -- (D1);
\draw [arrow] (D1) -- (DG1);
\draw [arrow] (DG1) -- (X1);
\end{tikzpicture}
...
%Green lines depict the best fit line for each points' neighbors within the informative scales selected by maximizing the local correlation, 
...
In the above example, this means for each observation, \Mgc~finds the observation with the most similar cloud density and the observation with the most similar grass wetness.  
