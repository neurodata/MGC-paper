...
Column 3: Double centering yields $A=\{a_{ij}\}$ and $B=\{b_{ij}\}$, from which we compute the element-wise product ($C=A \circ B$, center).  Double summing over $C$ yields 
the un-normalized \Mcorr~statistic, which is significant only when that double sum is positive. 
...
(A) $1$-dimensional simulations, where $D=1$ and the sample size is chosen by the power threshold $0.8$ as in Figure~\ref{f:powermaps1}.
