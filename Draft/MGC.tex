...
% In each of the above approaches the   comparison function is applied to the data; if the results are sub-optimal the routines typically must be run again almost entirely.  Thus,  they have been plagued with an inability 
to adaptively optimize the comparison function, a limitation that plagues manifold learning much more generally.
% in a fashion that is both computationally efficient and statistically consistent. 
...
Now consider observations  1, 2, and 3 (highlighted in black Figure \ref{f:newschem}, row 0).  Comparing observations 1 and 2, both cloud shape and grass wetness are close to one another for both the linear  and the  spiral relationships. 
...
\node (X1) [draw=none,node, fill=green,text width=3.5cm,minimum width=3.5cm,minimum height=2.65cm, above of=bot, yshift=1.19cm,label=west:(3),label=south: ] {\small Compute the p-value via permutation test, and determine the informative scales (those with small p-values)};
...
\end{tikzpicture}
\qquad
\includegraphics[width=0.32\textwidth,height=0.75\textheight,trim={0.83cm 1.88cm 1.85cm 1.59cm},clip]{Figures/Fig1.pdf}
\includegraphics[width=0.32\textwidth,height=0.75\textheight,trim={1.95cm 1.88cm 0.73cm 1.59cm},clip]{Figures/Fig8.pdf}
...
\textbf{(0)}  $150$ pairs of samples of cloud shape ($x_i$) and grass wetness ($y_i$). For ease of presentation, cloud shape is depicted in one-dimension.
Samples $1$, $2$, and $3$ (black) indicate how \Mgc~is able to discover nonlinear relationships. 
% 
\textbf{(1)} Distances are linearly correlated in the linear setting, whereas they are not in the spiral setting.  Distances that \Mgc~uses for its test statistic (green circles) are a subset of all distances (gray dots). Titles state $\GG(\Dcorr)$, which \Dcorr~uses to compute its p-value.
% 
\textbf{(2)} Green lines depict the best fit line for each points' neighbors within the informative scales selected by maximizing the local correlation, 
dashed gray line denotes the global linear fit to the data.  \Mgc's locality reconstructs the shape of the relationship. Titles state the value of the test statistic,  $\GG(\Mgc)$.
% 
...
The first step of \Mgc~is the computation of distances (Figure~\ref{f:newschem}, step 1). This step enables us to operate on distances (which are scalars), rather than the data in its observed space, which might be high-dimensional and contain complicated  structure, like time-series, shapes, or graphs.
% @jovo: clarify that all the magic is no in there. "The choice of distance function is important".  something about distance. beta test description of distance
The second step is the computation of the maximum ``local generalized correlation'', that is, the correlation only incorporating local distances.
...
% @jovo: if these two are the same point, "they count".  
\Mgc~then discards all distances that are not between nearest neighbors, and computes the correlation between the remaining distances.
Then \Mgc~iteratively increases the number of neighbors $k$ of $x_i$ to include, and the number of neighbors $l$ of $y_i$ to include.  
For each pair of scales ($k,l$), \Mgc~discards all distances that are not jointly the nearest $k$ neighbors for $x$ and the nearest $l$ neighbors for $y$, and computes the linear correlation between the remaining points.
...
% The green dot shows the optimal scale with highest power, and the green box shows the largest rectangle of scales whose power is nearly optimal (different from the optimal power by no more than 0.1).
}
\label{f:powermaps}
\end{figure}
