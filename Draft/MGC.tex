...
we not only want to know \textit{whether}  modalities are dependent on each other, but also \textit{how} they do (i.e. their \textbf{scales and nature of dependence}).
...
We developed the Multiscale Generalized Correlation (\Mgc) test to those needs.
...
The first step is to choose distance functions for both modalities and compute the distance between all pairs of observations within each modality. The choice of distance determines the metric upon which dependence will rest, and is the only choice in the entire algorithm.  
...
The third step is to compute a local variant of the test, to constraint the test statistic to only consider a particular scale.
The fourth step is to iterate the third step for all possible scales, which will enable \Mgc~to flexibly determine the scales of interest.
...
Figure \ref{f:real}{\color{magenta}B} provides the p-value map for testing on the right hemisphere using \Mgc. Again, many local scales yield significant p-values, whereas the global scale does not detect a significant dependence. The sample \Mgc~p-value is $0.0113$, and no other dependence test can identify significant dependence(e.g., global \Mcorr~and \Hhg~have p-values of $0.1132$ and $0.1208$). The left hemisphere seems less dependent on disease status, as both local and global tests yield p-values from $0.05$ to $0.1$ (not shown). 
