...
Note that for all simulations, there always exists a large consecutive region of local scales that are almost equally significant, which is an important observation that we use to approximate the optimal \Mgc~scales by the p-value map for data of unknown distribution.
...
Our first real data experiment investigates whether there is any dependency between the brain connectome and personality, from a data set of small sample size and high-dimensionality. There exists certain relationship between the brain activity and personality as shown in \cite{AdelsteinEtAl2011}, but existing statistical tests fail to discover any significant dependency from the raw data.

The data set contains $n=42$ subjects, where each person's personality is measured by questionnaires and the five-factor personality model. The brain activity of each person is measured by fMRI scan for $197$ brain regions by $194$ time steps. Thus the brain connectome feature is high-dimensional while the personality data is low-dimensional. To apply our method, two interpoint comparison matrices are required: for the brain data, we run a spectrum analysis for each region, bandpass and normalize it, then calculate the Kullback-Leibler divergence among regions and use the normalized Hellinger distance between each subject; for the personality data, we directly use the Euclidean distance.

Figure \ref{f:real}A provides the p-value map of \Mgc, based on $r=10$,$000$ random permutations. Many local scales yield significant p-values (around $0.01$), whereas the global scale fails to detect this significant dependence, neither does any previously proposed dependence test under consideration (\Mantel, \Dcorr, \Mcorr, or \Hhg). The approximate \Mgc~p-value is $0.037$, followed by \Hhg~at $0.0588$, with all others have p-values higher than $0.3$.
...
Figure \ref{f:real}B provides the p-value map for testing on right hemisphere using \Mgc. Again, many local scales yield significant p-values, whereas the global scale does not detect a significant dependence. The approximate \Mgc~p-value is $0.0260$, and no other dependence tests can identify the underlying local dependence. For left hemisphere vs disease, however, \Mgc~equals the corresponding global correlation, and all methods return close to significant p-values from $0.05$ to $0.08$ (not shown). 
...
Figure \ref{f:real}C shows that the global dependence tests can ascertain whether the whole brain-network is independent of the subject's creativity.  However, the global test is quite fragile, even ignoring a single subject from the global test can render the test non-significant. On the other hand, \Mgc~is more robust, there is a whole region of neighborhood sizes such that the test is quite significant.  Moreover, that the local tests performs optimally with approximately 30 neighbors suggests that these data have multiple cohorts, for which the dependence structure likely differs.  This result therefore suggests  the next investigatory steps to take to further understand the nature of the dependence structure between brain networks and creativity.
...
Estimated \Mgc~powers, while sometimes less than the \Mgc~powers computed by distribution directly, dominate to \Mcorr~and often improve upon \Hhg's powers.} %especially in the high-dimensionality settings.}
\label{f:simPerm}
\end{figure}
...
Five algorithms are presented in Appendix~\ref{appen:algorithms}: given the choice of a global correlation coefficient, Algorithm~\ref{alg:1scale} directly computes one local correlation coefficient at a given $(k,l)$. Algorithm~\ref{alg:all_scales} shows an efficient method to compute all local correlations simultaneously, in the same running time as computing one local correlation. Algorithm~\ref{alg:pval} computes the p-values of all local correlation by the random permutation test. Algorithm~\ref{alg:best_scale} approximates the optimal scales for \Mgc~by finding a consecutive region in the p-value map that has smooth p-value change and low median p-value, and outputs the approximated p-value and all optimal scales for \Mgc. Algorithm~\ref{alg:power} estimates the testing powers of all local statistics based on a given joint distribution or multiple pairs of data, which can be used to more accurately estimate the optimal scales and the testing power for \Mgc. More detailed discussions regarding the optimal scales approximation is offered in Appendix~\ref{appen:diss}.
...
\begin{algorithm}
\caption{Optimal Local Scale Approximation by P-values: Algorithm~\ref{alg:best_scale} approximates the optimal scale $(k^{*},l^{*})$ from the p-values of all local correlations, and outputs an approximate \Mgc~p-value.  The running time is $O(n^2)$, and more details are in Appendix~\ref{appen:diss}.}%Since adjacent local correlations are strongly correlated and differ by only one term, this algorithm validates the smoothness of the local p-values by $\psi$ within each row and column respectively, selects the scale that are smooth within consecutive rows or columns, and takes the smallest p-value within the validated region. If no row and column are sufficiently smooth, the largest scale (i.e., the global correlation) are used instead. The running time is $O(n^2)$.}
...
\State $[p_{r},\K]=\textsc{Validate}(P)$ \Comment{find consecutive scales of $P$ by row}
\State $[p_{c},\LL]=\textsc{Validate}(P\T)$ \Comment{find consecutive scales by column}
...
\Ensure The indices of valid rows $\K$, and the approximated \Mgc~p-value $p$.
\Function{Validate}{$P,r$}
\State initialize $\K$ as an empty set, $\psi, \rho$ as two $n \times 1$ vector of ones.
\State $\epsilon_{1} \rto \max\{0.0025,2/r\}$ \Comment{a threshold to ignore negligible p-value change}
\For{$i:=2,\ldots,n$}
\State $v \rto P[i,2:n]$ \Comment{get all p-values on each row}
\State $\psi_{i}=\textsc{Median}(v)$ \Comment{calculate the median p-value}
\State $v \rto \textsc{Diff}(v)$ \Comment{consecutive difference of the p-values on the row}
\State $\rho_{i} \rto \frac{1}{n-3} \sum_{i=1}^{n-3}\mb{I}\{v_{i} \times v_{i+1}<0 \ \& \ |v_{i}|>\epsilon_{1} \ \& \ |v_{i+1}|>\epsilon_{1}\}$ \Comment{the smoothness score}
\EndFor
\State $\K \rto \{k| \psi_{i}< \min\{0.0025,2/r\} \mbox{ for } i=k-1,k,k+1\}$ \Comment{find very significant p-values}
\If{$\K = \varnothing$} \Comment{otherwise find consecutive smooth rows with low median p-value}
\State $\epsilon_{2} \rto \min\{0.5-4 \sigma(r),0.3\}$ \Comment{a threshold for the smoothness score}
\For{$k:=2,\ldots,n$} \Comment{for each $k$, find the largest consecutive dominating region}
\State $\Omega_{k} \rto \{i=k_{1},k_{1}+1,\ldots,k,\ldots,k_{2} | \psi_{i} \leq \psi_{k} \ \& \ \rho_{i} \leq \rho_{k} \leq \epsilon_{2} \}$ %\Comment{$\Omega_{k}$ denotes consecutive indices including $k$}
\Lineif{$|\Omega_{k}|/(n-1) \geq \max\{0.15,5/(n-1)\}$}{$\K \rto [\K;k]$} \Comment{check the region size}
\EndFor
\EndIf
\State $p \rto \min_{i \in \K} \{\psi_{i}\}$ \Comment{take the minimal median p-value as an approximation of \Mgc~p-value}

\If{$\K = \varnothing$ or $p > P[n,n]$} 
\State $\K=\{n\}$
\State $p \rto P[n,n]$ \Comment{use the p-value of the global correlation instead}
\EndIf
\EndFunction
\end{algorithmic}
\end{algorithm}
