...
, 
\end{equation}
where $z$ is the product of standard deviations, that is $z=std(a) std(b)$, and $\bar{a}$ and $\bar{b}$ denote the sample means.
For $a_{ij}, b_{ij}$ that are not of zero-mean, the generalized correlation coefficient is of the form
\begin{equation}
\label{generalCoef}
\G= \tfrac{1}{z} {\textstyle \sum_{i,j=1}^n (a_{ij}-\bar{a}) (b_{ij}-\bar{b}) }, 
\end{equation}
where $\bar{a}$ and $\bar{b}$ denote the sample means of $a_{ij}$ and $b_{ij}$, and $z=std(a)std(b)$.
...
%  where
where $\bar{a}^{k}$ and $\bar{b}^{l}$ are the truncated means, and $z_{kl}$ is the product of the truncated standard deviations. There are a maximum of $n^2$ different local correlations, one for each possible combinations of $k$ and $l$.
...
\end{equation}
To express \Dcorr by the general correlation coefficient in Equation~\ref{generalCoef}, $a_{ij}$ and $b_{ij}$ are the respective matrix entry of $A$ and $B$, and the sample means $\bar{a}, \bar{b}$ are automatically $0$. The local version of \Dcorr follows by Equation~\ref{localCoef}.
...
\end{equation}
where $A=\tilde{A}$, $B=\tilde{B}$, $\bar{a}=\frac{1}{n(n-1)}\sum_{i \neq j}^{n}(a_{ij})$ and similarly for $\bar{b}$. In terms of Equation~\ref{generalCoef}, \Mantel lets $a_{ij}$ and $b_{ij}$ be the entries of $\tilde{A}$ and $\tilde{B}$ directly, and excludes $a_{ii}$ and $b_{ii}$ in the sample means because they are always $0$. Then the \Mantel~test is carried out by the permutation test.
