...
\jv{we haven't defined \mgcd, \mgcm, \mgcp up to this point. I have changed figure legend to this though.}
...
\label{appen:mgc}
Let us first repeat the generalized correlation coefficient:
\begin{equation}
\label{generalCoef}
\G= \tfrac{1}{z} {\textstyle \sum_{i,j=1}^n (a_{ij}-\bar{a}) (b_{ij}-\bar{b}) }, 
\end{equation}
...
For any general correlation coefficient, after sorting the distance matrices column-wise as in Equation~\ref{localCoef2}, the local correlations can be directly implemented by plugging in the respective $a_{ij}$ and $b_{ij}$:
\begin{equation}
\label{localCoef}
\G_{kl}=\dfrac{1}{z_{kl}} {\textstyle \sum_{i,j=1}^n (a_{ij}^k-\bar{a}^{k}) (b_{ij}^l-\bar{b}^{l})},
\end{equation}
...
Another technical detail worth mentioning: in our definition of $\bar{a}^{k} \ \sigma(a)^{k} \ \bar{b}^{l} \ \sigma(b)^{l}$, they are computed from all entries of $a_{ij}^k$ and $b_{ij}^l$, e.g., $\bar{a}^{k}$ equals $\dfrac{1}{n^2} \sum_{i,j=1}^{n} a_{ij}^k$. Alternatively, one may exclude the zero entries in computing the means and standard deviations of the truncated comparisons, then properly adjust the scalar factor in $z_{kl}$, i.e., $\bar{a}^{k}=\dfrac{1}{nk} \sum_{i,j=1}^{n} a_{ij}^k$ for \Dcorr, and the scalar factor in $z_{kl}$ should equal the number of non-zeros entries in the numerator rather than $n^2$, etc. This will yield an unbiased local correlation due to the exclusion of known zero entries, but is not used in the main paper for the ease of presentation; and the empirical testing performance are not affected by changing the simple centering.
