...
\begin{figure}
  ...
  Top row shows scatter plots of an exemplary dataset, the bottom row shows multiscale power maps.  It is evident that 
...
Here we apply  \Mgc~to visualize the relationship and test independence between real data sets. 
\cs{deleted: Since there is no known model for the given data, the optimal scale for \Mgc~is approximated (see Appendix \ref{appen:tests} for details and further discussions). }
...
The p-value curves of the local correlations are provided in Figure~\ref{f:real}(A) for $k=2,\ldots,n$ at $l=4$; we only show $l=4$ because the other curves look similar. We can clearly identify many common local scales between the two curves that yield significant p-values, which is the reason that \Mgc~can successfully identify the signals in both tests, and implies a similar local dependency structure. 

\begin{table*}[!t]
\caption{The p-values by the permutation test, for testing dependence on $4$ different pairs of real data. Significant p-values are highlighted. \Mgc~is able to consistently identify significant relationship for the first three rows, and does not inflate the p-value in the last row.\cs{table captions go above tables.} \cs{maybe add a row to table showing the average FDR  in the fake dependency case? i'm not sure if this is a good idea.}}
% \Large
