...
% jovo: cite something about the rise of nonlinear science?
% 
Second, the dimensionality of the data might be extremely high (millions or billions for genomics and connectomics datasets), while the sample size often remains low (tens or hundreds).  This ``\textbf{large $p$ small $n$}'' further compounds the challenges associated with nonlinear relationships because higher dimensional problems require more data to obtain quality estimates.
% jovo: cite something about p>n
% 
...
% 
Third, the data are often \textbf{structured}---sequences, images, networks, shapes, and text---creating problems for standard methods that were developed for unstructured feature sets.
% jovo: structured learning textbook cite?
% 
Fourth, because of the continuing data deluge,  \textbf{computationally efficient} methods are critical for generating results within acceptable time frames.
% jovo: 4th paradigm?
%
...
Suppose we wish to determine whether there is a relationship between cloud shape and the wetness of the ground under those clouds.  We  record observations of cloud shape and ground wetness on $n$ different days (Figure \ref{f:newschem} row 0).
Let $x_i$ denote cloud shape on day $i$ and $y_i$ denote ground wetness on that same day. 
If the relationship between cloud shape and ground wetness is approximately linear, the data might look like Figure \ref{f:newschem}{\color{magenta}A0}. 
On the other hand, if the relationship is a spiral, it might look like
like those in Figure \ref{f:newschem}{\color{magenta}B0}.
...
% 
Consider observation days 1, 2, and 3 (highlight in Figure \ref{f:newschem} row 0 by their color).  Comparing observations 1 and 2, both cloud shape and ground wetness are close to one another.  This is true  for both the linear  and the nonlinear (spiral) relationship.  However, comparing observations 2 and 3 is different.  When the relationship between shape and wetness is linear, if the distance between shapes across samples is large, then the distance between wetnesses also tends to be large.  This suggests that the correlation between distances can determine whether a relationship between shape and wetness exists.  
% 
...
Many statistical and machine learning methods have utilized pairwise comparisons for a wide variety of tasks, ranging from hypothesis testing (e.g., \cite{David1966,Mantel1967,Friedman1983,Schilling1986,Maa1996,SzekelyRizzo2009,SzekelyRizzo2013b,HellerGorfine2013}) 
...
The first step of these approaches compute comparisons (either  similarities or dissimilarities) between all pairs of observations (Figure \ref{f:newschem} row 1, gray dots).  In the context of the dependence testing scenario described above, all such approaches  would  compute comparisons between pairs of cloud shape observations as well as comparisons between pairs of ground wetness observations. These methods differ in how they choose the comparison function, and what they do with the comparisons having computed them. 
The popularity of these methods can be attributed to their wide-ranging applicability to structured data, 
% jovo: cite structured learning
 strong theoretical properties, 
% jovo: cite some stuff
...
\begin{figure}
\includegraphics[width=0.25\textwidth,trim={0cm 0cm 0cm 0cm},clip]
{Figures/mgc-flow.pdf}
\includegraphics[width=0.25\textwidth,trim={0.9cm 0cm 0cm 0cm},clip]{Figures/A3_type1_n100_noise3_dim1.pdf}
\includegraphics[width=0.25\textwidth,trim={0.9cm 0cm 0cm 0cm},clip]{Figures/A3_type8_n100_noise3_dim1.pdf}
...
To overcome this obstacle we have devised a method that we refer to as ``Multiscale Generalized Correlation'' (\Mgc).  
\Mgc~generalizes essentially all previously proposed pairwise comparison based approaches.   
Specifically, rather than computing the correlation between all pairs of distances, \Mgc~computes the correlation between subsets of pairs of distances, the local ones.
The intuition of using only local pairs follows from knowing that essentially any function can be fit by a piecewise linear function---that is, a union of many linear functions connected to one another.  And from the above example, it was clear that linear relationships between pairs distances are informative, as long as the pairs are close to one another along both dimensions.
The key is to be able to determine how many linear relationships are needed to fit the nonlinear function, or in other words, how local should we look (the scale).  
\textbf{Our main insight is realizing that we can adaptively choose the optimal scale for any relationship---linear or nonlinear, high-dimensional or low-dimensional, structured or unstructured---in a computationally efficient and statistically consistent fashion, therefore guaranteeing improved statistical performance over existing methods.}  This optimal scale is  informative about the nature of the dependence structure, therefore providing further guidance for subsequent experimental or analytical steps.

The second step of \Mgc, which differentiates from all previous approaches, is to compute the distance correlation for all possible scales, and choose the maximum as the test statistic. 
This step proceeds in a recursive fashion.  For each point, \Mgc~finds its nearest neighbor in $x$ and its nearest neighbor in $y$. 
In the above example, this means for each day, \Mgc~finds the day with the most similar cloud shape and the day with the most similar ground wetness.  
Given this  set of local pairs, \Mgc~computes the distances only between these nearest neighbor pairs, and then computes the correlation only among the resulting distances.  
\Mgc~iteratively increases the number of neighbors of $x$ to include, as well as those for $y$, for each computing  the linear correlation only between those local distances.   
% 
...
% Thus, computing the linear correlation for all pairs of distances will include many distance pairs that decrease the distance correlation, making the two properties seemingly unrelated in the \Dcorr~test."
 % 
% Doing so for all possible ``scales'' (or neighborhood sizes) 
% yields a \emph{multiscale correlation map}. Given this multiscale map, 
Having computed the test statistic for all the scales, \Mgc~selects  the best test statistic, that is, the largest test statistic after smoothing to address  noise (see Algorithm \ref{alg:sample_mgc} for details). The green dots in Figure \ref{f:newschem}Ai and Bi show the pairwise distances for the chosen scales for $x$ and $y$.  The green lines in Figure \ref{f:newschem}Aii and Bii connect the corresponding neighbors, the titles provide the value of the test statistics. Clearly, for the linear case all the neighbors are used, so the test statistic for \Mgc~is the same as that for \Dcorr.  For the nonlinear case however, the set of comparisons is greatly reduced to only local pairs, resulting in the test statistic for \Mgc~being much larger than \Dcorr's test statistic.   



The third step is to compute the p-value, as well as a \textbf{multiscale significance map} indicating which scales are maximally informative about the dependence relationship. 
% 
% via a permutation test, much like previously proposed dependence tests. 
The p-value for \Dcorr, $p(c)$, and \Mgc, $p(\hat{c}^*)$, are both highly significant for the linear example. On the other hand, \Dcorr~fails to detect a relationship for the nonlinear setting, where \Mgc~still detects a highly significant relationship.
...
\begin{figure}
  ...
  \caption{Power comparison of both Oracle and Sample \Mgc~to four benchmark dependence tests (denoted $\mathcal{A}$), for the $20$ different settings.  For each setting we compute the average power over a wide range of dimensions $\bar{\beta}$, and plot $\bar{\beta}(\mathcal{A}) - \bar{\beta}(\Mgc)$, for both Oracle (black dots) and Sample \Mgc~(gray asterisks).  
