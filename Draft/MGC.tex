...
A primary challenge that we faced was determining the optimal local scales in a computationally efficient and statistically consistent manner. Below, we describe and justify each of the five steps in detail.
...
\Lineif{$\sum_{k,l} R_{kl} < \gamma n^2$}{$R[:,:] \rto 1$} \Comment{if the rectangle is too small, use all scales}
\State $[H,E]=\textsc{Hist}(P(R),\min\{\sum{R}/20,100\})$ \Comment{partition smooth p-values into bins}
...
\If{$\sum_{k,l} R_{kl}==0$} 
\State $p \rto P[n,n]$ \Comment{if empty, use the global p-value instead}
\EndIf
