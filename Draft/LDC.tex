...
\cs{Tried again! I deleted the long table in appendix, for now; as the plot and the mean/std are informative enough, I think.}
...
\cs{I have added sub-sampling stuff into intro part as well as appendix, and local scale estimation and geometry implication into the next steps. I commented out other next steps in tex, that I think is not concrete enough yet; we can add back later. }
...
\State $r_{s}=max(ceil(r/s),50)$; \Comment{use $r_{s}$ random permutations instead of $r$ during screening}
\State $ind=1:n_{s}^2$; \Comment{initialize all local scales as the potential optimal scales}
\State $\epsilon=0.5$; \Comment{the threshold p-value to determine if a scale has low p-value or not}
\For{$i=1,\ldots,s$} \Comment{estimate the optimal scales by random sub-samples}
\State $ll=length(ind)$;$per=randperm(n)$; 
\State $p1=PermutationTest(X(per(1:n_{s}),:),Y(per(1:n_{s}),:),r_{s},option)$; 
\State $p2=PermutationTest(X(per(n_{s}+1:2*n_{s}),:),Y(per(n_{s}+1:2*n_{s}),:),r_{s},option)$;
\State $ind=ind(p1-min(p1)<\epsilon)$; $ind=ind(p2-min(p2)<\epsilon)$; \Comment{exclude all scales that have large p-values}
\If{$length(ind)==ll$} 
\State $ind=verifyScales(ind)$;$break$; \Comment{extract the most consecutive scales}
\EndIf
\If{$length(ind)<n_{s}/3$}
\State $ind=[]$;$break$; \Comment{stop sub-sampling if there are too few optimal scales}
\EndIf
\EndFor

\State $indN=[]$; \Comment{store the enlarged optimal scales}
\If{$length(ind)==0$}
...
\State $p=mean(p(indN))$; \Comment{take the men as the estimated MGC p-value}
\State \Return $p$, $indN$.
\EndFunction
\end{algorithmic}
\end{algorithm}
