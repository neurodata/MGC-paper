...
The x-axis is the simulation type, and y-axis shows the minimal sample size of each method to achieve the required testing power, the smaller the better. For panel (A), the dimension is fixed at $1$ for each setting, and the minimal size to achieve the required testing power is estimated. For panel (B), the dimension is chosen as the highest possible according to Supplementary Figure~\ref{f:nDAll} for each type. We bound the sample size to $1000$ in visualization and averaging. The results indicate that \Mgc~is a superior choice for finite-sample dependency testing due to its saving in required sample size, e.g., for the second best method \Hhg~, on average it requires at least twice the sample size of \Mgc~to achieve a power of $0.85$.
}
\label{f:Summary}
\end{figure}
...
\begin{figure}
  \centering
  \includegraphics[width=1.0\textwidth,trim={3.5cm 0 3.5cm 0cm},clip]{Figures/Fig1DPowerMGCM}
