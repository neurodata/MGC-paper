...
for r1=1:rep1
    ...
    for r2=1:rep2
        ...
        for i=1:lim
            ...
            if option1~=0
                dCor1A(r2,1:nn-1,i)=rankDCorr(C,P,disRank,1);
            end
            if option2~=0
                dCor2A(r2,1:nn-1,i)=rankDCorr(C,P,disRank,2);
            end
            if option3~=0
                dCor3A(r2,1:nn-1,i)=rankDCorr(C,P,disRank,3);
            end
            if option4~=0
                dCor4A(r2,i)=HHG(C,P);
            end
    ...
    for r2=1:rep2
        ...
        for i=1:lim
            ...
            disRank=[disToRanks(C) disToRanks(P)];
            if option1~=0
                dCor1N(r2,1:nn-1,i)=rankDCorr(C,P,disRank,1);
            end
            if option2~=0
                dCor2N(r2,1:nn-1,i)=rankDCorr(C,P,disRank,2);
            end
            if option3~=0
                dCor3N(r2,1:nn-1,i)=rankDCorr(C,P,disRank,3);
            end
            if option4~=0
                dCor4N(r2,i)=HHG(C,P);
            end
...
%     xlim([kmin K]);
%     ylim([0 1]);
%     legend('Rank Distance Covariance','Distance Correlation','Modified Distance Correlation','HHG','Location','SouthEast');
