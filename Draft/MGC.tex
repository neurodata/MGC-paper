...
\State $\tau \rto 20$ \Comment{determine the number of bins and check the area of optimal scale}
...
\State $p \rto P[n,n]$ 
\Else
\State $[H,E]=\textsc{Hist}(P(R),\max\{\sum{R}/\tau,100\})$ \Comment{partition smooth p-values into bins}
...
%\If{$\mu \geq \gamma$} \Comment{check if the area is large enough}
%\State $p = \textsc{Quantile}(P(R), \gamma / 2\mu)$ \Comment{use a smaller \Mgc~p-value by sample quantile}
%\Else 
%\State $p=\textsc{Mean}(P(P<1))$ \Comment{otherwise use the mean p-value}
%\EndIf
\EndIf
\State $R = \textsc{LargestRectangle}(R \cap (P\leq p))$ \Comment{calculate the optimal rectangle}
\If{$\sum_{k,l} R_{kl}<\max\{\gamma n^2/5,\tau\} \ \& \ p\neq P[n,n]$} 
\State $p=\textsc{Mean}(P(P<1))$ \Comment{use the mean p-value instead}
\State $R = \textsc{LargestRectangle}(P\leq p)$
\EndIf
