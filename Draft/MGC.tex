...
\State $[p_{r},\K]=\textsc{Validate}(P)$ \Comment{validate consecutive significant scales of $P$ by row}
\State $[p_{c},\LL]=\textsc{Validate}(P\T)$ \Comment{validate consecutive significant scales of $P$ by column}
...
\State $\alpha=0.05$
\For{$i:=2,\ldots,n$}
\State $\psi_{i}=\textsc{Median}(P[i,2:n])$ \Comment{the median p-value on each row}
\State $\LL \rto \{l | P[i,l]<\alpha \mbox{ and } |P[i,l]-P[i,l-1]|<\alpha/2\}$ \Comment{get adjacent significant scale}
\State $\rho_{i} = \textsc{Consecutive}(\LL) / (n-1)$ \Comment{return the maximal consecutive scales}
\Lineof{$\rho_{i} <0.5$}{$\rho_{i} \rto 0$} \Comment{disband row indices without enough significant adjacent scales}
\EndFor

\State $\K = \{k | \psi_{k}<\alpha\}$
\For{$k \in K$}

$\rto \{k | \psi_{k}<\alpha \mbox{ and } |P[i,l]-P[i,l-1]|<\alpha/2 \}$

\For{$i:=2,\ldots,n$}
\If{$\textsc{Consecutive}(\psi<\alpha) \times \rho_{i}/(n-1)>0.1$} 
