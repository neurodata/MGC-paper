...
Figure~\ref{f:schematic} schematically illustrates \Mgc~on a particular nonlinear dependence structure, and the table provides the exact values for two different pairs of samples.  In particular, Figure~\ref{f:schematic} demonstrates that when a pair of points (1,3) are close to each other only in $x$ but not in $y$, \Mgc~ignores it.  However, when a pair of points (3,9) is close in both $x$ and $y$, it considers it.  Since both the local and global methods' test statistics are summations over pairs,  \Mgc~can more powerfully detect dependence in such low-sample size and nonlinear settings.
...
(see Algorithm \ref{alg:best_scale} , \ref{alg:best_scale_aux} and \ref{alg:power} in Appendix \ref{appen:algorithms} for method details, and Appendix~\ref{appen:diss} for more discussions).
...
Six algorithms are presented in Appendix~\ref{appen:algorithms}: Algorithm~\ref{alg:1scale} directly computes the local correlation coefficient at a given scale $(k,l)$, for a given choice of a global correlation coefficient.
...
\State $p \rto P[n,n]$ \Comment{use the global p-value by default}
\If{$\sum_{kl}(P<p)/n^2>0.05$} 
\State $\Gamma \rto \{5,10,\ldots,50\}$ \Comment{specify the percentile of p-values for significance check}
\For{$j:=1:\textsc{Length}(\Gamma)$}
\State $\gamma=\textsc{Prctile}(P,\Gamma[j])$ \Comment{check the p-values in order}
\State $PR = \textsc{LargestComponent}(P<\gamma)$ \Comment{find the largest connected component}
\State $PC=\textsc{ValidateRow}(PR^{T})$ \Comment{find consecutive region within row}
\State $PR=\textsc{ValidateRow}(PR)$ \Comment{find consecutive region within column}
\Lineif{$\sum_{kl}PC_{kl} > \sum_{kl}PR_{kl} $}{$PR \rto PC$} \Comment{use the larger region}
\If{$\frac{\sum_{kl}PR_{kl}}{(n-1)^2}>\max\{\Gamma[j]/100 \times 0.2, 0.1\}$}
\State $p \rto \gamma$;
...
\begin{algorithm}
\caption{Consecutive Region Validation By Row. This algorithm checks the connected component of a binary matrix by adjacent rows, and only keeps the longest consecutive ones in rectangles. }%Since adjacent local correlations are strongly correlated and differ by only one term, this algorithm validates the smoothness of the local p-values by $\psi$ within each row and column respectively, selects the scale that are smooth within consecutive rows or columns, and takes the smallest p-value within the validated region. If no row and column are sufficiently smooth, the largest scale (i.e., the global correlation) are used instead. The running time is $O(n^2)$.}
\label{alg:best_scale_aux}
\begin{algorithmic}[1]
%\Statex
\Require A binary matrix $PR$.
\Ensure A binary matrix $PRS$ of connected components that are in rectangles.
\Function{ValidateRow}{$PR$}
\State $n_{2} \rto \left\lceil 0.02 \times n \right\rceil$ \Comment{number of adjacent rows to check}
\State $n_{3} \rto \left\lceil 0.1 \times n \right\rceil$ \Comment{number of adjacent columns to check}
\State $i \rto 2$
\While{$i\leq n$}
\State $i_{s} \rto \max\{i-n_{2},2\}$
\State $i_{e} \rto \min\{i+n_{2},n\}$
\State $\psi \rto \textsc{Find}\{PR[i,:] =0, \mbox{ for any } i \in \{i_{s},\ldots,i_{e}\}\}$
\State $\psi \rto [1;\psi;n+1]$
\State $j \rto \arg\max(\textsc{Diff}(\psi))$
\State $ind \rto \psi_{j}+1:\psi_{j+1}-1$
\State $i=i+1$
\If{$\textsc{Length}(ind) \geq n_{3}$}
\State $PRS[i_{s}:i_{e},ind] \rto 1$
\State $i=i_{e}+1$
\EndIf
\EndWhile
\State $PR = \textsc{LargestComponent}(PR)$
\EndFunction
\end{algorithmic}
\end{algorithm}
