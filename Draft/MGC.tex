...
Thus we use the single-centered \Mcorr~with the diagonal modification for computational expediency and simplicity.
...
\Ensure The power matrix $\beta_{\alpha} \in [0,1]^{n \times n}$ for all local correlations, and the \Mgc~optimal scale $(k^{*},l^{*}) \in \mathbb{N} \times \mathbb{N}$.
...
\begin{algorithm}
\caption{Computation of p-value and optimal scales for sample \Mgc. First we locate a smooth region of the p-value map as follows: we compute the difference of adjacent local p-values within each row, label each difference a smooth change if it is bounded above by a small positive number $\tau$ (i.e., the local p-values have a decreasing trend within the rows), and compute the largest rectangular region of smooth changes. Do it for decreasing/increasing p-values within each row/column, combine all four smooth rectangles into one region, then locate the largest rectangle in the region with all p-values less than some large threshold (for example, $0.5$). If the global p-value is among the top p-values, i.e., within $\gamma \%$ of all local p-values, we directly take the global p-value for sample \Mgc; otherwise if the area of the verified smooth region is larger than $\gamma$, we take a small p-value within the region as the p-value for sample \Mgc. Then the largest rectangle that is bounded above by the \Mgc~p-value within the smooth region is taken as the optimal scale. The running time is $O(n^2)$.}
...
\State $p \rto 1$ \Comment{default p-value to maximal}
\State $\gamma \rto 0.05$ \Comment{threshold to check significant global p-value and significant region}
\State $R=\textsc{SmoothRegion}(P,\tau)$ \Comment{the smooth region within the p-value map}
\If{$\sum (P<P[n,n])/n^2<\gamma$} \Comment{use the global p-value when it is small enough}
\State $p \rto P[n,n]$ 
\Else
\State $\mu \rto \sum_{k,l} R_{kl} /(n-1)^2$
\If{$\mu>\gamma$} \Comment{check if area of the smooth region is large enough}
\State $p = \textsc{Prctile}(P(R), \gamma / \mu)$ \Comment{get the \Mgc~p-value by percentile}
\EndIf
\EndIf
\State $R = \textsc{LargestRectangle}(R \cap (P\leq p))$ \Comment{the optimal scale}
\State $[k^{*},l^{*}] \rto (R==1)$
\EndFunction
\end{algorithmic}
\
...
\State $\eta \rto 0.5$ \Comment{threshold to check p-value}
...
\State $R = \textsc{LargestRectangle}(R \cap (P<\eta))$ \Comment{the verified smooth region}
\EndFunction
\end{algorithmic}
\end{algorithm}
...
The main targets of sample \Mgc~are (i) to achieve similar powers as Oracle \Mgc~under dependence, (ii) estimate the optimal scale, and (iii) not inflate the false positive rate under independence. Algorithm~\ref{alg:best_scale} searches for a consecutive region that has smooth p-values (i.e., mostly decreasing or increasing along either the row or column indices), and takes a small p-value within the smooth region if the region is large enough, or directly takes the global p-value when it is small enough. The parameter $\gamma$ is an important threshold to identify the significant region, which can be increased or decreased to make the algorithm more conservative or aggressive in searching the optimal scale. Increasing or decreasing the parameter $\tau$ has a similar effect as $\gamma$, though to a lesser extent, and a small positive number generally suffices to negate noisy p-value changes. Empirically, our choice of parameters achieve good testing power under dependence with minimal FDR inflation.
...
We further observe that the sample \Mgc~is only significantly inferior to the Oracle \Mgc~when the true optimal scale has a narrow region (e.g., function $16$ and $17$, see Figures~\ref{f:powermaps} and ~\ref{f:powermaps1}), but otherwise they are very similar. As narrow optimal scales may also occur from a pair of independently generated data by chance, narrow smooth regions cannot be trusted without training data or knowing the joint distribution, and are purposely excluded by algorithm~\ref{alg:best_scale} using $\gamma=0.05$.
...
\section{Proofs}
\label{appen:proofs}
\textcolor{green}{MM:should we use the notation $\Mgc_t$ in the statement and proof of the theorem? I'm a bit confused by the notation I think}
\begin{appThm}
$\beta_n(\G_t^*) \rightarrow 1$ as $n \to \infty$ for all $f_{xy}$ in $\mc{F}_t$.
In words, Oracle \Mgc~is consistent with all dependent alternatives for which its global counterpart is consistent. 
\end{appThm}
\begin{proof}
Since $\beta_n(\G_t^*)=\underset{kl}{\max}\{\beta_n(\G_t^{kl})\}$, for any $f_{xy}$ the power of \Mgc~satisfies
\begin{equation*}
\beta_n(\G_t^*) \geq \beta_n(\G_t)
\end{equation*}
at any type $1$ error level $\alpha$. So $\beta_n(\G_t^*) \rightarrow 1$ if $\beta_n(\G_t) \rightarrow 1$.
% 
Therefore $\beta_n(\G_t^*) \rightarrow 1$ for all $f_{xy}$ in $\mc{F}_t$. In particular, \Mgcd~and \Mgcm~are consistent with all alternatives satisfying certain regularity conditions, because \Dcorr~and \Mcorr~are consistent by \cite{SzekelyRizzoBakirov2007, SzekelyRizzo2013a}. 
\end{proof}

\textcolor{green}{MM:what do we mean here by "linearly dependent"? $\mb{x}=\alpha\mb{y}$ for some scalar $\alpha$, or $\mb{x}=A\mb{y}$ for some matrix $A$? From the proof I'd say the former, while in the text I was thinking of the latter, and wonder if the conclusions ain't true for the general linear dependence (the latter case). And of course we are talking about $\mb{x}$ and $\mb{y}$ in Euclidean (or some vector) space at this point, maybe worth saying it.}
\begin{appThm}
If $\mb{x}$ is linearly dependent on $\mb{y}$, then for any $n$ it always holds that
\begin{equation}
\beta_n(\G^{*}_t) = \beta_n(\G_t).
