...
Column 2: Compute all pairwise distances for $x$ and $y$ yielding
 $\tilde{A}$ (top) and $\tilde{B}$ (bottom), the two interpoint comparison matrices used by the \Mantel~test \cite{Mantel1967}  (this requires choosing appropriate distances for both $x$ and $y$).  The heatmaps are arbitrarily sorted by $x$'s.
...
\cs{state lemma about dcorr, which includes the result of dcov and dvar's.}
...
\cs{nor this one, not at all.}
\cs{another lemma about preserving ranks}
...
\[a_{ij} = \left\{
  \begin{array}{lr}
    ...
    \frac{1}{n}\sum_{i}\tilde{a}_{ii}-\frac{1}{n^2}\sum_{ij}\tilde{a}_{ij}, &\mbox{ if } i = j,
  \end{array}
\right.
\]
...
Generally, there are a total of $\max(R(a_{ij})) \times \max(R(b_{ij}))$ local correlations, which equals $n^2$ when there exists no repeating values of \mbx~or \mby. Note that we use minimal ranks in sorting when ties occur, which indexes all local correlations more conveniently than breaking ties randomly or using average or max ranks.
\cs{why is this "more convenient"?} 
...
\cs{fix ties for data and optimal scales.}
...
\State $\K \rto \{\K,k\}$
\EndFor
\If{$\textsc{Median}(P_{kl},k \in \K, l=2,\ldots,n) > \alpha \times \frac{|\K|}{n-1}$}
...
Therefore the p-value of $\G^{kl}$ cannot be less than the p-value of $\G$ under linear dependency, in which case the global correlation is the optimal scale for \Mgc. \cs{i don't get it.} \jovo{expanded a bit now} \cs{connect permutation and p-values.}
\end{proof}
