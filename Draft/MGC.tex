...
\cs{is sample mgc the right name??}
...
\begin{algorithm}
\caption{Compute p-values for all local tests and sample \Mgc, and estimate the optimal scales. This algorithm computes the p-values of all local correlations and sample \Mgc~by the permutation test with $r$ random permutations, which takes $O(rn^2 \log n)$. Then we estimate the optimal scales by the largest rectangle with local p-values less than or equals to the p-value of sample \Mgc. $\pi$ denotes a random permutation, and $\tilde{B}(\pi,\pi)$ denotes the distance matrix with the observation label permuted. In the real data experiment we always set $r=10$,$000$. Note that the p-value computation for any other benchmark follows from the same algorithm by replacing sample \Mgc~with the respective test statistic without the optimal scale estimation.}
\label{alg:pval}
\begin{algorithmic}[1]
