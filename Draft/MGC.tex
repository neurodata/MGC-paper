...
%
% ^.
% \sd{undstructure would just be vectors whose order does not matter, in the above, the "order" matters, does that make sense?}
...
\jv{done, in alg 4}
\cs{did you update the fig though? i still see colons}
\jv{corrected...they should be $\pi$ but pdf pics changed them to colon..let's generate figA again though}
...
Figure \ref{f:real}{\color{magenta}B} provides the p-value map for testing on the right hemisphere using \Mgc. Again, many local scales yield significant p-values, whereas the global scale does not detect a significant dependence. The sample \Mgc~p-value is $0.0096$, and no other dependence test can identify significant dependence. The left hemisphere seems less dependent on disease status, 
...
Algorithm~\ref{alg:best_scale} approximates the optimal scales and p-value for sample \Mgc~by finding a smooth region of the p-value map, searching for the \Mgc~p-value within the smooth region, and outputting the largest smooth rectangle with same or less p-values as the optimal scale if and only if the area is large enough. More detailed discussions regarding the optimal scales approximation is offered in Appendix~\ref{appen:diss}.
...
\begin{algorithm}
\caption{Optimal Local Scales Approximation by P-values. If the global p-value is among the top p-values, i.e., within $\gamma \%$ of all local p-values, we directly take the global p-value for \Mgc. Otherwise we locate a smooth region of the p-value map as follows: we compute the difference of adjacent local p-values within each row, label each difference a smooth change if it is bounded above by a small positive number $\tau$ (i.e., the local p-values have a decreasing trend within the rows), and compute the largest rectangular region of smooth changes. Do it for decreasing / increasing p-values within each row / column, combine all four smooth areas, and locate the largest rectangle region with all p-values less than $0.5$ inside the combination. If the area of the verified smooth region is larger than $\gamma$, we take a small p-value within the region as the p-value for sample \Mgc, and use the largest rectangle that is bounded above by the \Mgc~p-value within the smooth region as the optimal scale. The running time is $O(n^2)$.}
...
\State $\gamma \rto 0.05$ \Comment{threshold to check significant global p-value and significant region}
\If{$\sum (P<P[n,n])/n^2<\gamma$} 
\State $p \rto P[n,n]$ \Comment{use the global p-value when it is small enough}
\State $[k^{*},l^{*}] = \textsc{LargestRectangle}(P\leq p)$
\Else
\State $R=\textsc{SmoothRegion}(P,\tau)$ \Comment{the smooth region within p-value map}
\State $\mu \rto \sum_{k,l} R_{kl} /(n-1)^2$ \Comment{the area of the smooth region}
\If{$\mu>\gamma$} \Comment{check if the smooth area is large enough}
\State $p = \textsc{Prctile}(P(R), \gamma / \mu)$ \Comment{get the \Mgc~p-value by percentile}
\State $[k^{*},l^{*}] = \textsc{LargestRectangle}((R==1)\& (P\leq p))$ \Comment{the optimal scale}
\EndIf
\EndIf
\EndFunction
\end{algorithmic}
...
\State $R = (R^{1} \mbox{ or } R^{2} \mbox{ or } R^{3} \mbox{ or } R^{4})$ \Comment{combine all smooth rectangles}
\State $R = \textsc{LargestRectangle}((R==1)\&(P<0.5))$ \Comment{the verified smooth region}
\EndFunction
\end{algorithmic}
\end{algorithm}
