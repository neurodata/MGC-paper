...
\newcommand{\jovo}[1]{{\todo{#1}}}
\newcommand{\cs}[1]{{\todo{#1}}}
...
Interpoint pairwise comparison matrices \cite{Maa83}, have been used for over 100 years for various statistical purposes. Perhaps one of the earliest examples  comes from  Karl Pearson, who created Pearson's Product-Moment Correlation \cite{Pearson1895}. Given pairs of observations $(x_i,y_i)$, Pearson's computes the sample covariance and normalized by the product of the sample standard deviations. Kendall  \cite{KendallBook} pointed out that this is a special case of something called a  ``generalized correlation coefficient'', whose general form is:
\begin{equation}
\label{generalCoef}
...
(Spearman and Kendall's rank correlations are also special cases of the generalized correlation matrix).
 % set $a_{ij}$ to be $R(x_i)-R(x_j)$ and $sign(x_i-x_j)$ respectively \cite{KendallBook}; 
Three special cases of the above are particularly popular for independence testing.  First, the Mantel coefficient \cite{Mantel1967}, a very popular method in biology and ecology, 
let's $a_{ij}=||x_i-x_j||_{2}$. Second, Szekely et al. \cite{SzekelyRizzoBakirov2007} define a new method, whereby they let $a_{ij}$ be the ``doubly centered'' distances, that is, they subtract the mean from each row and each column.  Impressively, they proved that this ``distance correlation'' (\Dcorr) statistic is a consistent test for independence for any joint distribution (under suitable regularity conditions), that is, the distance correlation test's power approaches $1$ as sample size approaches infinity, for any joint distribution of finite dimension and finite second moments. Third, Szekely et al. \cite{SzekelyRizzo2013a} noted that \Dcorr~could be generalized to achieve consistency even as the dimensions increase to infinity by simply modifying \Dcorr, essentially by dividing by powers of $n$, the sample size. 
% By further adjusting the properly centered distance entries, the modified distance correlation is consistent as the dimensionality of $x_{i}$ and $y_{i}$ approach infinity \cite{SzekelyRizzo2013a} faster than the sample size. 
...
%  where
where $\bar{a}^{k}$ and $\bar{b}^{l}$ are the sample means of $a_{ij}^{k}$ and $b_{ij}^{l}$, respectively. For example, $\G_{kl}$ can be a local Pearson's correlation by plugging in the respective $a_{ij}$ and $b_{ij}$ from the corresponding global correlation coefficient $\G$. We have a total of $n^2$ local correlations for all possible combinations of $k$ and $l$.
\jovo{i took this out, it is a technical detail ``$\max(R(a_{ij})) \times \max(R(a_{ij}))$ local correlations, which equals $n^2$ when there exists no repeating point in the data.''}
...
\Mgc~computes all $n^2$ local statistics, $\{\G_{kl}\}$, providing all local p-values and selecting the optimal neighborhood size,  denoted  $(k^{*},l^*)$. Clearly the optimal scale exists, is distribution dependent, and may not be unique. But different from a manifold learning task, once the rank information is given, all local correlation coefficients can be simultaneously computed in the same running time complexity as the global correlation coefficient, which allows our definition of local correlations to be an efficient and general tool in data analysis.

If the distribution of the data is already known (of course, this is only for simulations), the optimal scale for \Mgc~can be easily determined via simulation.  When the distribution of data is unknown, we have a procedure for estimating the optimal scale by finding a region of neighborhood sizes for which p-values are consistently low (guarding against noisy scales that appear optimal, and combatting bias added by looking at many different scales). 
\jovo{this stuff was confusing for me: if the underlying joint distribution is known, or multiple dependent data are already given, the true optimal scale can be derived by maximizing the testing powers among all local correlations; if there is only one pair of observations with unknown underlying model, the optimal scale can be reasonably approximated based on p-values of all local correlations from the permutation test. }
Once the optimal scale is determined, we may use the permutation test to calculate the \Mgc~p-value (i.e., the p-value of the optimal local correlation), and declare significant dependency when the p-value is less than $\alpha$, often $0.05$. A schematic illustrating  how to calculate the \Mgc~statistic is provided in Figure~\ref{fig:A}, with more details provided in Appendix \ref{appen:tests}.
...
Figure~\ref{figSim6} illustrates how the powers of local correlations $\{\G_{kl}\}$ change with respect to increasing neighborhood sizes, for the high-dimensional simulations (the $1$-dimensional case is included in appendix)\cs{is this still true?}. They are plotted at a fixed sample size and a fixed dimension by the same power thresholds as in Figure~\ref{fig:pp}(A) and (C). 
...
\label{main4}
To show that \Mgc~is robust against ourliers, let us consider a simple mixture model: suppose the joint distribution satisfies $f_{xy}=w f_{1}+(1-w) f_{2}$, where $f_{1} \sim \mc{N}(\mu_{1},\Sigma_{1})$ and $f_{2} \sim \mc{N}(\mu_{2},\Sigma_{2})$. We set $\mu_{1}=(0,0)$, $\mu_{2}=(5,-5)$, $\Sigma_{1}$ is a matrix of all ones, 
% $ = \mb{1}\T\mb{1}$, $\begin{bmatrix} 1&1\\ 1&1 \end{bmatrix}$,
 ...
 The above mixture implies that with probably $w$,  $x=y$, and with probability $(1-w)$ $x$ and $y$ are independent. Given $w$, the simulating sample pairs $(X,Y)$ at $n=100$ can be repeatedly generated to estimate the testing powers at $\alpha=0.05$. 

For sample data generated from the mixture model, there are around $w \times n$ observations in $X$ that are exactly the same as the corresponding observations in $Y$; and the remaining observations in $X$ are outliers, which are independent of the corresponding observations in $Y$. One such sample pair $(X,Y)$ at $p=0.5$ and $n=100$ is plotted in Figure~\ref{figSim3}(A), and the testing powers of all local mcorr correlations are shown in Figure~\ref{figSim3}(B)(C)(D) for $p=0.3$,$0.5$,$0.7$ respectively, using the same procedure as in section~\ref{numer1}. \cs{fix after fixing figure}

The optimal local mcorr is clearly much better than global mcorr. Furthermore, the optimal scale $(k^{*},l^{*})$ is directly related to the mixture probability $w$, where $k^{*}=l^{*}=\min(wn,(1-w)n)-1$ is always optimal. Intuitively, \Mgc~is robust against outliers as the optimal local correlation automatically excludes the independent outliers from the dependent observations in testing. The robustness still holds as the dimension, sample size, or the underlying dependency change, which makes \Mgc~more advantageous in practice against noise and outliers. \cs{explicilty mention that we discover the fraction of outliers.  and the other don't}
...
\end{table*}
\cs{maybe bold everything in table that is significant?}
...
Using the same testing procedure as the previous experiment, the resulting p-values for \Migraine~vs CCI and \mtg~vs CCI are reported in the last two rows of Table~\ref{table1}. The dependency between \Migraine~and CCI is strong enough for all dependence tests to return significant p-values. But there appears to be a loss of dependency between \mtg~and CCI, as neither the global methods nor \Mgc~return significant p-values. 
