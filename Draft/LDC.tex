...
\label{main2}
In this section we present the theoretical advantages of multiscale graph correlation. The proofs are provided in the appendix section~\ref{appen:proofs}. 
...
\label{numer1}
In this section and the next, we show the numerical advantage of multiscale graph correlation (using mcorr / dcorr / Mantel) via the empirical testing powers. The detailed evaluation procedure is provided in appendix section~\ref{appen:tests}, and the benchmarks are mcorr, dcorr, Mantel, and HHG.
...
\jv{ok, propose a modified paragraph that is correct?}
\cs{now I begin to think the above paragraph is actually fine, depending on how I look at it...so I keep it for now}
...
\begin{align*}
\mb{x} &= p \mb{u} + (1-p) \mb{\epsilon}_{1}, \\
\mb{y} &= p \mb{u} + (1-p) \mb{\epsilon}_{2}, 
\end{align*}
where $\mb{u}$ is uniformly distributed in $(-1,1)$, $\mb{\epsilon}_{1}$ and $\mb{\epsilon}_{2}$ are independently normally distributed with mean $-5$ and $5$ respectively and variance $1$, and $p$ is the prior probability. Given $p$, the simulating sample pairs $(X,Y)$ at $n=100$ are repeatedly generated to estimate the testing powers at $\alpha=0.05$. \jv{can you explicitly write the mixture model, i.e., F = F1 + F2, and specify both F1 and F2? this will make it more clearly a mixture model, and in particular, a mixture of a linear function and an independent function.}
\cs{isn't the above formulation already in the form of F=p F1+(1-p)F2, where $p$ is the mixture weight, and $\mb{u}$ is the linear function common in both models?}
...
\label{numer3}
Here MGC is applied to test independence between real data sets. Based on the estimation and evaluation algorithms in section~\ref{appen:tests}, the p-value of each method from the permutation test is reported. In this section we will concentrate on MGC$\{$mcorr$\}$ among the three MGC implementations.
...
To apply the methods, the respective data set needs to be transformed into a distance matrix. For the brain data, two dissimilarity matrices representing the left and right hippocampus data are already obtained based on landmark matching (see \cite{ParkEtAl2011} for more details on data processing). For the disorder information vector, it is transformed into the Euclidean distance matrix with all off-diagonal entries added by $1$, i.e., only the diagonals are zero, and subjects of the same disorder status have smaller distance than subjects of different disorder status. 
...
The p-value curves of the local tests are provided in Figure~\ref{figReal}(a) with respect to the choice of $k$ at $l=4$. We only show the curves at $l=4$ because the p-values of local test statistics at a given $k$ are the same throughout any $l \in [4,114]$ (the largest rank is $3$ for the disorder distance matrix), and the scales at $l<4$ are neither optimal nor yield significant p-values. The estimated optimal scales are $k^{*}=65$ and $k^{*}=64$ (and $l^{*}$ can be any number larger than $3$) respectively for left brain and right brain vs disease, based on $2,000$ MC replicates. As the optimal scales are very similar, it indicates the left brain and the right brain may have similar dependence structures when testing against the disease.

\jv{make a comment about how the optimal scales are the same, and what this means? i don't know what it means, something about the same "model" is appropriate for both hemispheres, which is what we would hope}
\cs{ok I added it above.}
\jv{i don't think it makes sense to show a matrix for neighborhood choice of Y.  there are only 2 different choices, because Y is binary.  just show the 2 curves?}
\cs{Y is not binary, but the largest rank is $3$. So I replace the heatmaps by p-value curves wrt k at $l=4$}
...
\jv{is it the same l because y is the same for the two? did it have to be the same l?}
\cs{not necessarily. If y is the same, but x differs a lot, then l won't be the same. But Migrain and M2g is similar to each other, so the scales are similar; the same phenomenon holds for brain vs disease.} 
Note that there exists many local scale indicating significant dependency in Figure~\ref{figReal}(c); but only a few such scale in Figure~\ref{figReal}(d), none of which is the optimal scale for testing power.
...
\jv{why does this indicate strong \emph{linear} dependency?}
\cs{\emph{linear} is deleted; since it can cause confusion...}
...
\caption{Testing Power Heat map of Local Statistics with respect to Neighborhood Choices for Dimension 1. See Figure \ref{figSim2} for details.}
\label{figSim2a}
\end{figure}
...
The second algorithm computes the testing powers of all local statistics, by repeated simulating samples generated from the joint distribution $f_{xy}$ or the given observations $(X, Y)$. In case of known $f_{xy}$, it is easy to generate sample data under the null and the alternative; in case of given data of unknown $f_{xy}$, we treat the upper-diagonal entries of the doubly-centered distance matrices as independent scalar samples, 
\jv{this isn't quite right, and it is confusing.  we treat those as independent scalar samples.  please clarify here and later below in the discussion of this choice}
\cs{yes! it is all changed accordingly}
use kernel density estimation \cite{BotevEtAl2010} to estimate and repeatedly generate sample distance matrices under the alternative. By estimating all local powers and maximizing the power, this algorithm can be used to either estimate the optimal scale of MGC or estimate the testing power of MGC.
...
\If{$f_{xy}$ is not given}
\State $C=dist(X)$; $D=dist(Y)$; $H=diag(n)-ones(n)/n$; 
\State $meanC=C- H \times C \times H $; $meanD=D- H \times D \times H$; 
\State $f_{CD}=kernelDensity(upper(C-meanC),upper(D-meanD))$; \Comment{kernel density estimation on the upper-diagonal doubly-centered distance entries}
\EndIf
\For{$m=1,\ldots,r$} 
\If{$f_{xy}$ is given}
...
\Else
\State $(C_{A},D_{A})=generateData(f_{CD},'dependent')$; \Comment{generate dependent doubly-centered distance matrices}
\State $C_{A}=C_{A}+meanC$; $D_{A}=D_{A}+meanD$; \Comment{add back the means to form dependent sample distance matrices}
\State $per1=randsample(n)$; $per2=randsample(n)$; 
\State $C_{N}=C(per1,per1)$; $D_{N}=D(per2,per2)$; \Comment{generate independent sample distance matrices}
\EndIf
...
\EndFor
\EndFor
%\State $scale=find(power==max(power))$; \Comment{find the optimal local scale}
\State \Return $power$.
\EndFunction
\end{algorithmic}
\end{algorithm}
...
To evaluate MGC in simulations or real data, the optimal scale of MGC needs to be estimated first. By running Algorithm~\ref{algPower}, it estimates the testing powers $\hat{\beta}_{\alpha}(g_{kl})$ of all local statistics, so the optimal scale can be estimated by maximizing the empirical powers:
\begin{equation}
\label{power}
...
The same procedures can be applied to evaluate the p-values and powers for global correlations like dcorr / mcorr / Mantel / HHG, by excluding the first step for estimating the optimal scale. 

\subsection{Discussions on Optimal Scale Estimation}
\label{appen:discuss}
If only one pair of observations $(X,Y)$ are given with the underlying joint distribution being unknown, the optimal scale estimation (i.e., Algorithm~\ref{algPower}) has to rely on the given data; and to estimating the testing power of local test statistics, we need to repeatedly generate dependent distance matrices $(C_{A},D_{A})$ for the alternative statistics, and two independent distance matrices $(C_{N},D_{N})$ for the null statistics.

The independent distance matrices can be easily generated by using two independent re-samples of $X$ and $Y$. To generate dependent distance matrices, the simplest way is to use the same re-samples for both $X$ and $Y$, which may cause some bias in MGC p-value calculation: since the permutation test is also carried out on $(X,Y)$, the MGC p-value calculation is not independent of optimal scale estimation based on re-samples of $(X,Y)$, such that the estimated p-value can be smaller than the true MGC p-value (i.e., the true MGC p-value equals the p-value at the optimal scale based on the joint distribution). 

Therefore, we use kernel density estimation in Algorithm~\ref{algPower} to generate dependent distance matrices rather than re-samples: the upper-diagonal entries of the doubly-centered distance entries are treated as independent scalar samples, and the joint distribution of the paired entries is estimated by normal kernels using the method in \cite{BotevEtAl2010}. This allows us to efficiently generate distance matrices that are independent of the given observations $(C,D)$ for optimal scale estimation. Comparing to the naive re-sampling method, using kernel density estimation to generate dependent distance entries for optimal scale estimation can almost eliminate the MGC p-value bias at the cost of slightly reduced testing power. 

Alternatively, it is possible to use the naive re-sampling method to estimate the testing power without bias, if at least two pairs of dependent data of similar dependency structures are given: one pair of observations can be used to estimate the optimal scale, while the other pair can be used to derive the MGC p-value. Take the brain vs disease experiment for example, we can observe from the p-value curves that if the optimal scale estimated from left brain vs disease is used for MGC testing between right brain vs disease, the resulting MGC p-value is still significant than all other benchmarks, and vice versa.  
\jv{why are we maximizing a sum of testing powers? summing over what? monte carlo? samples?}
\cs{this could be confusing; so I deleted this part.}

As a side note, our approach also offers a fast and reliable way to test dependency on large amount of data. For example, although distance correlation are consistent, under certain dependencies it may require too large a sample size to reliably test independence (i.e., achieve a good testing power), for which the running time can be a major issue; sub-sampling of the large data followed by distance correlation can improve the running time, but may have a lower testing power. MGC is an ideal choice in this scenario: we may take two independent sub-samples from the population, one to estimate the optimal scale by simple re-samples, and the other to calculate the MGC p-value. This approach yields an unbiased p-value, enjoys an equivalent or higher testing power than distance correlation using sub-sampling, and is still efficient in running time. Therefore MGC can be an invaluable tool for big data.

\jv{we should plot k.hat vs k.star and l.hat vs l.star for at least a few examples (maybe all 20 of them), to see whether our estimation procedure is finding the right answer?
alternatively, we can plot power.hat vs actual power for all (k,l), for each setting, which might be more informative? i remain a bit worried about our procedure without ever seeing how well it actually estimates k and l.}
\cs{I have roughly checked the power.hat vs actual power for 300 MC replicates: power.hat is usually close to or a little lower than the actual power under some dependencies; but it is still higher than or the same as global dcorr and HHG in most cases. One problem is that deriving power.hat directly from a permutation test is not efficient and very time consuming for large $n$ or large number of replicates, comparing to the actual power from independence test; the other problem is that providing power.hat may make our MGC less attractive than providing the true power, though the true power may not be attainable if only one pair of data is provided. If the reviewers ask for this, we can certainly provide it then; and we may want to investigate and prove better ways to estimate the optimal scale, if ever possible... }
...
{\begin{tabular}{|c||c|c|c|c|c|c|c|}
\hline
Data Set & MGC & mcorr & dcorr & Mantel & HHG \\
\hline
...
The last row shows the mean and the standard deviation. \jv{this caption is wrong, right? this isn't power, it is false positive rate?}
\cs{yes;changed}}
\label{table2}
}
\end{table*}
