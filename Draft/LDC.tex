...
Figure~\ref{f:nD} shows the testing powers of \Mgc, \Mcorr, and \Hhg~versus the dimensionality of \mbx~(see Methods for details), with the sample sizes fixed at $n=100$ for each simulation. Note that for all dependencies, the dimensionality of \mbx~is increasing, but the dimensionality of \mby~increases in only a subset of the settings (see Appendix \ref{appen:function} for details). 

The advantage of \Mgc~over its global counterpart and \Hhg~are very stark. For the nearly linear settings, \Mgc~and \Mcorr~are essentially identical and significantly better than \Hhg~as the dimension increases.  For the remaining nonlinear dependencies, \Mgc~consistently achieves superior powers than all benchmarks for \emph{all} functions, often by a significant margin.  Finally, for the independent simulation, all tests yield powers at the significance level $\alpha$, as they should, indicating no more false positives than there should be. 
