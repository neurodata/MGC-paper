...
% * <stefaniejacinto@gmail.com> 2016-08-15T02:14:58.154Z:
%
% > \textbf{structured}
%
...
% this paragraph could use some love
...
\Mgc~uses the smoothness of these maps to conduct efficient and valid searches.
Specifically, \Mgc~looks for the largest rectangular region of smooth p-values.
All consecutive scales within the smooth region having the same or smaller p-values are the estimated optimal scales (Algorithm \ref{alg:best_scale}), and the maximum p-value within that region is the p-value.
...
% * <stefaniejacinto@gmail.com> 2016-08-16T05:48:20.835Z:
%
% > Powers
%
% In Figure 2, in panels 16 and 17, remove colons after 4 and 16
%
% ^.
...
\cs{please put the above content that stefanie removed into the description of the algorithm, and remove the colons}
\jv{done, in alg 4}
...
% * <stefaniejacinto@gmail.com> 2016-08-16T04:58:28.199Z:
%
% > with
%
% Not sure why, but the whitespace seems to be getting disregarded in between "MGC" and "with" when generating the PDF.
%
% ^.
% 
% \sd{we need to put a "~" to ensure their is a space because of the way we defined the macro}
...
Even when estimating the optimal scales via p-value maps, the sample \Mgc~achieves higher power than \Mcorr~and \Hhg~in nearly all settings, often very close to the power of Oracle \Mgc,  especially high-dimensional ones (see Appendix~\ref{appen:figs}).
...
\begin{figure}
  ...
  \caption{
Average powers slopegraphs comparing tests by global correlation and the respective oracle \Mgc.  Each panel shows average power for settings 6-19, because \Mgc~equals the global correlation for simulations 1-5 and  20. 
...
% * <stefaniejacinto@gmail.com> 2016-08-16T05:50:18.640Z:
%
% > Multiscale
%
% In Figure 4, in panels 16 and 17, remove colons after 4 and 16
%
% ^.
...
\begin{equation}
\beta_n(\G^{*}_{t}) > \beta_n(\G^{k,l}_{t}) > \beta_n(\G_{t}).
\end{equation}
...
Figure \ref{f:real}{\color{magenta}A}  shows that many local scales yield significant p-values (around $0.01$), whereas the global scale fails to detect this significant dependence. The previously proposed dependence test under consideration (\Mantel, \Dcorr, \Mcorr, or \Hhg) also all fail to detect dependence at a significance level of $0.05$. The sample \Mgc~p-value is $0.0285$, \Hhg~yields a nearly significant p-value $0.0588$, while the rest all have p-values higher than $0.3$, and none of the other tests provide any further insight into the scales of dependence.
% * <stefaniejacinto@gmail.com> 2016-08-16T06:09:49.146Z:
%
% > A
%
% Is there any way to get this A into red font, for consistency?
%
% ^.
...
Figure \ref{f:real}{\color{magenta}B} provides the p-value map for testing on the right hemisphere using \Mgc. Again, many local scales yield significant p-values, whereas the global scale does not detect a significant dependence. The sample \Mgc~p-value is $0.0087$, and no other dependence test can identify significant dependence. The left hemisphere seems less dependent on disease status, 
% * <stefaniejacinto@gmail.com> 2016-08-16T06:17:07.438Z:
%
% > B
%
% Try to make this in red font, if possible (for consistency)
%
% ^.
% is more globally dependent on disease: 
% however, \Mgc~equals the corresponding global correlation, and 
as both local and global tests yield  p-values from $0.05$ to $0.08$ (not shown). 
\jv{let's check whether anybody knew that?}
\cs{what are p-values for Mcorr and HHG on the right hemisphere?}
\jv{0.1132 and 0.1208 respectively}
...
\begin{figure}
  ...
  \end{tabular}
\caption{
(A) P-value map of all local correlations for fMRI brain scan vs five-factor personality model.
(B) P-value map for right hemisphere of brain vs disease.
...
% Specifically \Mgc~provided the scales for which the dependence is strongest.  
...
% * <stefaniejacinto@gmail.com> 2016-08-17T05:43:23.339Z:
%
% > a non-existent stimulus
%
% Added an "a" here. But if this was supposed to be plural, remove the "a" and change "stimulus" to "stimuli".
%
% ^.
...
\cs{should those distances be squared?}
\jv{no}
...
We demonstrate via simulations that \Mgc~performs well empirically regardless of the nonlinearity, dimension, or sample size.  Moreover, in addition to a valid p-value, \Mgc~provides a map enoding which scales contain the dependence structure. We then prove that \Mgc~dominates global generalized correlation coefficients in finite samples.
% * <stefaniejacinto@gmail.com> 2016-08-17T05:55:44.925Z:
%
% > enoding
%
% showing? encoding? please confirm
%
% ^.
...
% * <stefaniejacinto@gmail.com> 2016-08-17T06:00:07.399Z:
%
% > manuscript's
%
% Added " 's" here, please confirm this is what you meant
%
% ^.
...
\jv{not sure, let us discuss iin person tmr}
...
\State $\gamma \rto 0.04$ \Comment{threshold to check significant global p-value and significant region}
\If{$\sum (P<P[n,n])/n^2<\gamma$} 
\State $p \rto P[n,n]$ \Comment{use the global p-value when it is small enough}
\State $[k^{*},l^{*}] \rto [n,n]$
\Else
\State $R=\textsc{SmoothRegion}(P,\tau)$ \Comment{smooth region within p-value map}
...
\State $R^{1} = \textsc{LargestRectangle}(P<\tau)$ \Comment{largest rectangle with decreasing trend within row} 
\State $R^{2} = \textsc{LargestRectangle}(P>-\tau)$ \Comment{largest rectangle with increasing trend within row} 
\State $R^{3} = \textsc{LargestRectangle}(P^{'}<\tau)$
\State $R^{4} = \textsc{LargestRectangle}(P^{'}>-\tau)$
\State $R = \max_{i}\{\sum_{k,l}{R^{i}_{kl}}, i=1,\ldots,4\}$ \Comment{use the largest region}
\EndFunction
\end{algorithmic}
\end{algorithm}
