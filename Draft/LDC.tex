...
\end{equation}
where $z$ is the product of standard deviations multiplied by the number of entries, that is $z=n^2\sigma(a) \sigma(b)$. %denote the sample means of all $\{a_{ij}\}$ and $\{b_{ij}\}$.
...
\end{equation}
where $z_{kl}=n^2 \sigma(a^{k})\sigma(b^{l})$ with $\sigma(a^{k}), \sigma(b^{l})$ being the standard deviations for the truncated pairwise comparisons. There are a maximum of $n^2$ different local correlations, one for each possible combinations of $k$ and $l$ (more technical details of \Mgc~are in Appendix \ref{appen:mgc}).
...
\Mgc~requires the computation of all $n^2$ local statistics, $\{\G_{kl}\}$, among which the best local statistic is used for testing (see Pseudocode \ref{alg1} in Appendix \ref{appen:algorithms} for details on how to compute \Mgc~for a given neighborhood size).
...
\jv{we should defined \Mgcd, \Mgcm, \Mgcp at this point, which I added above; and changed figure legend for this.}
