...
Our key insight is that we can restrict the analysis to jointly local observations---that is, observations that are local in both properties---yielding an approach that statistically dominates previous approaches while maintaining computational efficiency.
...
Dependence tests have developed specifically to address the above issues.  Hoeffding and Renyi proposed non-parametric tests to address nonlinear relationships \cite{Hoeffding1948,Renyi1959}.  In the 1970s and 1980s nearest neighbor style approaches were popularized \cite{Stone1977}, including for dependence testing purposes \cite{Friedman1983,Schilling1986}.  These tests discard all but the $k$ nearest neighbors for both properties, and then simply compute the fraction of pairs that are ``jointly neighbors'', that is, pairs that are neighbors in both measurement types. This requires specifying $k$ in advance, and ignores the magnitudes of the correlation, meaning that when $k$ is large the resulting test statistic is  uninformative. 
% 
More recently, these ``local'' tests were discarded for global approaches.  The Distance correlation test (\Dcorr) was proven to work asymptotically for arbitrary nonlinearities \cite{SzekelyRizzo2009}, for large dimensions \cite{SzekelyRizzo2013a}, and structured data \cite{Lyons2013}.
Around the same time, Heller, Heller and Gorfine proposed a test (\Hhg), that shared similar properties.  Empirically however, \Dcorr~performs well in high-dimensional linear data, whereas \Hhg~performs well in low-dimensional nonlinear data,
but no test performs particularly well in high-dimensional nonlinear data, which characterizes a large fraction of real data challenges in this big data era.
...
\includegraphics[height=1.0\textheight,trim={0cm 2cm 0 2cm},clip]{Figures/Fig1Allb.pdf}
...
\Mgc~explicitly addresses the fundamental trade-off suggested above: as more neighbors are included (as the scale increases), the effective sample size increases; however, more neighbors also means the neighbors are less likely to be linearly related, potentially decreasing power.
\textbf{Our main insight is that we can adaptively estimate informative scales for any relationship---linear or nonlinear, low-dimensional or high-dimensional, unstructured or structured---in a computationally efficient and statistically consistent fashion, therefore guaranteeing improved statistical performance over existing global methods for any finite sample size.}
...
Unlike \Dcorr, which is a global test, and Friedman's test, which specifies a local scale and then merely counts the number of joint neighbors,  
\Mgc~searches for the  maximum ``local generalized correlation''.
...
The estimated optimal scales (green boxes) are all the scales that are maximally informative for the given dataset.
