...
% Our key insight is that we can adaptively adjust the locality of our analysis
...
% @jovo: yielding....is a different sentence.
...
% @jovo: maybe something about  "including our motivating problems of brain activity, shape, and networks."
...
\begin{figure}[h!]
  \centering
%  trim={<left> <lower> <right> <upper>}
  \includegraphics[width=1.0\textwidth,trim={3.5cm 0 3.5cm 1.0cm},clip]{Figures/FigHDPowerMGCM}
...
% @jovo: green circle and rectangle, maybe do more explicitly.
}
\label{f:powermaps}
\end{figure}
...
Furthermore, similar dependencies have similar local generalized correlation structure, and thus, similar multiscale power maps. For example, (10) and (11), though very different functions analytically, are qualitatively similar, and yield very similar power maps.
...
\Mgc~can be applied to real data scenarios with complex structures, provided appropriate distance measures are available. We apply \Mgc~to three different scenarios: (i) brain activity versus personality, (ii) brain shape versus disease, and (iii) brain networks versus creativity.  
% These three brain properties span the most popular \emph{in vivo} whole brain techniques and representations of the brain: functional, structural, and diffusion magnetic resonance imaging, while the different mental properties span cognition and health.  
...
For each scan we  estimated brain networks from diffusion and structural MRI data via  \Migraine, a pipeline for estimating brain networks from diffusion data \cite{GrayRoncal2013}.  Then, we compute the distance between the graphs using the semi-parametric graph test statistic \cite{Sussman2013,ShenVogelsteinPriebe2016,Tang2016}, embedding each graph into two dimensions for simplicity and aligning the embeddings via a Procrustes analysis.
% (i) process diffusion and structural MRI data via  \Migraine, a pipeline for estimating brain networks from diffusion data \cite{GrayRoncal2013}, (ii) 
...
% we are surprised here that global scale does better than local scales. did we reveal the nature of?
...
% @jovo: maybe move this stuff to appendix.
...
% * <stefaniejacinto@gmail.com> 2017-01-24T01:24:02.037Z:
% 
% > [SUGGEST:]
% > \GG(X,Y)= \tfrac{1}{z} {\textstyle \sum_{i=1}^n \sum_{j=1}^n a_{ij} b_{ij}}
% @jovo: I think it would be clearer if we used a double sigma notation here instead of i,j=1
% -@sd
% 
% ^.
\GG(X,Y)= \tfrac{1}{z} {\textstyle \sum_{i=1}^n \sum_{j=1}^n a_{ij} b_{ij}},
