...
Thus $g_{nn}$ is the corresponding global test statistic, and $\{g_{kl}\}$ becomes our family of multiscale/local test statistics, which includes the global one. Clearly the optimal local test exists, is distribution dependent, and may not be unique; the optimal neighborhood choice can be determined by calculating and maximizing the testing power of the multiscale family for given dependent distribution, or estimated by resampling on the given data of unknown distribution; more details are in Section~\ref{main3} of the appendix.
...
\cs{Not the actual running time. We can, but LGC will be the slowest (though not much), in either R or Matlab version. If it can be efficiently implemented in FlashR, it may be worthwhile to add it.}
...
\jv{i don't quite get $y_ \sim \mb{y}$, i think i know what you mean, but why not write $\mb{y} \sim f_y$, since we define $f_y$ as the marginal distribution below? then, $x_i$ are \emph{realizations} of $\mb{x} \sim f_x$}
\cs{Done}
...
\jv{the issue with theorem 2 being in the main paper, is that it doesn't contribute to the main arguement of the paper, it is secondary, at least, that is how i see it.  if you can explain why it is primary, then it makes sense to leave here.}
\cs{I get it. Feel free to move it then.}
...
\jv{ why $\beta_\alpha(g)$, rahter than $\beta_\alpha{g_{kl}}$? i don't think we ever defined the latter term, explicitly?}
\cs{ We do not need the individual powers in the theorems, since individual local test is not always consistent; and it looks more elegant to use $\beta_\alpha(g)$ only. The latter term is used only in the appendix.}
...
\cs{I added Rudin's real analysis book; let me know if you want something else :)}
...
\jv{explain whether we change any parameters from their settings, and if so, why.}
\cs{There is no parameter change for the old settings; but type 3,12,13,19 are our own addition based on their old setting. The main difference is the weight vector, and the noise level, described above.}
...
\end{align*}
\item Joint normal: Let $\rho=\frac{1}{2d_{x}}$, $I_{d_{x}}$ as the identity matrix of size $d_{x} \times d_{x}$, $J_{d_{x}}$ as the matrix of ones of size $d_{x} \times d_{x}$, and $\Sigma = \begin{bmatrix} I_{d_{x}}&\rho J_{d_{x}}\\ \rho J_{d_{x}}&I_{d_{x}} \end{bmatrix}$. Then let $(\mb{x},\mb{u}) \sim \mc{N}(0, \Sigma)$ and $\mb{y}=\mb{u}+0.5c\epsilon$. 
...
\end{align*}
\item Logarithmic: $\mb{x} \sim \mc{N}(0, I_{d_{x}})$,
\begin{align*}
...
\begin{align*}
\mb{y}&=sin(4\pi \frac{w\mb{x}}{\max(w\mb{x})})+c\epsilon.
\end{align*}
\item Sine Period 1/8: $\mb{x} \sim \mc{U}(-1,1)^{d_{x}}$,
\begin{align*}
\mb{y}&=sin(16\pi \frac{w\mb{x}}{\max(w\mb{x})})+0.5c\epsilon.
\end{align*}
\item Multiplicative Noise: $\mb{x} \sim \mc{N}(0, I_{d_{x}})$, $\mb{u} \sim \mc{N}(0, 1)$,
\begin{align*}
\mb{y}&=\mb{x} \cdot \mb{u}+0.5c\epsilon.
\end{align*}
\item Uncorrelated Binomial: $\mb{x} \sim \mc{B}(0.5)^{d_{x}}$, $\mb{u} \sim \mc{B}(0.5)$,
\begin{align*}
\mb{y}&=w\mb{x} \cdot (2\mb{u}-1)+0.6c\epsilon.
\end{align*}
...
\State 2. \textbf{Estimate the optimal neighborhood of MGC by bootstrap}:
...
\State 3. \textbf{Calculate the p-value of Multiscale Graph Correlation by permutation test}: 
