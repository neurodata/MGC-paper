...
% Computationally efficient MATLAB and R code are provided here: \website.
\cs{I commented out the sentence regarding false positives, because 1: we are actually valid now, 2. those false positives are not plagues by dependence tests in their paper, but rather some regression methods. What do you think?}
\end{abstract}
...
% operate on scalars or vectors, rather than more complicated objects.
% \jv{"rather than more complicated objects" not obviously useful}
%
...
% 
Fourth, the dependencies between different properties (or experimental modalities) of data can be highly \textbf{nonlinear}.  While this has always been the case, the relative abundance of data has led to an increased demand in checking for dependence in many previously uninvestigated settings.
%
Fifth, because of the near ubiquity of data deluges, it is important to have a  \textbf{computationally efficient} method of generating results.
%
...
% Nonlinear functions, however, can be \emph{locally approximated} by linear functions \cite{Allard2012}.
% Therefore, even for highly nonlinear  functions like a spiral, there are pairs of points that are both close in $x$ and $y$ (such as points $1$ and $2$);   but there are also pairs of points that are close only in $y$ and not $x$  (points $2$ and $3$).  
% Thus, locality is a joint property of both $x$ and $y$.
% Test statistics for dependence typically operate on \emph{all} joint pairs (that is, $(\mbx_i,\mbx_j)$ and $(\mby_i,\mby_j)$ for all $(i,j)$ pairs), making them well-suited for detecting global  relationships. However, nonlinear relationships evident on finer scales can be missed in the global din. 
\Mgc~achieves this by identifying the jointly local pairs, 
% containing the most information about the dependence structure, 
and then using only those scales to conduct its test, thereby revealing both the existence and scales of dependence.
% flexibly determines the scale(s) at which to conduct its test...
%
% We desire an approach that only 
% MGC ... by 
...
% below are the numbers rounded to nearest 0.1
% &   & $\mbx$ & $\mby$ 
% & 1 & -5.0 & 0.2
% & 2 & -4.9 & 0.8 
% & 3 & -3.0 & -0.3
...
\multicolumn{1}{l}{\textbf{6. Table}} & & & \\
$\delta_x$(1,2)   & \hspace{1.5em} \color{magenta}-2.32  & \hspace{3.5em} \color{magenta}-2.75  &  \hspace{3.0em} \color{magenta}-3.60  \\ 
 $\delta_y$(1,2) & \color{magenta}-1.74 & \color{magenta}-1.58 & \color{magenta}-1.58  \\ 
 $\delta_x \times \delta_y$ & \color{green}4.02 & \color{green}4.34 & \color{green}4.34  \\ 
 
\hline

 $\delta_x$(2,3) & \color{green}4.18 & \color{green}3.61 & 0.00  \\ 
 $\delta_y$(2,3) &  \color{magenta}-0.54 & \color{magenta}-0.03 & 0.00  \\ 
 $\delta_x \times \delta_y$ & \color{magenta}-2.25 & \color{magenta}-0.12 & 0.00  \\ 

\hline
 $\sum{\delta_x \times \delta_y}$ & \color{green}254.52   & \color{green}61.11 & \color{green}74.85  \\ 
 test statistic &  \color{green}0.04  & \color{green}0.01 & \color{green}0.12  \\  
...
\node[xshift=-5.2cm,yshift=-0.6cm] at (current page.east){%
    \includegraphics[width=0.35\textwidth]{Figures/FigB}};
...
and their element-wise product $\tilde{C}=\tilde{A} \circ \tilde{B}$ (bottom), whose normalized sum is the  \Mantel~statistic \cite{Mantel1967} (bottom row of table).
% 
...
\textbf{4.} Compute the correlation (top), power (middle), and p-value (bottom) for all local scales, resulting in multiscale maps that reveal the  scales of dependency. 
...
Whereas \Mcorr, the global test, has very low power (magenta dot in multiscale p-value map) and therefore yields a non-significant p-value ($0.187$),  there are many local scales that achieve nearly perfect power, so both Oracle and sample \Mgc~($\G^{*}$ and $\hat{\G}^{*}$) obtain a highly significant p-value ($\approx 0.001$) and reveal the scales of dependency. 
...
\label{f:schematic}
\end{figure}
\cs{probably doesn't need any more re-wording in fig 1 caption?}

% jv{kernel machines project data into a linear regime}
...
% The next step is to ``single-center'' these matrices.  
...
Thus, existing generalized correlation coefficient based tests  work well in nonlinear and high-dimensional data,  including structured domains. 


Empirically, however, distance correlation tests struggle in various nonlinear settings. We conjectured that their struggles in nonlinear and high-dimensional settings were due to the fact that they were \emph{global methods}, and therefore they consider all pairs of points, including non-local ones. They also cannot provide insight into the scales of the dependence. We therefore adopted the locality principle, which has already reaped considerable benefits in a wide range of data science problems, including  classification and regression  \cite{Stone1977}, data compression \cite{DaubechiesWaveletBook}, and recommender systems \cite{Sarwar2000}.
...
the third step is to compute a local variant of the distance covariance test.
...
The fourth step is therefore to compute all local tests in a nested algorithm, re-using adjacent smaller local correlations to procedurally build all local statistics, efficiently yielding multiscale maps that reveal the scales of dependence. 
...
\cs{I slightly rephrased the next paragraph for sample mgc, and added definition of $\G^{*}$ and $\hat{\G}^{*}$: they have been used in figure 1 without defining.}
The fifth step is to report a valid p-value for testing, and to calculate the optimal scale for discovering dependency structure. The optimal scales for any given distribution and finite sample size are defined as the scales that maximize power (the probability of correctly rejecting a false null hypothesis).  In simulations, or datasets for which we have a known model or extensive training data, we can find the optimal scales by maximizing the testing powers; ``Oracle \Mgc'' chooses the optimal scale in this fashion  (Algorithm \ref{alg:power}), for which the optimal local correlation $\G^{*}$ can be used in a permutation test to yield the p-value.
In real data settings, the power of a test cannot be known because the true distribution is often unavailable, so  we design a ``sample \Mgc'' tailored for local \Mcorr, which provides an estimation $\hat{\G}^{*}$ of the optimal correlation from the local correlation map (Algorithm \ref{alg:sample_mgc}) without relying on the power map. The resulting p-value by sample \Mgc~is then compared to the p-value map (Algorithm \ref{alg:pval}), for which the largest rectangle with same or less local p-values are taken as the estimated optimal scale. Neither the Oracle \Mgc~or sample \Mgc~suffers from the multiple hypothesis testing problem \cite{Benjamini1995}.

The table in Figure \ref{f:schematic} illustrates how \Mgc~is able to detect dependence, even in this spiral example with only $50$ samples, while the global generalized correlation tests fail.  Samples $1$ and $2$ are close to each other in both $x$ and $y$; they therefore contribute to \emph{positive} dependence for all three tests.  However, samples $2$ and $3$ are only close in $x$, not in $y$.  Therefore, in the global tests, \Mantel~and \Mcorr, pair $(2,3)$   \emph{negatively} contributes to dependence.  \Mgc~achieves a larger value of the test statistic in this scenario by ignoring  pairs like that, thereby achieving a smaller p-value, and greater power in general nonlinear settings.  


\subsection*{Finite Sample Simulation Experiments}

% \jv{the job of a paper characterizing a novel method is to understand the settings in which the new approach outperforms existing approaches, and the settings in which it does not.}
...
All three algorithms require specifying distances, we define  $\delta_x$ and $\delta_y$  as  Euclidean distance, that is, $\delta_x(\mb{x}_i,\mb{x}_j) = \|\mb{x}_i - \mb{x}_j\|_{2}$, and the same for $\delta_y$. 
...
% \jv{the first sentence should not be merely descriptive, but also get across the concepts of the problem, and motivation/question/gap}
Figure~\ref{f:nD} shows  the power of each test for all $20$ settings with sample size fixed at $n=100$, but with dimensionality of \mbx~increasing (the dimensionality of \mby~increases in only a subset of the settings; see Methods for details).  
%
\cs{I added one sentence saying sample \Mgc is similar to Oracle \Mgc. Sufficient?}
The advantage of Oracle \Mgc~over its global counterpart \Mcorr~and \Hhg~is stark, with sample \Mgc~performing very close to Oracle \Mgc. For the nearly linear settings, \Mgc~and \Mcorr~are essentially identical and significantly better than \Hhg~for all dimensionalities.  For the remaining nonlinear dependencies, \Mgc~achieves power levels that are superior to that of \Hhg~and \Mcorr~for almost all functions, often by a significant margin.  For the independent simulation, all tests yield powers at the significance level $\alpha$,  indicating no more false positives than expected.
...
% \jv{last sentence says the "claim" of the paragraph}
% The conclusion is that \Mgc~outperforms the current state of the art in essentially all settings and dimensionalities.
...
%Even when estimating the optimal scales via p-value maps,  sample \Mgc~achieves equivalent or higher power than \Mcorr~and \Hhg~in nearly all settings, and is often very close to the power of Oracle \Mgc~(see Appendix~\ref{appen:figs}).
\cs{commented out fig permutation sentences. }
...
For nearly linear dependencies (1-5), the best neighborhood choice always includes the largest scale, i.e., $k=l=n$. For all strongly nonlinear dependencies (6-19), Oracle \Mgc~chooses smaller scales for $x$ or $y$. This implies that a global optimal scale indicates that the underlying dependency is nearly linear, otherwise the dependency is strongly non-linear.
...
%
Note that the local structures can be similarly identified by the p-value map or the local correlation map, and there always exists a large smooth region of local scales that are nearly equally significant in the multiscale maps. These are two important observations that we utilize to design the sample \Mgc~in algorithm~\ref{alg:sample_mgc}.
\cs{slightly changed the sentence above}
...
% To cast this dependence testing problem as a statistical inference query requires specifying a statistical model, that is, a collection of possible distributions from which we may assume the data arise.  To make the investigation as general as possible, we consider the largest possible set of distributions: any possible joint distribution $f_{xy}$. 
If \mbx~and \mby~are independent, then it  follows that $f_{xy}=f_x f_y$; in other words, for independent data, the joint distribution is equal to the product of the marginals $f_x$ and $f_y$.  Therefore, we have the following :
\begin{align*}
...
%We consider two dependence tests, one for each hemisphere.
...
% Specifically, where as global \Mcorr~and \Hhg~have p-values of $0.1132$ and $0.1208$, respectively, \Mgc~obtains a p-value of $0.0113$, and identifies the scales of significance. 
%The left hemisphere seems less dependent on disease status, as both local and global tests yield p-values between $0.05$ and $0.1$ (not shown). 
...
\hline
Activity vs Personality & $\textbf{0.0306}$  & $0.9870$ & $0.6693$ & $0.4475$ & $0.0588$ \\
\hline
Shape vs Disease & $\textbf{0.0113}$  & $0.0818$ & $0.1070$ & $0.1132$ & $0.1208$ \\
\hline
Network vs Creativity & $\textbf{0.0111}$  & $\textbf{0.0093}$ & $\textbf{0.0088}$ & $\textbf{0.0111}$ & $0.341$ \\
\hline
\end{tabular}
\caption{The p-values for real data testing. \Mgc~is the only method that always finds significant relationship and the only method that ever discovers the underlying optimal scale. Bold indicates $<0.05$.}
\label{t:real}%
\end{table*}

\subsubsection*{Brain Network vs. Creativity}
% : Brain Structural Network vs. Personality}
\cs{change interpretation for this one?}
...
Figure \ref{f:real}{\color{magenta}C} shows that even a global dependence test can ascertain whether the whole brain network is independent of the subject's creativity.  However, the global test is quite fragile,  even ignoring a single subject from the global test can render the test non-significant. On the other hand, the \Mgc~multiscale map shows a large region that is quite significant  (Table \ref{t:real}).  
...
This figure includes seven different tests: \Mantel, \Dcorr, and \Mcorr  (magenta dotted, dashed, and solid lines, respectively), as well as their corresponding \Mgc~counterparts, \Mgcp, \Mgcd, \Mgcm~(green with same line styles), as well as sample \Mgc~(cyan) and \Hhg (gray).
Each panel shows the testing power on the abscissa at a significance level $\alpha=0.05$, and sample size on the ordinate.
Oracle \Mgc~empirically achieves similar or better powers than the previous state of the art approaches for all sample sizes on almost all problems, with sample \Mgc~being very close and overall superior to other benchmarks.}
\label{f:1DAll}
\end{figure}
...
In this section, we review the \Mantel~test, distance correlation (\Dcorr), modified distance correlation (\Mcorr),  \Hhg, and \Mgc~along with some additional insights: for each global generalized correlation test (including \Mantel, \Dcorr, and \Mcorr) we have constructed a multiscale variant, called \Mgcp, \Mgcd, and \Mgcm, respectively. And we implement \Dcorr~and \Mcorr~in a slightly different but equivalent way from the original definition, such that the resulting multiscale variants work better. Moreover, our sample \Mgc~is tailored for Oracle \Mgcm~due to the unbiased-ness of \Mcorr. Therefore, unless otherwise specified, \Mgc~always means Oracle or sample \Mgcm~in the paper.
...
Unlike distance correlation and \Hhg, the \Mantel~test is not consistent against all dependent alternatives, but it has been a very popular method in biology and ecology, possibly due to its simplicity and effectiveness. One can observe from Figure~\ref{f:1DAll} and ~\ref{f:nDAll} that global \Mantel~is sub-optimal relative to much more recently proposed tests (\Mantel~was proposed in 1967, \Dcorr~was not proposed until about 40 years later), and appears to be inconsistent for many dependencies. Nonetheless,  \Mgcp~achieves comparable performance to other variants of \Mgc, which implies that \Mgcp~may be consistent with most, if not all dependent alternatives.
...
It is shown in \cite{SzekelyRizzo2013a} that $\Mcorr(X,Y)$ is an unbiased estimator of the population distance correlation $\Dcorr(\mb{x},\mb{y})$ for all $D, D_y, n$; and \Mcorr~is approximately normal even if $D,D_y \rightarrow \infty$. Thus it is a consistent statistic for testing independence, and may work better than \Dcorr~for high-dimensional dependencies.

Similar to the alternative implementation of \Dcorr, we can also use singly centered distance matrices for $A'$ and $B'$ in defining \Mcorr~without altering the theoretical advantages of the original \Mcorr. We further set $A_{ii}=B_{ii}=0$ for all $i$, which simplifies the expression of \Mcorr~and is asymptotically equivalent for testing. 
...
Among all possible local correlations, Oracle \Mgc~picks the optimal local correlation $\G^{*}$ by maximizing the power map. Optimal scales always exist,  are distribution dependent, and are often non-unique. Among all local correlations, it suffices to exclude $\G^{1l}$ and $\G^{k1}$: since $\G^{1l}=\G^{k1}=\G^{11}$, they do not include any neighbor other than each observation itself, merely count the diagonal terms in the distance matrices, and will not be selected as optimal after all.
...
Note that a general estimation technique exists for \Mgcd~and \Mgcp as well: instead of estimating an optimal correlation within the local correlation map, one may estimate the optimal p-value from the p-value map, treat the estimated p-value as a test statistic, and run the permutation test again to compute the p-value for the sample version of any multiscale variant. This technique is immune to the bias of local correlations, and is suggested by Heller et al. in \cite{heller2016consistent}. Since this general technique requires more random permutations and is much slower, and \Mgcm~is theoretically and numerically superior to \Mgcd~and \Mgcp, we do not delve into the general technique in the paper.
...
Algorithm~\ref{alg:power} computes the testing powers of all local statistics using a known or estimated joint distribution, which can be used to accurately calculate the optimal scales and the testing power for Oracle \Mgc. 
Algorithm~\ref{alg:sample_mgc} provides an estimate of the Oracle \Mgc~among all local correlations. When there is a sufficiently large region with significant local correlations, a large local correlation within the region is used as the sample \Mgc; otherwise the global correlation is used instead. %More detailed discussions regarding sample \Mgc~is offered in Appendix~\ref{appen:diss}.
...
\begin{algorithm}
\caption{Compute power for all local tests. This algorithm computes the  power of all local correlations. By repeatedly simulating samples by the joint distribution $f_{xy}$, sample data of size $n$ under the null and the alternative are generated for $r$ Monte-Carlo replicates. Then all sample local correlations under the null and the alternative hypotheses are computed by Algorithm~\ref{alg:all_scales}, followed by estimating the testing power at each local correlation. The \Mgc~optimal scales can be found by selecting all scales that maximizes the powers. The running time is $O(rn^2 \log n)$. In the simulations we use $r=2$,$000$ MC replicates to estimate the optimal scale, and another $r=10$,$000$ MC replicates to estimate the power. This algorithm can be similarly adapted to training data, for which the alternative statistic can be computed from the training data while the null statistic can be computed by permutation. Note that power computation for sample \Mgc~or other benchmarks follows from the same algorithm, by plugging in the respective test statistic in the first loop without the optimal scale computation. }
\label{alg:power}
\begin{algorithmic}[1]
...
\State $\beta_{kl} \rto \sum_{j=1}^{r}(\G^{1}_{kl}[j]>c_{\alpha}) / r$ \Comment{get the power map}
\EndFor
...
\begin{algorithm}
\caption{Compute the sample \Mgc~test statistic. In the local correlation map, we first find the largest connected component that is larger than $\epsilon$, where $\epsilon$ is based on the standard deviation of all negative correlations. When the region is larger than $\gamma$, we iterate through all large correlations within the region, find the median correlation within a box of each large correlations, and take the minimal median value for sample \Mgc. If the region is too small, or the minimal median p-value is less than the global correlation, we use the global correlation for sample \Mgc. The running time is $O(n^2)$.}
\label{alg:sample_mgc}
\begin{algorithmic}[1]
\Require All local statistics $\G^{kl} \in \Real^{n \times n}$.
\Ensure The sample \Mgc~statistic $\hat{\G}^{*} \in \Real$.
\Function{SampleMGC}{$\G$}
\State $\epsilon \rto \G^{kl}(\G^{kl}<0)$ \Comment{all negative local correlations}
\State $\epsilon \rto 6\max\{0.005,\|\epsilon\|_{F}\}$ \Comment{threshold to default to global correlation}
\State $\gamma \rto n$ \Comment{threshold for significant region}

\State $R=\textsc{Connected}(R(R>\epsilon))$ \Comment{a binary matrix indicating largest connected component}
\If{$\sum_{k,l} R_{kl} \leq \gamma$} 
\State $\hat{\G}^{*} \rto \G_{nn}$ \Comment{default to the global correlation}
\Else
\State $\Omega \rto \arg_{k,l}\{(\G^{kl}\geq \textsc{Quantile}(\G^{kl},99)) \bigcap (R_{kl}=1)\}$ \Comment{all scales with large correlations}
\State $\hat{\G}^{*}=\max(\G^{kl}, \forall k,l)$
\State $i=\textsc{Ceiling}(0.01n)$;
\For{$(k',l') \in \Omega$}
\State $\theta = \textsc{Median}(\G^{kl},k \in [k'-i,k'+i] \ \& \ l \in [l'-i,l'+i])$ 
\Lineif{$\theta < \hat{\G}^{*}$}{ $\hat{\G}^{*} \rto \theta$} %\Comment{if empty, use global p-value}
\EndFor
\Lineif{$\hat{\G}^{*} < \G_{nn}$}{ $\hat{\G}^{*} \rto \G_{nn}$}
\EndIf
\EndFunction
\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
\caption{Compute p-values for all local tests and sample \Mgc, and estimate the optimal scale. This algorithm computes the p-values of all local correlations and sample \Mgc~by the permutation test with $r$ random permutations, which takes $O(rn^2 \log n)$. Then the estimated optimal scale equals the largest rectangle with local p-values less than or equals to the p-value of sample \Mgc. $\pi$ denotes a random permutation, and $\tilde{B}(\pi,\pi)$ denotes the distance matrix with the observation label permuted. In the real data experiment we always set $r=10$,$000$. Note that the p-value computation for any other benchmark follows from the same algorithm by replacing sample \Mgc~with the respective test statistic without the optimal scale estimation.}
\label{alg:pval}
\begin{algorithmic}[1]
...
\State $(\hat{k}^{*},\hat{l}^{*}) = \textsc{LargestRectangle}(P_{kl}\leq p)$ \Comment{estimate the optimal scales}
\EndFunction
\end{algorithmic}
\end{algorithm}

%\clearpage
%\subsection{Discussions of Sample \Mgc}
%\label{appen:diss}
%Evaluating \Mgc~requires estimating the optimal scales. Algorithm~\ref{alg:power} is used whenever the true distribution of the data is known or easy to estimate or multiple training data are available, which yields Oracle \Mgc. Otherwise, we shall approximate the optimal scale by Algorithm~\ref{alg:sample_mgc}. Once the optimal scale is determined, the testing power of \Mgc~for the given distribution can be quickly determined by Algorithm~\ref{alg:power}, and its p-value for testing on a particular data realization can be determined by Algorithm~\ref{alg:pval}.

%The main targets of sample \Mgc~are (i) to achieve similar powers as Oracle \Mgc~under dependence, (ii) estimate the optimal scale, and (iii) not inflate the false positive rate under independence. Algorithm~\ref{alg:best_scale} first searches for consecutive regions that have smooth p-values (i.e., mostly decreasing or increasing along either the row or column indices), then either locates a local p-value by histogram within the smooth regions, or directly takes the global p-value when the global p-value is small enough. As the algorithm makes use of the smooth p-values to effectively locate the optimal scales, our sample \Mgc~should yield power close to Oracle \Mgc~for most cases, and optimal scales overlap with those by Oracle \Mgc; and the algorithm does not directly compare local p-values, so it should not inflate the false positive rate.
...
%We further observe that the sample \Mgc~is only significantly inferior to the Oracle \Mgc~when the true optimal scale has a small area (e.g., function $16$ and $17$, see Figures~\ref{f:powermaps} and ~\ref{f:powermaps1}), but otherwise they are very similar. As narrow optimal scales are also likely to occur from a pair of independently generated data by chance, they cannot be trusted without training data or knowing the joint distribution, and are purposely excluded by algorithm~\ref{alg:best_scale}.

%Note that it is tempting to directly use the scale that minimizes all local p-values rather than by Algorithm~\ref{alg:sample_mgc}, or generate random samples based on  data  and use a bootstrap variant of Algorithm~\ref{alg:power}. However, both of those approaches may significantly inflate the false positive rate due to noisy p-values: when the optimal scales are separately estimated for each test, the resulting FPR is often higher than the pre-set $\alpha$, such that the test is no longer valid. This is because p-value is a random variable (it is a function of a random realization from some distribution), so some local p-values will be smaller than the true p-value. Erroneous scales often still exist after a straightforward resampling, so such approaches do not mitigate this problem as much as our proposed strategy. On the other hand, techniques from multiple testing can eliminate FPR inflation, but often yield too low testing powers as there are too many scales in the p-value map.
