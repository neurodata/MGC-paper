...
\jv{the next two paragraphs are great!!!}
...
\jv{I think next paragraph is now repetitive, as we already discussed computational efficiency of MGC in last section}
...
\begin{figure}
  ...
  \end{tabular}
  \caption{Average powers comparison between each global correlation and \Mgc. For each global correlation, each dot on the left side corresponds to the mean power of each simulation in Figure~\ref{f:nD}, while the corresponding dot on the right side corresponds to the respective \Mgc~mean power. The dotted line are shown for type 6-19, because \Mgc~equals the global correlation for type 1-5 and type 20. Then the thick solid line summarizes how the overall mean power (including type 1-19) changes from global to \Mgc.
It is clear that \Mgc always has a large advantage margin regardless of the actual global correlation to use.}
\label{f:pp}
\end{figure}
...
Furthermore, future investigations on the optimal scale for \Mgc~is also of interest, such as how to more accurately select the local scale under unknown models for a particular inference task, and the implication of the optimal scale on the geometry of underlying dependency, etc. Another direction is to choose a proper metric for given data, which may improve both the optimal scale estimation and the resulting power. 
