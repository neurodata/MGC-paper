...
\label{numer1}
In this subsection and the next, we show the numerical advantage of multiscale graph correlation (using mcorr) via the empirical testing powers. The detailed evaluation procedure is provided in appendix Section~\ref{appen:tests}, and the benchmarks are dcorr, mcorr, Mantel, and HHG.
...
In total we consider $20$ different distributions of $f_{xy}$ taken from existing literature \cite{SzekelyRizzoBakirov2007, SimonTibshirani2012, GorfineHellerHeller2012, HellerGorfine2013}: they consist of various linear and close to linear dependencies (type 1-5), polynomial-based nonlinear relationships (type 6-12), trigonometry-based nonlinear dependencies (type 13-17), two uncorrelated dependencies (type 18-19), and an independence relationship (type 20) to show that MGC does not detect signal in the absence of dependency. The exact simulation distributions are given in appendix Section~\ref{appen:function}, with a visualization of each dependency follows in Supplementary Figure~\ref{fig0}.

We first experiment on a dimension $1$ scenario at $d_{x}=d_{y}=1$. To observe how fast the testing power of each method converges to $1$ for different dependencies, the testing powers are calculated against increasing $n$ from $5$ to $100$. The empirical powers based on $r=10$,$000$ Monte-Carlo replicates are plotted in Figure~\ref{fig:1D} at type $1$ error level $\alpha=0.05$, and we use $2$,$000$ MC replicates to estimate the optimal scale of MGC. %Note that appropriate level of white noise are added to $Y$ depending on the distribution type, otherwise the power can quickly converge to $1$ at very small sample size for certain dependencies like linear.
...
We can clearly see from Figure~\ref{fig:pp}(a)(b) that MGC is indeed the most reliable method in the dimension $1$ scenario, comparing to all the global statistics. HHG is slightly better than dcorr/mcorr in the performance profiles, because there are more nonlinear simulations than linear in the $20$ dependencies, and HHG has a larger advantage for nonlinear dependency than its disadvantage in linear dependency; and the Mantel test has the lowest performance profile. 
...
Like the dimension $1$ scenario, MGC also surpasses all the benchmarks in the increasing dimension scenario: for close to linear dependencies from type 1-5, MGC is as good as mcorr, whose power deteriorates much slower than the others; and for the remaining nonlinear dependencies (especially type 6-11 and type 18-19), MGC almost always has a significant advantage comparing to others, due to its capability to better handle nonlinearity and high-dimensionality at the same time. The performance profiles of MGC in Figure~\ref{fig:pp}(c)(d) clearly reflect its superiority.
...
\label{numer3}
Here we apply MGC to test independence between brain features and personal characteristics from two different experiments, for which the data sets are relatively small in sample size due to the expensive data collection process. Based on the evaluation procedure in Section~\ref{appen:tests}, we report the p-value of each method from the permutation test, with the optimal scale of MGC estimated by smooth bootstrap.
...
The last experiment uses MGC to test independence between brain voxel activities and non-existent stimulus, similar to the experiment in \cite{EklundKnutsson2012}; and we take the BNU1 data for illustration. Two fMRI scans are done for a total of $50$ persons, including $200$ regions of interest for $200$ timesteps; then we generate an independent stimulus by a standard normal distribution for each person, and the stimuls is the same for all voxels of a given subject during the two scans.

For each region, we can form two $50 \times 50$ distance matrices by the Euclidean distance; and we can form another distance matrix for the stimulus, and test independence between each region and the stimulus for significance. By using $10000$ permutations to yield the p-value, each voxel is declared significant when the p-value is less than $\alpha=0.05$. 

Since the stimulus is independent, the testing power, i.e., the percentage of brain regions declared significant, should be close to $\alpha$. Indeed, all methods including MGC return power no larger than $0.06$ for testing dependence between either scan and the stimulus.
...
\begin{align*}
\mb{y}&=((2\mb{u}-1) \cdot |1-\bigg(2\frac{w\mb{x}}{\max(w\mb{x})}-1\bigg)^2|^{\frac{1}{2}}+1)/2+0.1c\epsilon.
\end{align*}
\item Ellipse: Let $\mb{v} \sim \mc{U}(-1,1)^{d_{x}}$, $\mb{u} \sim \mc{B}(0.5)$. Then
\begin{align*}
\mb{x}&=5\sin(\pi \mb{v}),\\
\mb{y}&=(2\mb{u}-1) \cdot |1-\bigg(\frac{w\mb{x}}{\max(w\mb{x})}\bigg)^2|^{\frac{1}{2}}+0.3c\epsilon.
\end{align*}

\item Spiral: Let $\mb{u} \sim \mc{U}(0,20)$, $\mb{v} \sim \mc{U}(0,20)^{d_{x}-1}$. Then
\begin{align*}
\mb{x}_{1}&=\mb{u}\sin(\mb{u}),\\
\mb{x}_{d}&=w_{d}\mb{v}_{d-1} \mbox{ for $d=2,\ldots,d_{x}$},\\
\mb{y}&=\mb{u}\cos(\mb{u})+0.3c\epsilon.
\end{align*}
\item Square: Let $\mb{u} \sim \mc{U}(-1,1)$, $\mb{u}' \sim \mc{N}(0,1)^{d_{x}}$, $\mb{v} \sim \mc{U}(-1,1)$, $\mb{v}' \sim \mc{N}(0,1)^{d_{x}}$, $\theta=-\frac{\pi}{8}$. Then
\begin{align*}
\mb{x}_{d}&=(\mb{u}+0.05\mb{u}'_{d}) \cos(\theta) + (\mb{v}+0.05\mb{v}'_{d}) \sin(\theta),\\
\mb{y}_{d}&=-(\mb{u}+0.05\mb{u}'_{d}) \sin(\theta) + (\mb{v}+0.05\mb{v}'_{d}) \cos(\theta),
\end{align*}
for $d=1,\ldots,d_{x}$.
\item Diamond: Let $\mb{u} \sim \mc{U}(-1,1)$, $\mb{u}' \sim \mc{N}(0,1)^{d_{x}}$, $\mb{v} \sim \mc{U}(-1,1)$, $\mb{v}' \sim \mc{N}(0,1)^{d_{x}}$, $\theta=-\frac{\pi}{4}$. Then
\begin{align*}
\mb{x}_{d}&=(\mb{u}+0.05\mb{u}'_{d}) \cos(\theta) + (\mb{v}+0.05\mb{v}'_{d}) \sin(\theta),\\
\mb{y}_{d}&=-(\mb{u}+0.05\mb{u}'_{d}) \sin(\theta) + (\mb{v}+0.05\mb{v}'_{d}) \cos(\theta),
\end{align*}
for $d=1,\ldots,d_{x}$.
\item Sine Period 1/2: $\mb{u} \sim \mc{U}(-1,1)$, $\mb{v} \sim \mc{N}(0,1)^{d_{x}}$,
\begin{align*}
\mb{x}_{d}&=\mb{u}+0.02 d_{x} \mb{v}_{d} \mbox{ for $d=1,\ldots,d_{x}$}, \\
\mb{y}&=\sin ( 4\pi \mb{x} )+c\epsilon.
\end{align*}
\item Sine Period 1/8: $\mb{u} \sim \mc{U}(-1,1)$, $\mb{v} \sim \mc{N}(0,1)^{d_{x}}$,
\begin{align*}
\mb{x}_{d}&=\mb{u}+0.01 d_{x} \mb{v}_{d} \mbox{ for $d=1,\ldots,d_{x}$}, \\
\mb{y}&=\sin ( 16\pi \mb{x} )+0.5c\epsilon.
\end{align*}
...
\caption{Same Figures as Figure~\ref{fig:pp} but including MGC$\{$dcorr$\}$ and MGC$\{$Mantel$\}$}
\label{fig:pp2}
\end{figure}
...
In subsection~\ref{appen:algorithms} we present three essential algorithms for testing: givens the global correlation coefficient, the first algorithm computes all local correlations between two sample data sets; the second algorithm computes the testing powers of the local correlations, based on repeated simulating samples from known distributions or bootstrap samples from given observations; and the third algorithm computes the p-value of the local statistics by a random permutation test. 
...
The second algorithm computes the testing powers of all local statistics, by either repeated simulating samples generated by the joint distribution $f_{xy}$, or smooth bootstrap samples generated by given observations $X$ and $Y$. This algorithm is pretty standard, but is required for MGC to estimate the optimal scale among all neighborhoods by maximizing the powers among all local statistics. Note that smooth bootstrap is used rather than usual bootstrap, and the reason is given in the next subsection.

The third algorithm computes the p-values of all local statistics by the permutation test. This algorithm is also straightforward to implement, and is used to output the p-values of all local tests for given real data.
...
\For{$m=1,\ldots,r$} \Comment{generate dependent $(X_{1},Y_{1})$ and an independent $Y_{2}$}
\If{$f_{xy}$ is given}
\State $(X_{1},Y_{1})=generateData(f_{xy})$; $(X_{2},Y_{2})=generateData(f_{xy})$; 
\Else
\State $per=randsampling(n)$; \Comment{generate a random resampling with replacement of size $n$}
\State $X_{1}=X(:,per)$;$Y_{1}=Y(:,per)$; 
\State $Z=generateData('normal')$;\Comment{generate a matrix of same size as $X$ by independent normal distribution for each entry}
\State $X_{1}=X_{1}+Z*norm(X,'frob')/norm(Z,'frob')/\sqrt{n}$;\Comment{generate a smooth bootstrap sample}
\State $per=randsampling(n)$; $Y_{2}=Y(:,per)$; \Comment{generate an independent resampling for $Y_{2}$}
\EndIf
...
Once the optimal scale is estimated, the p-value of MGC can be calculated by running Algorithm~\ref{algPerm} at the optimal scale $(k^{*},l^{*})$. The testing power of MGC can be quickly determined by running Algorithm~\ref{algPower} again to estimate the power at the optimal scale $(k^{*},l^{*})$. We can apply the same procedures to evaluate the p-values and powers for global correlations like dcorr/mcorr/Mantel/HHG, by excluding the first step for estimating the optimal scale.

Note that when we estimate the optimal scale by bootstrap, we used smooth bootstrap rather than usual bootstrap. This is because if the optimal scale is estimated by usual bootstrap samples of the given observations $(X,Y)$, then the MGC p-value may be slightly biased when the permutation test is carried out on the same observations $(X,Y)$, such that the MGC testing power can be higher than the type 1 error level even under independence.

But the bias is mostly eliminated by using smooth bootstrap samples for sufficient number of times: given $(X,Y)$, suppose $(X',Y')$ is the usual bootstrap sample (a direct resampling of the given data), then we let $(X'+Z,Y')$ be the smooth bootstrap sample, where $Z$ is independently generated by a normal distribution and scaled such that $\|Z\|_{F}=\frac{\|X\|_{F}}{\sqrt{n}}$. Alternatively, if the distance matrix pair $(C,D)$ is given instead, we represent $(C',D')$ as the usual bootstrap distance matrix pair, and let $(C'+E, D')$ be the smooth bootstrap distance pair, where $E$ is the distance matrix of $Z$ and scaled such that $\|E\|_{F}=\frac{\|C\|_{F}}{\sqrt{n}}$. 

From the real data experiment on testing non-existent stimulus, the resulting MGC testing power is indeed very close to the type $1$ error level $0.05$; but if we were to use usual bootstrap to estimate the optimal scale, the power will be slightly inflated to around $0.09$. Other than smooth bootstrap, there are two other ways to eliminate the bias in certain special cases:

If the given observations $(X,Y)$ can be modeled by a joint distribution $f_{xy}$, then we can estimate the optimal scale by repeated simulating samples rather than bootstrap. Then the estimating step is independent of the permutation test step and no bias is introduced. But modeling the given data can be difficult in general.

Alternatively, if we are given multiple observations of the same source, say $(X,Y)$ and $(X',Y')$, that we can reasonably assume to have the same or similar joint distribution but not identical, then we can output the MGC p-value of $(X,Y)$ by estimating the optimal scale from the bootstrap samples of $(X',Y')$. This in fact can be done for our real data experiments: for testing left brain shape versus personality, if we were to estimate the optimal scale by the bootstrap samples from the right brain and personality, the resulting MGC p-value is still the most significant; for the voxel versus non-existent signal, if we were to estimate the optimal scale by the first fMRI scan, then output the resulting MGC p-value by the permutation test on the second fMRI scan, the resulting MGC power is still very close to the type $1$ error level.
