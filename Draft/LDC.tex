...
\label{alg1}
\begin{algorithmic}
\Function{LGC}{$\tilde{A}$,$\tilde{B}$,$k$,$l$}
...
\State $R^{Z}=sort(\tilde{Z})$ for $Z=\{A,B\}$ \Comment{assuming column-wise sorting and no ties exists}
\State $Z=center(\tilde{Z})$ for $Z=\{A,B\}$ \Comment{proper centering of the distance matrices}

\For{$i,j=1,\ldots,n$}
\State $\G_{kl}=\G_{kl}+A_{ij}B_{ij}\mb{I}(R^{A}_{ij} \leq k)\mb{I}(R^{B}_{ij} \leq l)$ \Comment{store local distance covariance}
\State $V^{A}_{k}=V^{A}_{k}+A_{ij}^2\mb{I}(R^{A}_{ij} \leq k)$ \Comment{store local distance variance for the first data}
\State $V^{B}_{l}=V^{B}_{l}+B_{ij}^2\mb{I}(R^{B}_{ij} \leq l)$ \Comment{store local distance variance for the second data}
...
\State $\G_{k,l}=\left(\G_{k,l}-E^{A}_{k}E^{B}_{l}/n^2\right)/\sqrt{\left(V^{A}_{k}-{E^{A}_{k}}^2/n^2\right) \left(V^{B}_{l}-{E^{B}_{l}}^2/n^2\right)}$ \Comment{normalize the local covariances yields the local correlations.}
...
\For{$j=1,\ldots,r$} 
\State $(X^{1}_{i},Y^{1}_{i}) \stackrel{iid}{\sim} f_{xy}, i \in [n]$ \Comment{generate dependent samples}
\State $X^{0}_{i} \stackrel{iid}{\sim} f_{x}$ for $i \in [n]$ \Comment{generate independent samples}
\State $Y^{0}_{i} \stackrel{iid}{\sim} f_{y}$ for $i \in [n]$
\State $\tilde{Z}^{1}=dist(Z^{1})$ for $Z=\{A,B\}$ \Comment{the distance matrices under the alternative}
\State $\tilde{Z}^{0}=dist(Z^{0})$ for $Z=\{A,B\}$ \Comment{the distance matrices under the null}
\State $\G_{kl}^{1}[j]=LGC(\tilde{A}^{1},\tilde{B}^{1})$ \Comment{calculate all local correlations under the alternative} 
\State $\G_{kl}^{0}[j]=LGC(\tilde{A}^{0},\tilde{B}^{0})$ \Comment{calculate all local correlations under the null} 
\EndFor

\For{$k,l=1,\ldots,n$}
\State $c_{\alpha}=CDF_{1-\alpha}(\G^{0}_{kl}[j],j \in [r])$ \Comment{get the critical value by the empirical cumulative distribution under the null at each scale}
...
\State $\tilde{A}=dist(X)$
\State $\tilde{B}=dist(Y)$
\State $\G_{kl}=LGC(\tilde{A},\tilde{B})$ \Comment{calculate the observed local correlations}
\For{$j=1,\ldots,r$}
\State $\pi=randperm(n)$ \Comment{generate a random permutation of size $n$}
\State $\G_{kl}^{0}[j]=LGC(\tilde{A},\tilde{B}(\pi,\pi))$ \Comment{calculate the permuted test statistics}
\EndFor

\For{$k,l=1,\ldots,n$}
\State $p_{kl}=\sum_{j=1}^{r}(\G_{kl}<\G^{1}{k,l}[j])/r$ \Comment{get the p-value at each local scale}
\EndFor
\EndFunction
\end{algorithmic}
\end{algorithm}
...
\State $\alpha=0.05$ \Comment{the type $1$ error level}
\State $\delta=0.8$ \Comment{the threshold to determine whether a row/column is sufficiently powerful}
\State $\gamma=0.1$ \Comment{the threshold to determine whether there are consecutive rows/columns that are powerful}
