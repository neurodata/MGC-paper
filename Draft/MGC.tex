...
\title{\bf Revealing the Nature of Nonlinear Relationships}
...
% \bigskip
\begin{abstract}
One of the most basic kinds of questions we ask is of the form: is this kind of property (e.g., cloudy skies) related to another kind (e.g., wet ground).  If we can determine that those two kinds of properties are not independent of one another, only then does it make sense to engage in causal or predicative investigations.  However, detecting whether or not said properties are related to one another is challenging for a number of reasons. First, the nature of the relationship between these two kinds of properties can be highly nonlinear.  Second, sometimes the characterization of one or the other property is  complicated (for example, cloud shape, size, rates of change, etc.).  Third, unfortunately, data collection can be extremely expensive, so the number of samples might be quite low.  A statistical procedure, therefore, that can detect dependence in spite of these challenges is highly desirable.  
% Discovering and understanding dependence between multiple  measurement types is fundamental not just in the natural sciences, but also policy, commerce, and other domains.
% Given a set of paired observations, an ideal dependence test would efficiently detect dependence in both simulated and real, ``dirty'' data, regardless of the nature of the dependence structure. This would include nonlinear,  high-dimensional, low-sample size, and structured observations, such as images and their corresponding captions.  
% Moreover, rather than merely providing a theoretically consistent test under all those conditions, and state-of-the-art performance empirically, it would also provide insight into the nature of the dependence between the different modalities.  
...
Our key insight is that if the two different modalities are dependent, then \emph{local} distances within one modality (images) must correlate with \emph{local} distances within the other modality (captions). 
% \bm{daddy points out that the above sentence implies the below, and we don't have room for both}
% Thus, to optimally detect signal in nonlinear functions, the method must discard  uninformative global distances.
% \jv{global distances need not correlate is not quite perfect. we need a way to ignore the global ones; still not quite right}
We demonstrate that \Mgc~has all the above properties via simulation and theory.
We then apply \Mgc~in several real applications to detect the presence and nature  of dependence between: (i)  brain activity and personality, (ii) brain shape and disorder, and (iii) brain connectivity and creativity. 
We also demonstrate that \Mgc~does suffer from uncontrolled false positives that have plagued other studies.
% not inflate non-existent dependence between brain activity and spurious stimulation.  
\Mgc~can therefore be applied in a wide variety of applications.
% , requiring only data and a dissimilarity function for each measurement type.  
Both MATLAB and R code are provided here: \website.
\end{abstract}
...
%
Fourth, because we will often have a data deluge, with myriad of different measurements, it is important to have a  \textbf{computationally efficient} method of generating results.
%
Finally, 
% as alluded to above, checking for dependence is rarely the final step in the analysis.  Frequently, 
we not only want to know \textit{whether}  modalities are dependent on each other, but also \textit{how} they are (i.e. their \textbf{scales and nature of dependence}).
A method is needed that addresses all of these desiderata in theory, with extensive simulations, and in real data problems.
...
But there are also pairs of points that are close only in $y$ and not $x$  (points $2$ and $3$) and vice-versa.  
Thus, locality is a joint property of both $x$ and $y$.
Test statistics for dependence typically operate on \emph{all} pairwise relationships, making them well-suited for detecting global relationships. However, nonlinear relationships evident on finer scales can be missed in the global din. \Mgc~solves this problem by identifying the jointly local pairs that contain the most information about the dependence structure, and then using only those scales to conduct its test, thereby revealing both the existence and scales of dependence.
...
% The first step is to choose distance functions for both modalities and compute the distance between all pairs of observations within each modality. The choice of distance determines the metric upon which dependence will rest, and is the only choice in the algorithm.  
% The second step is to ``single-center'' these matrices to ensure that each data point counts equally (without such centering, single data points far from the others could skew the distances).
% The third step is to compute a local variant of the test, to constrain the test statistic to only consider a particular scale.
% The fourth step is to efficiently compute the local test statistics for all possible scales via a nesting algorithm. 
%which will enable \Mgc~to flexibly determine the scales of interest.
% The fifth and final step is to select the optimal scales, and compute the p-value corresponding to those scales, providing both an assessment of whether the two modalities are consistent, and if so, at which scales. 

% \jv{the only "choice" we have in the algorithm is the choice of what metric to use.}
% \jv{maybe summarize the 5 steps before getting into the nitty-gritty?}
% \jv{make first sentence conceptually clear for each of the five steps}
% 1. 
The first step is to choose distance functions for both modalities --- this serves to provide a nonlinearity, much like kernel machines \cite{scholkopf2002learning}, which will enable us to detect dependence structures for arbitrary dependence functions.  
 The choice of distance is the only choice in the algorithm.
Given this choice, compute the distance between all pairs of observations within each modality.  More formally, let $\delta_x$ be the distance for $\mbx$ and $\delta_y$ for $\mb{y}$, and compute the $n \times n$ interpoint comparison matrices $A=\{a_{ij}\}$ and $B=\{b_{ij}\}$, where $a_{ij}=\delta_x(\mb{x}_i,\mb{x}_j)$ and  $b_{ij}=\delta_y(\mb{y}_i,\mb{y}_j)$.  
...
This ``generalized correlation coefficient'' (because it generalizes Pearson's correlation coefficient) \cite{KendallBook}, assuming $A$ and $B$ have zero mean without loss of generality, is written:
...
\Mgc~can be thought of as a sparse or regularized variant of a global correlation test, and therefore faces the same dilemma as all regularized algorithms (including sparse methods, feature selection, and dimension reduction): how to efficiently choose the parameters. By choosing the locality scales in a principled fashion, \Mgc~both yields a consistent test and reveals the scales of dependence.  
...
The fourth step is therefore to compute all local tests in a nested algorithm, re-using the more local scale statistics to build the increasingly global statistics, efficiently yielding multiscale maps that reveal the scales of dependence. 
...
The fifth step is to estimate the optimal scales and report the corresponding p-value in a statistically consistent fashion. The optimal scales for any given distribution and finite sample size are defined as the scales that maximize power (the probability of correctly rejecting a false null hypothesis).  In simulations, or datasets for which we have extensive training data, we can estimate the optimal scales; ``Oracle \Mgc'' chooses the optimal scale in this fashion  (Algorithm \ref{alg:power}).
In real data settings, the power of a test cannot be known because the true distribution is unavailable, so  ``sample \Mgc'' (or just \Mgc) uses the p-values as a surrogate. 
...
\cs{
The fifth step is to estimate the optimal scales and report the \Mgc~p-value by a valid statistical procedure. The optimal scales for any given distribution and finite sample size are defined as the scales that maximize power (the probability of correctly rejecting a false null hypothesis).  In simulations, or datasets for which we have extensive training data, we can estimate the optimal scales directly by the testing power map; ``Oracle \Mgc'' chooses the optimal scale in this fashion  (Algorithm \ref{alg:power}), and the \Mgc~p-value follows by a permutation test at the optimal local test. In real data settings, the power of a test cannot be known because the true distribution is unavailable, so  ``sample \Mgc'' (or just \Mgc) uses the p-values as a surrogate, i.e., we estimate the \Mgc~p-value first by a valid procedure (Algorithm \ref{alg:best_scale}, which does not suffer from multiple hypothesis testing problems \cite{Benjamini1995}), and take the largest rectangle within the p-value map that is bounded above by the \Mgc~p-value as the optimal scales.
}
...
Whereas \Mcorr, the global test, has very low power (magenta dot in multiscale p-value map) and therefore yields a non-significant p-value ($0.827$),  there are many local scales that achieve nearly perfect power, so \Mgc~obtains both a highly significant p-value ($\approx 0.005$) and reveals the scales of dependency. 
...
It is the responsibility of the developers of any novel method to place it in the context of the previous art.  More specifically, developers ought to provide guidance to practitioners as to when the new approach is expected to outperform other approaches, and equally importantly, when it is not. 
...
Figure~\ref{f:nD} shows  the power of each test for all $20$ settings with sample size fixed at $n=100$, but dimensionality of \mbx~increasing (the dimensionality of \mby~increases in only a subset of the settings; see Methods for details).  
%
...
% 
Note that for all simulations, there always exists a large smooth region of local scales that are nearly equally significant in the power map, which is an important observation that we use to approximate the test statistic and p-value for sample \Mgc.
\cs{changed above}
...
% As defined above, 
A test is consistent if its power converges to one as sample size increases when $f_{xy} \neq f_x f_y$, and has power equal to the type $1$ error level when $f_{xy}=f_x f_y$.
Let $\G_t$ denote a global generalized correlation coefficient based test statistic, that is, $t$ might indicate \Mantel, \Dcorr, or \Mcorr, and let $\beta_n(c_t^*)$ denote the power of the corresponding oracle multiscale version for $n$ samples; and we drop the subscript when considering asymptotic power.
...
\begin{thm}
\label{thm1}
$\beta_n(c_t^*) \rightarrow 1$ as $n \to \infty$ for all $f_{xy}$ in $\mc{F}_t$.
...
\begin{equation}
\beta_n(c_t^*) \geq \beta_n(\G^{k,l}_{t}) > \beta_n(\G_{t}).
\end{equation}
...
\cs{this holds for oracle mgc with training data or known model, but not for our sample mgc. shall we also state it here?}

\subsection*{Real Data Experiments}
\label{numer3}

\cs{we should say we use sample \Mgc~by \mcorr here, due to its an unbiased statistic that offers advantage in hd simulations and sample mgc computation. And we can put last experiment into appendix if necessary, since our procedure is now theoretically valid and shouldn't inflate FPR anyway.}
...
Note that we generally recommend using \Mgc~by \Mcorr, because \Mcorr~is consistent and unbiased, while \Mantel~is not always consistent and \Dcorr~is biased. In particular, the unbiased-ness of \Mcorr is important for Algorithm~\ref{alg:p_val} to function properly, because larger local statistic for local \Mcorr generally implies more significant dependency, while it is not always true for \Dcorr. 
...
\begin{algorithm}
\caption{Compute p-value for all local tests. This algorithm computes the p-values of all local correlations by the permutation test with $r$ random permutations, which takes $O(rn^2 \log n)$. $\pi$ denotes a random permutation, and $\tilde{B}(\pi,\pi)$ denotes the distance matrix with the observation label permuted. In the real data experiment we always set $r=10$,$000$.}
\label{alg:pval}
\begin{algorithmic}[1]
...
\begin{algorithm}
\caption{Compute p-value and optimal scales for sample \Mgc. First it locates the smooth regions of the p-value map: we compute the difference of adjacent local p-values within each row, label each difference a smooth change if it is bounded above by a small positive number (i.e., the local p-values have a decreasing trend within the rows), and compute the largest rectangular region of smooth changes. Do it for p-value changes with increasing / decreasing trend within each row / column, and combine the four smooth rectangles into the smooth regions. If the global p-value is among the top p-values, i.e., within $\gamma \%$ of all local p-values, we directly use the global p-value for sample \Mgc; otherwise we partition the p-values in the smooth regions into at most $100$ bins, find the bin with most p-values, and use the largest p-value of that bin for sample \Mgc. Then the largest rectangle that is bounded above by the \Mgc~p-value within the smooth regions is taken as the optimal scale. The running time is $O(n^2)$.}
...
\State $\gamma \rto 0.02$ \Comment{threshold to check significant global p-value and significant region}
\State $R=\textsc{SmoothRegions}(P)$ \Comment{find the smooth regions within the p-value map}

\If{$\sum_{k,l} (P_{kl}<P[n,n]) \leq \gamma n^2$} \Comment{use the global p-value if it is sufficiently small}
\State $p \rto P[n,n]$ 
\Else
\State $R = \textsc{LargestRectangle}(R)$ \Comment{find the largest smooth rectangle}
\If{$\sum_{k,l} R_{kl} \geq \gamma n^2$} 
\State $[H,E]=\textsc{Hist}(P(R),\min\{\sum{R}/10,100\})$ \Comment{partition smooth p-values into bins}
\State $i \rto \arg\max_{i}\{H_i\}$ \Comment{find the bin with most counts}
\State $p \rto E_{i+1}$ \Comment{the largest p-value of that bin is used for \Mgc~p-value}
\Else
\State $p = \textsc{Mean}(P)$ \Comment{use the mean p-value if the rectangle is too small}
\EndIf
\EndIf
\State $R = \textsc{LargestRectangle}(R \cap (P\leq p))$ \Comment{calculate the largest rectangle covered by $p$}
\Lineif{$\sum_{k,l} R_{kl}==0$}{ $p \rto P[n,n]$} \Comment{if empty, use global p-value}
\State $[k^{*},l^{*}] \rto \textsc{Find}(R==1)$ \Comment{get the optimal scales}
\Lineif{$p \geq P[n,n]$}{$[k^{*},l^{*}]=\textsc{Add}([k^{*},l^{*}], [n,n])$} \Comment{add the global scale accordingly}
\EndFunction
\end{algorithmic}
\
\
\begin{algorithmic}[1]
\Require The p-value matrix $P \in \Real^{n \times n}$, the number of permutations $r$.
\Ensure A binary matrix $R \in \{0,1\}^{n \times n}$ indicating the smooth regions in $P$.
\Function{SmoothRegions}{$P,r$}
\State $\epsilon \rto \min\{2/r,0.005\}$ \Comment{threshold to check smoothness}
...
\State $R^{1} = \textsc{LargestRectangle}(P \leq \epsilon)$ \Comment{largest smooth rectangle with decreasing trend} 
\State $R^{2} = \textsc{LargestRectangle}(P \geq -\epsilon)$ \Comment{largest smooth rectangle with increasing trend} 
\State $R^{3} = \textsc{LargestRectangle}(P^{'} \leq \epsilon)$ \Comment{repeat for columns changes}
\State $R^{4} = \textsc{LargestRectangle}(P^{'} \geq -\epsilon)$
...
%The parameter $\gamma$ is an important threshold to identify the significant region, which can be increased or decreased to make the algorithm more conservative or aggressive in searching the optimal scale. Decreasing or increasing the parameter $\tau$ has a similar effect, though to a lesser extent, and a small positive number generally suffices to negate noisy p-value changes. Empirically, our choice of parameters achieves good testing powers under dependence with minimal FDR inflation.
