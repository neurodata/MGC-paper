...
\State $PR = \textsc{LargestComponent}(P<\gamma)$ \Comment{find the largest connect component}
\State $PC=\textsc{ValidateRow}(PR')$
\State $PR=\textsc{ValidateRow}(PR)$
\Lineif{$\sum_{ij}PC_{ij} > \sum_{ij}PR_{ij} $}{$PR \rto PC$} \Comment{break if it is checked too many times}
\EndFor
...
\Ensure The approximated \Mgc~p-value $p$.
\Function{Validate}{$P,\alpha$}
\State Initialize $\rho$ as a vector of zeros of size $n$
\State $p \rto P_{nn}$ \Comment{take the global p-value by default}
\State $n_{2} \rto \left\lceil 0.025 \times n \right\rceil$ \Comment{number of adjacent columns to check}
\State $\Gamma \rto \{10,20,\ldots,50\}$ \Comment{specify the percentile of p-values for significance check}
\For{$j:=1:\textsc{Length}(\Gamma)$}
\State $\gamma=\textsc{Prctile}(P,\Gamma[j])$ \Comment{check the p-values in order}
\Lineif{$\gamma \times j>P_{nn}$}{break} \Comment{break if it is checked too many times}
\For{$i:=2,\ldots,n$}
\State $i_{s} \rto \max\{i-n_{2},2\}$
\State $i_{e} \rto \min\{i+n_{2},n\}$
\State $\psi \rto \textsc{Find}\{P[i,:] > \gamma, \mbox{ for any } i \in \{i_{s},\ldots,i_{e}\}\}$ \Comment{find all columns indices larger than $\gamma$}
\State $\rho[i] \rto (\textsc{Max}(\textsc{Diff}(\psi))-1) / (n-1)$ \Comment{find the largest consecutive region within $\gamma$}
\EndFor
\State $\epsilon \rto \min\{\textsc{Mean}(\rho),\rho[n]\}+3\textsc{Std}(\rho)$ \Comment{threshold to check consecutive region size}
\Lineif{$\max\{\rho\}>\max\{0.3,\epsilon\}$}{$p \rto \gamma$} %\Comment{check the region size}
\EndFor
\EndFunction
\end{algorithmic}
\end{algorithm}
