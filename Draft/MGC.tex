...
Note that for almost all simulations, there exist a large portion of adjacent local neighborhoods that are equally significant, and the power changes are always smooth in adjacent scales, which is an important observation that we use to approximate the optimal \Mgc~scales for real data by the p-value map.
