function [pdsC1, pdsC2, pdsC3]=TibsSimuDim2(type,n,dim)
%Tibs Simu w.r.t. dimension
%For y=f(x*A)
%TibsSimuDim2(1,100, 1000);
rep=1000;noise=0;
dimRange=0:10:dim;
dimRange(1)=1;
lim=length(dimRange);
...
for r=1:rep
    ...
    switch type
        case 1
            y=xA+noise*mvnrnd(zeros(n, dim1),eye(dim1)); %linear
        case 2
            y=4*(xA-0.5).^2+noise*mvnrnd(zeros(n, dim1),eye(dim1)); %Quadratic
        case 3
            y=128*(xA-1/3).^3+48*(xA-1/3).^2-12*(xA-1/3)+10*noise*mvnrnd(zeros(n, dim1),eye(dim1)); %Cubic
        case 4
            y=sin(4*pi*xA)+2*noise*mvnrnd(zeros(n, dim1),eye(dim1)); %sine 1/2
        case 5
            y=sin(16*pi*xA)+noise*mvnrnd(zeros(n, dim1),eye(dim1)); %sine 1/8
        case 6
            y=(xA).^0.25+noise*mvnrnd(zeros(n, dim1),eye(dim1)); %x^0.25
        case 7
            y=(binornd(1,0.5,n,dim1)*2-1).*(1-(xA*2-1).^2).^0.5+noise/4*mvnrnd(zeros(n, dim1),eye(dim1)); %circle
        case 8
            y=(xA>0.5)+noise*5*mvnrnd(zeros(n, dim1),eye(dim1)); %step
        case 9
            y=exp(xA)+noise*mvnrnd(zeros(n, dim1),eye(dim1)); %exponential
        case 10
            y=log(xA+1)+noise*mvnrnd(zeros(n, dim1),eye(dim1));%log
    end
    x=unifrnd(0,1,n,dim);
