...
\State $[p_{r}]=\textsc{Validate}(P)$ \Comment{find consecutive scales of $P$ by row}
\State $[p_{c}]=\textsc{Validate}(P\T)$ \Comment{find consecutive scales of $P$ by column}
...
\For{$i:=2,\ldots,n$}
\State $i_{s} \rto \min\{i-n_{2},2\}$
\State $i_{e} \rto \max\{i+n_{2},n\}$
\State $\psi \rto \textsc{Find}\{P[i_{s}:i_{e},:] > \gamma\}$ \Comment{find all columns indices larger than $\gamma$}
