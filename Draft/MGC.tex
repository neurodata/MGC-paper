...
Our key insight is that if two properties are related, comparisons between measurements of similar pairs of the first property (e.g., two clouds of similar shape) should be correlated with the comparisons between corresponding measurements of the second property (grass wetness under those clouds). 
We prove that \Mgc~is always as good as (in linear settings) and often better than (in nonlinear settings) the previous state of the art. 
Moreover, we demonstrate that \Mgc~is the only dependence test that also provides insight into the nature of dependence.
We apply \Mgc~to detect the presence and nature of the relationships between brain activity and personality, brain shape and disorder, and  brain connectivity and creativity.  Finally, we demonstrate that \Mgc~does not suffer from the false positives that have plagued parametric methods.  Our open source implementation of \Mgc~is easy to use and applicable to fundamental questions confronting science, government, finance, and many other disciplines. 
\end{abstract}
...
There is thus a  need for methods that satisfactorily address all of these challenges as demonstrated by compelling evidence for its theoretical efficacy and practical utility.



  
Imagine that the $x$ dimension quantifies cloud shape, and the $y$ dimension quantifies ground wetness, and that we observe  samples of (cloud shape, ground wetness) across $n$ different days.  
If cloud shape and ground wetness were linearly related to one another (Figure \ref{f:newschem}{\color{magenta}A}), then a pair of samples that were similar in terms of cloud shape would also be similar in ground wetness. 
Indeed Mantel noticed that when developing the \Mantel~test in 1967 \cite{Mantel1967}.
This test is essentially a test of the linear correlations across pairwise comparisons, rather than the original observations.
Indeed, operating on pairs of observations rather than individual observations has a rich history in statistics and machine learning, where many authors have demonstrated that operating on pairwise comparisons can achieve optimal performance in a variety of settings (for example \cite{Maa1996,scholkopf2002learning}).  Specifically for testing relationships between two different properites, a landmark paper from Szekely and Rizzo proved that such an approach yields asymptotically optimal tests, regardless of the nature of the relationship \cite{SzekelyRizzo2009}. Figure \ref{f:newschem}{\color{magenta}B} shows the effectiveness of both \Mantel~and \Dcorr~in this setting.  
...
However, specifically for nonlinear  \cite{HellerGorfine2013} and high-dimensional settings \cite{SzekelyRizzo2013a}, this ``distance correlation'' (\Dcorr) approach requires an unfortunately high sample size.  To see why, consider a canonical nonlinear function: a spiral (Figure \ref{f:newschem}{\color{magenta}D}).  
In this specific example, points $1$ and $2$ are close in both $x$ and $y$, whereas points $2$ and $3$ are close only in $y$ and not $x$. By comparison, in the linear function, essentially all pairs that are close in $x$ or $y$ are close in both.
``Global methods'' that test for linear correlation across \emph{all} pairs of samples can therefore have their signal swampped by the large number of nonlinearly related pairs. Figure \ref{f:newschem}{\color{magenta}E} demonstrates the ineffectiveness of \Mantel~and \Dcorr~in this setting.  

Restricting analysis to only local pairs of samples has been profitably used in a wide range of data science problems (for example,  \cite{Stone1977, DaubechiesWaveletBook, Sarwar2000}).
Most recently considering locally linear pairs has been quite effective in unfolding nonlinear geometry in many nonlinear dimensionality reduction (or manifold learning) algorithms (for example, \cite{
TorgersonBook, 
...
However, this strategy has not been as effective for testing purposes \cite{David1966,Friedman1983,Schilling1986}.  
The main challenge for all of these ``local methods'', regardless of task, is determining the optimal \emph{scale}, that is: how can one determine which pairs of points are local in a statistically consistent and computationally efficient manner?


We therefore developed a ``multiscale method'' that we call \emph{ Multiscale Generalized Correlation} (\Mgc).  Rather than using only the global scale, or a single local scale, we compute our test statistic for all scales, and then find the optimal ones.  More specifically, for each property, for each sample, we can restrict  correlations to its $k$-nearest neighbors, for all possible values of $k$ from $1$ to $n$.  Because we are testing whether two properties are independent from one anoter, we can vary $k$ separately for each property.  Thus, we obtain an $n \times n$ multiscale map of local correlations.  These maps  indicate the nature of the relationship between the two properties.  For example, when the data are linearly related, the global scales are optimal, because all pairs of points are linearly related to one another (Figure \ref{f:newschem}{\color{magenta}C}).  On the other hand, for this spiral example, local in $x$ or $y$ is sufficient 
(Figure \ref{f:newschem}{\color{magenta}F}).



Our work is differentiated from the previous state of the art in two important ways.  First, empirically \Mgc~performs as well or better than global methods in essentially all benchmark settings and all sample sizes and dimensions.  In fact, we prove that \Mgc~statistically dominates global methods, meaning that it always performs as well or better than global methods, but never worse. 
Second, \Mgc~is the only approach to our knowledge that provides additional insight beyond a mere p-value. Specifically, the multiscale maps that \Mgc~provides indicates the strength of the relationship between the two properties as a function of the local scales.  Finally, utilizing \Mgc~is easy---it merely requires inputing $n$ samples of two measured properties---we provide open source implementations in both MATLAB and R from our website, \website.
...
% Thus only pair $(1,2)$ has linear information about whether $x$ and $y$ are dependent, which is easier to detect than nonlinear relationships.
% \Mgc~therefore detects dependence by identifying and restricting analysis to the jointly local pairs---the pairs that are close in both $x$ and $y$----thereby revealing dependence, in contrast to  previously proposed tests that are global.
% Crucially, by determining \emph{how local} to look, \Mgc~is the only test that can reveal information about the nature of dependence.

% \jv{add paragraph here explaining: we dominate, we provide multiscale maps, we are easy to use}




% \jv{make it super clear that "using" is easy.  }

\subsection*{Finite Sample Simulation Experiments}

When does \Mgc~outperform other approaches, and when does it not?
To answer this question, we compare  \Mgc~with previously proposed state-of-the-art tests:
\Mantel~\cite{Mantel1967}, \Dcorr~\cite{SzekelyRizzoBakirov2007}, \Mcorr~\cite{SzekelyRizzo2013a} and \Hhg~\cite{HellerGorfine2013}. Due to the theoretical advantages of \Mcorr~over \Dcorr~and \Mantel, we always use \Mcorr~as the global correlation for the \Mgc~implementation, unless mentioned otherwise. \Hhg~has previously been demonstrated to perform very well on all sorts of nonlinear dependencies, especially in low-dimensional settings, and enjoys strong theoretical guarantees. 
...
We consider $20$ different dependence settings;  a large fraction of these are taken precisely from the existing literature \cite{SzekelyRizzoBakirov2007, SimonTibshirani2012, GorfineHellerHeller2012, HellerGorfine2013, SzekelyRizzo2013a}.  
The settings include
linear and nearly linear  (1-5),
polynomial,
trigonometric,
uncorrelated but nonlinearly dependent (6-19),
and an independent relationship (20).
Details for each setting are given in Appendix \ref{appen:function}, with a visualization showing both noise-free (black) and noisy (gray) samples from each dependence function in Supplementary Figure~\ref{f:dependencies}.  Many of these functions were only used in $1$-dimensional settings in the previous literature.  We therefore defined multidimensional variants for each setting.  This is important because in our motivating applications, as well as many other real data problems, data are high-dimensional with low-sample sizes. 

Supplementary Figure~\ref{f:nDAll} shows  the power of each test for all $20$ settings with sample size fixed at $n=100$, but with dimensionality of \mbx~increasing (the dimensionality of \mby~increases in only a subset of the settings; see Appendix \ref{appen:function}).  
%
The advantage of Oracle \Mgc~over all benchmarks is stark, with sample \Mgc~performing very close to Oracle \Mgc. We summarize the advantage in Figure~\ref{f:nDSummary}, which compares the mean power of Oracle/sample \Mgc~to each benchmark. For the nearly linear settings, \Mgc~and \Mcorr~are almost identical and significantly better than all others for all dimensionalities.  For the remaining nonlinear dependencies, \Mgc~achieves power levels that are superior to all benchmarks for almost all functions, often by a significant margin.  For the independent simulation, all tests yield power at the significance level $\alpha$,  indicating no more false positives than expected.
More exhaustive benchmark experiments, including those focusing on the one-dimensional scenarios,
are qualitatively similar, and are therefore relegated to Figure~\ref{f:1DAll} and~\ref{f:1DSummary}.

%\cs{below is no longer necessary?}
%\subsection*{\Mgc~Empirically Dominates Global Counterparts}

% \jv{make more clear relationship between global and mgc}
...
\begin{figure}
  ...
  \caption{Summarizing the \Mgc~advantage over existing methods, based on the testing power of the $20$ different simulation settings in Figure~\ref{f:nDAll}. 
Each figure shows the mean power difference on the ordinate between \Mgc~and the benchmark we consider, with the simulation type on the abscissa, based on the mean power in each panel of Figure~\ref{f:nDAll}. The gray dots represent Oracle \Mgc~and the black dots represent sample \Mgc~(both are implemented based on \Mcorr); and the big dots on the x-axis stands for the average of all $20$ dots.
  Clearly \Mgc~is the most superior method over all existing benchmarks, exhibiting similar or better power for most simulations, with sample \Mgc~being very close to the Oracle \Mgc. Note that \Mgc~is a valid test, and does not inflate the false positive rate for the independent relationship.
  }
\label{f:nDSummary}
\end{figure}
...
% \jv{add paragraph here explaining what is happening}

The first step is to compute distances between all pairs of points \emph{within} each measured property (or modality). Converting each modality from its original representation to pairwise distance matrices enables comparisons across otherwise incommensurate data.  Moreover, this nonlinear operation on each data point is a special case of something known as the ``kernel trick'' that makes linear operations on the distance matrices equivalent to nonlinear operations on the original data  \cite{Maa1996, scholkopf2002learning}.
...
For all of these tests, and those that follow,  one can conduct a permutation test to obtain the p-value \cite{GoodPermutationBook}.

The second step is to ``single-center'' these matrices to ensure that each data point counts equally; without such centering, single data points far from the others could distort the statistics underlying Mgc's measure of dependency.
...
More formally, Szekely et al. (2007) \cite{SzekelyRizzoBakirov2007} proved the following when  $\delta_x$ and $\delta_y$  are  Euclidean distances. 
Subtracting the row and column means of $A$ and $B$ prior to taking their element-wise products and summing achieves optimal power as 
...
Szekely et al. \cite{SzekelyRizzo2013a} further proposed a modified version that we call \Mcorr. The authors prove that \Mcorr~is unbiased as the dimensions of $\mb{x}$ and $\mb{y}$ increase to infinity, thus consistent for high-dimensional dependencies as well.
...
Note that these distance-based tests merely require a distance function for both $X$ and $Y$. Using these distances, Lyons was able to prove that \Dcorr~is consistent even in general metric spaces satisfying certain conditions, including certain networks, shapes, and other structured spaces  \cite{Lyons2013}.
...
\subsection{Multiscale Generalized Correlation (\Mgc) Details}
\label{appen:mgc}
...
Algorithm~\ref{alg:sample_mgc} is the sample \Mgc~algorithm, which provides an estimate of the Oracle \Mgc~among all local correlations. It finds a sufficiently large region with significantly large and monotonically changing local correlations, and uses a large local correlation within the region as the sample \Mgc~statistic; otherwise it uses the global correlation. Algorithm~\ref{alg:pval} computes the p-values of all local correlation by the random permutation test, which also includes the p-value computation and the optimal scale estimation by sample \Mgc.
