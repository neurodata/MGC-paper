...
if sum(sum(P<P(end)))/(m-1)/(n-1)<=gamma
    ...
    [~,~,~,R]=FindLargestRectangles(R, [0 0 1], ceil(0.05*[m,n])); % find the largest rectangle within the smooth regions
    tmp=mean(mean(R));
...
[~, ~, ~, R]=FindLargestRectangles((R==1) & (P<=p), [0 0 1], ceil(0.05*[m,n]));
if sum(sum(R))==0 
    ...
    [~, ~, ~, R]=FindLargestRectangles((P<=p), [0 0 1], ceil(0.05*[m,n]));
end
...
for i=1:4
    [~, ~, ~,RC{i}] = FindLargestRectangles(RC{i} & (P<1), [0 0 1], ceil(0.05*[m,n])); % find the largest rectangle within each (approximately) monotonically decreasing / increasing region
end
