function [power1, power2, power3, power4]=CorrIndTestNoise(type,n,dim,lim,rep1,rep2)
% Author: Cencheng Shen
% Independence Tests for identifying correlation, returning empirical testing power with respect to increasing noise.
...
% CorrIndTestNoise(1,n,dim,lim,rep1,rep2);
%
...
% Output
power1=zeros(lim+1,K,rep1);power2=zeros(lim+1,K,rep1);power3=zeros(lim+1,K,rep1);power4=zeros(lim+1,rep1);%powers for rankdCorr, dCorr, modified dCorr, HHG
dCor1A=zeros(rep2,K,lim+1);dCor2A=zeros(rep2,K,lim+1);dCor3A=zeros(rep2,K,lim+1);dCor4A=zeros(rep2,lim+1);
dCor1N=zeros(rep2,K,lim+1);dCor2N=zeros(rep2,K,lim+1);dCor3N=zeros(rep2,K,lim+1);dCor4N=zeros(rep2,lim+1);
d=dim;
...
for r1=1:rep1
    ...
    for i=0:lim
        ...
        for r2=1:rep2
            ...
            if noise ~=0
                eps=i/lim*mvnrnd(0,1,n); %Gaussian noise
            else
                eps=zeros(n,1);
            end
            x=unifrnd(-1,1,n,d);
            ...
            if option1~=0
                dCor1A(r2,:,i+1)=rankDCorr(C,P,disRank,1);
            end
            if option2~=0
                dCor2A(r2,:,i+1)=rankDCorr(C,P,disRank,2);
            end
            if option3~=0
                dCor3A(r2,:,i+1)=rankDCorr(C,P,disRank,3);
            end
            if option4~=0
                dCor4A(r2,i+1)=HHG(C,P);
            end
        end
        
        % Then generate null distribution, i.e., X and Y are correlated by certain function type.
        for r2=1:rep2
            x=unifrnd(-1,1,n,d);
            %x=tan((2*x-1)*pi/2);%Cauchy
            ...
            disRank=[disToRanks(C) disToRanks(P)];
            if option1~=0
                dCor1N(r2,:,i+1)=rankDCorr(C,P,disRank,1);
            end
            if option2~=0
                dCor2N(r2,:,i+1)=rankDCorr(C,P,disRank,2);
            end
            if option3~=0
                dCor3N(r2,:,i+1)=rankDCorr(C,P,disRank,3);
            end
            if option4~=0
                dCor4N(r2,i+1)=HHG(C,P);
            end
        end
    end
...
% Save the results
filename=strcat('CorrIndTestNoiseType',num2str(type),'N',num2str(n),'Dim',num2str(dim));
%save(filename,'power1','power2','power3','power4','type','n','noise','std1','std2','std3','std4','rep1','rep2','lim','dim','dCor1N','dCor2N','dCor3N','dCor4N','dCor1A','dCor2A','dCor3A','dCor4A');
save(filename,'power1','power2','power3','power4','type','n','noise','std1','std2','std3','std4','rep1','rep2','lim','dim');
...
if display~=0
    ...
    % Plot figure
    noiseRange=0:1/lim:1;
    % Plot the best power among all neighborhood choice
    %plot(noiseRange,max(power1,[],2),'ro-',noiseRange, max(power2,[],2),'bx-',noiseRange,max(power3,[],2),'g^-',noiseRange,power4,'c.-','LineWidth',2);
    % Plot the power at the largest neighborhood size of dCorr
    plot(noiseRange,max(power1,[],2),'ro-',noiseRange, power2(:,end),'bx-',noiseRange,power3(:,end),'g^-',noiseRange,power4,'c.-','LineWidth',2);
    ...
    ylim([0 1]);
    titleStr = strcat('Independence Test for ', titlechar,' with Noise at n=', num2str(n), ' and m= ',num2str(dim));
    title(titleStr);
