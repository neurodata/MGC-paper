...
Given a set of paired observations, an ideal dependence test would efficiently detect dependence in both simulated and real ``dirty'' data, regardless of the nature of the dependence structure, including nonlinear,  high-dimensional, low-sample size, and non vector-valued observations.  Moreover, rather than merely providing a theoretically valid p-value, it would also provide insight into the nature of the dependence such as images and text.  
% 
...
% (4) Achieves high power and reveals nature of dependence not just on simulations, but also on real ``dirty'' data.
...
Our key insight is that if the two different modalities are depdendent, then local distances within one modality (images) should correlate with local distances within the other modality (text).  
...
We demonstrate that only \Mgc~has all of the above properties via simulation and theory.
...
% by finding a consecutive region in the p-value map that has smooth p-value change and low median p-value, and 
...
\For{$i,j:=1,\ldots,n$} \cs{name/clarify each loop in notes}
...
\For{$k:=1,\ldots,n-1$} \cs{name/clarify each loop in notes}
...
\For{$k,l:=1,\ldots,n$} \cs{explained this step already in the above pseudo-code, and you did not include comments for lines that were similar to the previous code, so i delete this comment for consistency. } %\Comment{center and normalize all local covariances}
...
\begin{algorithm}
\caption{Optimal Local Scales Approximation by P-values. This algorithm approximates the optimal scales $(k^{*},l^{*})$ from the p-values of all local correlations, and outputs both the optimal scales and their corresponding  \Mgc~p-value.  The running time is $O(n^2)$; see Appendix~\ref{appen:diss} for more discussion. \cs{you don't define $v_i$.  what is it? do you mean $v[i]$? and don't define $v$ to be a vector and its own difference, that confuses people and leads to coding errors. also, why $n-3$ for the sum? i don't understand line 10.  what is $k | \psi$?in fact, what is $k$ at all?? also, you require a threshold for smoothness, call it something, provide it as input? so icky :)}}%Since adjacent local correlations are strongly correlated and differ by only one term, this algorithm validates the smoothness of the local p-values by $\psi$ within each row and column respectively, selects the scale that are smooth within consecutive rows or columns, and takes the smallest p-value within the validated region. If no row and column are sufficiently smooth, the largest scale (i.e., the global correlation) are used instead. The running time is $O(n^2)$.}
...
\State $[p_{c},\LL]=\textsc{Validate}(P\T)$ \Comment{find consecutive scales of $P$ by column}
\State $p \rto \min\{p_{r},p_{c}\}$ \Comment{take the smaller value as the approximated p-value for \Mgc}
\State $[k^{*},l^{*}] \rto \argmin_{\left\{k \in \K \mbox{ or } l \in \LL \right\}} P\leq p$ \Comment{return all optimal scales within the validated range}
\EndFunction
\end{algorithmic}
\begin{algorithmic}[1]
%\Statex
...
\State Initialize $\K$ as an empty set, $\psi, \rho$ as two $n$-dimensional vectors of ones.
