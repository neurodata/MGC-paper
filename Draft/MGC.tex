...
% , which enables us to operate on scalars (univariate real numbers), rather than the data in its observed space, which might be high-dimensional and contain complicated  structure, like time-series, shapes, or graphs (for example, imagine studying the relationship between cloud \emph{shape} and grass wetness, rather than cloud density). 
...
% , that is, the correlation only incorporating local distances for the appropriately chosen local scale. 
...
% whereas the overlaid green dots only show the pairwise distances for the scales chosen by \Mgc~for $x$ and $y$.  The gray line in Figure \ref{f:newschem} (step 2) is the best linear fit to all the data; the green lines connect only the neighbors chosen by \Mgc. 
