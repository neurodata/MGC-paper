...
\end{figure}
% \end{comment}
%this was in the previous description, but not accurate: In almost all simulations, LDC outperforms the global special case.
...
18. Uncorrelated Binomial: $\mb{x} \sim Bernoulli(0.5)^{d_{x}}$, $\mb{u} \sim Bernoulli(0.5)^{d_{x}}$, $\mb{y}=w\mb{x} \cdot w(2\mb{u}-1)+0.6c\epsilon$.

19. Multiplicative Noise: $\mb{x} \sim normal(0, 1)^{d_{x}}$, $\mb{u} \sim normal(0, 1)$, $\mb{y}=\mb{x} \cdot \mb{u}+0.5c\epsilon$.
