...
\Dcorr, however, breaks down as the dimensionality of the data increases.  By adjusting the high-dimensional bias of \Dcorr, Szekely et al. \cite{SzekelyRizzo2013a} further proposed \Mcorr, which is proven to work better in high-dimensions (Figure \ref{f:schematic}C).
...
we define the rank-truncated pairwise comparisons:
\begin{equation}
\label{localCoef2}
    a_{ij}^k=
    \begin{cases}
    ...
    \end{cases}
\end{equation}
where $\bar{a}^{k}$ and $\bar{b}^{l}$ are two mean-adjusting scalars such that $\sum_{i,j=1}^{n} a_{ij}^k = \sum_{i,j=1}^{n} b_{ij}^l=0$. Then we can define a \emph{local} variant of any global generalized correlation coefficient:
\begin{equation}
\label{localCoef}
...
\Mgc~requires the computation of all $n^2$ local statistics, $\{\G_{kl}\}$, among which the best local statistic is used for testing (see Pseudocode \ref{alg:1scale} in Appendix \ref{appen:algorithms} for details on how to compute \Mgc~for a given neighborhood size).
...
We define the p-value of \Mgc~to be the p-value from the optimal scale, and declare significant dependency when the p-value is less than $\alpha$, often $0.05$ (see Pseudocode \ref{alg:best_scale} in Appendix \ref{appen:algorithms} for details). 
...
From this section onwards, we denote \Mgcd~as the \Mgc~for \Dcorr, \Mgcm~as the \Mgc~for \Mcorr, \Mgcp~as the \Mgc~for \Mantel.
...
%to generate repeating simulating samples and estimate the optimal scale for \Mgc, which are estimated separately for each implementation by dcorr / \Mcorr\ / Mantel.

It is immediately clear that in all 20 scenarios for all sample sizes, \Mgcm~performs as well or better than its global counterpart \Mcorr, and this is nearly true comparing \Mgcm~to \Hhg~as well. 
...
More specifically, for dependencies that are close to linear (type 1-5), \Mgcm~achieves the same power as \Mcorr~(as it should according to theory), both of which achieve a power as high or higher \Hhg, for all sample sizes.  
...
Figure~\ref{f:nD} shows power versus dimensionality of \mbx~(see Methods for details).  
...
% For these high dimensional settings, 
In the high-dimensional settings, the advantage of \Mgc~over its global counterparts is even more stark. For the nearly linear settings, \Mgcm~and \Mcorr~are essentially identical and significantly better than \Hhg~for all samples sizes.  For the remaining nonlinear dependencies, \Mgcm~achieves equal or high power than both \Hhg~and \Mcorr~for \emph{all} functions, often by a large margin.  Finally, for the independent setting, all tests yield power at the significance level $\alpha$, as they should, indicating no more false positives than there should be. 

% For all $20$ settings, for essentially all sample sizes, \Mgc~significantly outperforms both \Hhg~and \Mcorr.
...
\Mgcm~empirically achieves as high or higher power than the previous state of the art methods for all problems and all dimensions.
}
\label{f:nD}
\end{figure}
...
% In the following sections, unless mentioned otherwise, we concentrate on \Mgcm, because the three different \Mgc~implementations often yield similar numerical results with \Mgcm~being more advantageous based on the high-dimensional simulations.

\begin{figure}
  \centering
  \begin{tabular}{@{}p{0.2\linewidth}@{\quad}p{0.2\linewidth}@{\quad}p{0.2\linewidth}@{\quad}p{0.2\linewidth}@{}}
    \subfigimg[width=\linewidth]{}{Figures/Fig1DMantel} &
    \subfigimg[width=\linewidth]{}{Figures/Fig1DDcorr} &
		\subfigimg[width=\linewidth]{}{Figures/Fig1DMcorr} &
		\subfigimg[width=\linewidth]{}{Figures/Fig1DAll} \\
		...
		\subfigimg[width=\linewidth]{}{Figures/FigHDAll} \\
  \end{tabular}
...
\caption{
Same as Figure~\ref{f:1D} but includes all of \Mgcd, \Mgcm~and \Mgcp.}
\label{f:1DAll}
\end{figure}
...
\caption{
Same as Figure~\ref{f:nD} but includes all of \Mgcd, \Mgcm~and \Mgcp.}
\label{f:nDAll}
\end{figure}
...
Five algorithms are presented in section~\ref{appen:algorithms}: given the choice of a global correlation coefficient, algorithm~\ref{alg:1scale} computes one local correlation coefficient at a given $(k,l)$; then algorithm~\ref{alg:all_scales} shows how to compute all local correlations simultaneously; algorithm~\ref{alg:power} estimates the testing powers of all local statistics based on a given joint distribution or multiple pairs of data, which can be used to estimate the powers of all local tests / \Mgc~and thus the optimal scale for \Mgc; algorithm~\ref{alg:pval} computes the p-values of all local correlation by the random permutation test; and algorithm~\ref{alg:best_scale} approximates the optimal scale for \Mgc~based on the p-values of all local correlations, and outputs the approximated p-value of \Mgc. More detailed discussions regarding the optimal scale approximation is offered in section~\ref{appen:diss}.
...
Then by choosing $\alpha=0.25$, \Mgcm~has power $1$ while global \Mcorr~has power $0$, i.e., \Mgcm~successfully identifies the dependency in this example while global \Mcorr~fails. 
