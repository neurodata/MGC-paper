...
\item Joint normal $(\mb{x},\mb{y}) \in \Real^{D} \times \Real^{D}$: Let $\rho=1/2D$, $I_{D}$ be the identity matrix of size $D \times D$, $J_{D}$ be the matrix of ones of size $D \times D$, and $\Sigma = \begin{bmatrix} I_{D}&\rho J_{D}\\ \rho J_{D}&I_{D} \end{bmatrix}$. Then let $\epsilon \sim \mc{N}(0, I_{D})$,
\begin{align*}
...
\jovo{that is what Gabor wrote, plus it is not necessary to make them consistent?}
...
\jovo{no, $O(n^2)$: centering needs to count all matrix entries}
...
In the case of ties, \Mgc~selects the scale whose product, $k \times l$, is largest. \cs{is this correct?} \jovo{yes; although in practice it doesn't matter much}
...
\EndFor
%\Statex \Comment{the next two for loops with respect to the scales guarantee the computation of all local covariances and variances in $O(n^2)$} \cs{i don't know what this comment means.}
\For{$k:=1,\ldots,n-1$}
...
\State $[k^{*},l^{*}] \rto \arg\min_{\left\{k \in \K, l \in \LL\right\}} P_{kl}$ \Comment{find the optimal scale within the valid range}
\State $p \rto P_{k^{*}l^{*}}$
\EndFunction
\end{algorithmic}
...
\State $[k^{*},l^{*}] \rto \arg\min_{k, l} \{P_{kl},k,l=2,\ldots,n\}$
\For{$k:=k^{*},\ldots,2$} \Comment{check all row scales no larger than $k^{*}$}
\Lineif{$\min\{P_{kl},l=2,\ldots,n\}>\alpha$}{break}
\State $\K \rto [k,\K]$
\EndFor
...
\State $\K \rto \{n\}$ \Comment{take the largest scale if the median p-value is not sufficiently small}
\EndIf
\EndFunction
\end{algorithmic}
\end{algorithm}
...
\label{appen:diss}
Evaluating \Mgc requires estimating the optimal scale. Algorithm~\ref{alg:power} computes the testing powers of all local correlations for known or estimated distribution, so the optimal scale $(k^{*},l^{*})$ can be directly estimated by maximizing the testing powers (if there are more than one optimal scales, we pick the scale that maximizes the mean difference of the test statistic under the null and the alternative). Once the optimal scale is determined, the testing power of \Mgc~for the given distribution can be quickly determined by Algorithm~\ref{alg:power}, and its p-value for testing on a particular data realization can be determined by Algorithm~\ref{alg:pval}.
...
\begin{proof}
To show that \Mgc~is equivalent to the global correlation coefficient, it suffices to show the p-value of $\G^{kl}$ is always no less than the p-value of $\G$ for all $k,l$ under linear dependence. Without loss of generality, we assume all of $a_{ij}$, $b_{ij}$, $a_{ij}^{k}$, and $b_{ij}^{l}$ are of zero-mean, because simple centering or not does not affect the p-value.
...
\begin{equation*}
1=\G(X, Y) \geq \G(X, Y_{\pi})
\end{equation*}
for any permutation $\pi$, where the equality holds if and only if $X$ is a scalar multiple of $Y_{\pi}$, i.e., $a_{ij}=b_{\pi^{-1}(i) \pi^{-1}(j)}$ for all $i,j$, where $\pi^{-1}(\cdot)$ is the inverse permutation. %It follows that the p-value of $\G$ is $0$, which is at the minimal.

The identity permutation is one such example. However, for any permutation $\pi$ such that the equality holds, it is also applicable to rank-truncated comparisons too, i.e., $a_{ij}^{k}=b_{\pi^{-1}(i) \pi^{-1}(j)}^{l}$ if $a_{ij}=b_{\pi^{-1}(i) \pi^{-1}(j)}$. Therefore, for any $\pi$ such that $\G(X, Y) = \G(X, Y_{\pi})$, we always have $\G^{kl}(X, Y)=\G^{kl}(X, Y_{\pi})$ for all $k,l$; moreover, there may exist other permutations such that $\G^{kl}(X, Y)<\G^{kl}(X, Y_{\pi})$, but there is no $\pi$ such that $\G(X, Y) < \G(X, Y_{\pi})$.

Therefore the p-value of $\G^{kl}$ cannot be less than the p-value of $\G$ under linear dependency, in which case the global correlation is the optimal scale for \Mgc. \cs{i don't get it.} \jovo{expanded a bit now}
\end{proof}
...
\jovo{yes, the smallest p-value is 0.24.}
