...
\new{While existing approaches can test for whether two properties are related, they may require unacceptably large sample sizes, and do not provide insight into the geometry underlying the structure of the relationship. }
...
\new{We prove that to achieve a given false positive and false negative rate, \Mgc~requires equal or far fewer samples than existing methods for all possible dependence structures and dimensions, while maintaining computational efficiency. Moreover, \Mgc~uniquely reveals the latent geometry underlying the relationship.}
...
Identifying the existence of a relationship is the initial, critical step in the investigation of any properties within a dataset. Only if there is a statistically significant relationship does it make sense to further investigate; \new{such questions arise in high-content high-throughput screens, causal analyses, and predictive tasks.}
% @Jovo: perhaps put more content into HCHTS
% determine whether the relationship has predictive power or whether it reflects causality.
...
\includegraphics[width=0.52\linewidth]{Figures/Fig1All}
\caption{
...
% @jovo: update fig to use new one from lucidchart
...
\includegraphics[width=0.48\textwidth,trim={1.5cm 0 0cm 0cm},clip]{Figures/FigHDPowerSummarySize}
  ...
  \caption{
 \new{Sample size of different methods to achieve a power of $85\%$ at type 1 error level $0.05$, for the $20$ different settings under 1-dimensional (A) and high-dimensional (B) settings. 
...
\end{align*}
Any method for testing for independence can make two kinds of errors: it can falsely identify a dependence when none exists ($\alpha$, type I error), or it can falsely fail to identify a dependence when one exists ($\beta$, type II error). 
% It is standard to require that any test has a type I error less than $0.05$.  
% Therefore, t
To evaluate different tests, we compute the sample size required to achieve a power of $85\%$ (power is $1-\beta$) at $\alpha=0.05$.  
One test is better than another if it requires fewer samples to achieve the same power for a fixed type I error. }


We compare \Mgc~with four  state-of-the-art tests: (i) \Mantel, which is widely used in biology and ecology despite a lack of theoretical support \cite{Mantel1967}, (ii) \Dcorr, as discussed above, (iii) \Mcorr, a version of \Dcorr~designed to be unbiased in high-dimensional data \cite{SzekelyRizzo2013a}, and (iv) \Hhg, a powerful test designed for low-dimensional nonlinear settings \cite{HellerGorfine2013}. 
We consider $20$ different noisy dependence settings, most taken from the existing literature (some newly introduced here), including  nearly linear (1-5), strongly nonlinear (6-19), and independent (20) settings \cite{SzekelyRizzoBakirov2007, SimonTibshirani2012, GorfineHellerHeller2012, HellerGorfine2013, SzekelyRizzo2013a}.  
...
\new{For each setting, we compute sample size required to achieve $85\%$ power for both a one-dimensional setting, and a high-dimensional setting. The high-dimensional settings are more difficult because each dimension has less and less signal, so there are many noisy dimensions. 
Figure~\ref{f:Summary} demonstrates that for essentially all settings, in both low and high dimensions, \Mgc~requires far fewer samples to achieve high power.  The far right of each panel indicates the median sample size required over all $20$ settings.  For the one-dimensional cases, the best competitor still requires a median of $33\%$ more samples than \Mgc.  For the high-dimensional cases, the best competitor requires a median of nearly $250\%$ more samples.  For an experimentalist, the implications of this result is that using \Mgc~can substantially reduce the number of samples to identify a dependence. 
Supplementary Figures \ref{f:nDAll} and \ref{f:1DAll} further explore the relationship between power, sample size, and dimensionality for the different tests, including several additional variants of \Mgc.  
In nearly every setting \emph{and dimensionality}, \Mgc~requires substantially fewer samples than competing methods to achieve the same or higher power. 
Stated another way, at a fixed sample size of $100$, other methods only achieve between $15\%$ and $81\%$ average power as \Mgc~in the 1-dimensional settings, and between $26\%$ and $58\%$ power in the high-dimensional settings (see Supplementary Figure \ref{f:Summary2} for details).}




% shows the required sample size of \Mgc~and other benchmarks to achieve a testing power of $85\%$ at type $1$ error level $0.05$ for various 1-dimensional and high-dimensional settings. 
% It shows a significant improvement of \Mgc~over existing approaches, as \Mgc~requires less sample observations to achieve the same testing power throughout nearly all simulations with a wide margin on the median size.
...
\subsection*{\Mgc~\new{Characterizes the Geometry} of Dependence}
\label{main3}
...
\new{Moreover, the multiscale power maps for the nearly linear cases are all qualitatively similar.  This suggests that the signature of a linear relationship is a multiscale power map whose power is increasing as we increase both scales.}
...
Furthermore, similar dependencies have similar local generalized correlation structures, and thus, similar multiscale power maps. For example, logarithmic (10) and fourth root (11), though very different functions analytically, are \new{geometrically} similar, and yield very similar power maps.
...
Both circle (16) and ellipse (17), as well as square (14) and diamond (18), are closely related \new{geometrically}, and have similar  power maps. 
...
\subsection*{\Mgc~Theoretically Dominates its Global Counterparts \new{by Always Achieving a Given Power with Fewer (or the same number of) Samples}}
\label{s:theory}
...
\new{Thus, no matter which dependence function, dimensionality, and type I error level, Oracle \Mgc~requires fewer (or an equal number of) samples to achieve a given power, compared to its global counterparts.}
% 
% More precisely, in \emph{linear} settings Oracle \Mgc~achieves the same power as the global test, and in various nonlinear settings, Oracle \Mgc~achieves {higher} power than the global test. 
\new{More precisely, in \emph{linear} settings Oracle \Mgc~requires the same number of samples to achieve power of the global test, and in various nonlinear settings, Oracle \Mgc~requires far fewer samples to achieve the power of the global test. }
% 
\end{thm}
...
\new{Finally, though a na\"ive implementation of \Mgc~requires $\mc{O}(n^4)$ operations, we have devised a serial implementation that requires only $\mc{O}(n^2 \log n)$.  And, assuming access to parallel computation, for example on a modern multi-core architecture, common even in mobile phones now, \Mgc~requires only $\mc{O}(n^2 \log n / T)$ operations, where $T$ is the number of parallel threads (see Algorithm \ref{alg:all_scales} for details).  Since, $T$ is often larger than $\log n$, in practice, \Mgc~is actually $\mc{O}(n^2)$, and a constant factor slower than \Dcorr.  For example, when $n=5000$, \Dcorr~requires 0.5 seconds, whereas \Mgc~requires 5 seconds. Obviously, the cost and time to obtain $2.5 \times$ more data far exceeds a few seconds.  } 
...
% @cs: let's wait until we finish the other data experiments
\label{f:real}
\end{figure}
...
% 
Once all local significance values are computed, the optimal scales are estimated by the tightest bounding box in the local scales that is no larger than the p-value of Sample \Mgc~and no smaller than the statistic of Sample \Mgc~(see Algorithm \ref{alg:pval} for details), which serves as an estimation of all potential optimal scales. This is useful because the estimated Sample \Mgc~statistic is only from one local scale, but the multiscale power maps in Figure~\ref{f:powermaps}  shows many scales are close to optimal.
...
This algorithm uses the random permutation test with $r$ random permutations, resulting in the p-value, the estimated optimal scales, and the multiscale significance map, requiring $\mc{O}(rn^2 \log n/T)$. Specifically, it computes the p-values by comparing the multiscale correlation map and the sample \Mgc~statistic of the observed data, to those of each permuted resample.  Then, the optimal scales are estimated by taking the largest rectangle with local statistics no smaller than Sample \Mgc~and local p-values no larger than the p-value of Sample \Mgc.  In the real data experiment we always set $r=10$,$000$. Note that the p-value computation for any other global generalized correlation coefficient follows from the same algorithm by replacing Sample \Mgc~with the respective test statistic.
}
\label{alg:pval}
\begin{algorithmic}[1]
