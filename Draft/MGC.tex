...
% 
% @jovo: maybe seomthing about  "including our motivating problems of brain activity, shape, and networks."
% @bm: clear? 
...
{\color{green}Insights into the data available only from running \Mgc~are highlighted in green.}  Results using \Dcorr~\cite{SzekelyRizzo2009}, a state-of-the-art dependence test that \Mgc~extends, are shown for comparative purposes. 
% 
...
The heatmap shows p-values for all scales (computed via a permutation test), the scale with maximum local correlation (green dot), the estimated set of optimal scales  (those in the largest rectangle with significant local p-values, green box), and the global scale (gray dot; 
