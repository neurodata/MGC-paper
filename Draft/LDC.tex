...
\jv{ok, let's make an explicit ``running times'' subsubsection in the theory section (by just moving the paragraph over there, and wordsmithing as appropriate.)}
...
\end{align*}
\jv{use a $\left($ or $\bigg($ or something to make the parens the right size.}
...
\end{align*}
\jv{did you really mean $\cdot$? u is a scalar? you seem to use $ab$ to denote times.}
...
\jv{crop fig0 using latex to make it wider (ie, remove whitespaces)}
}
\label{fig0}
\end{figure}
