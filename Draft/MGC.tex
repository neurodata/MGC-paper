...
Because the p-value maps are reasonably smooth, sample \Mgc~selects the largest smooth region of the multiscale p-value map as the estimated set of optimal scales, and the largest p-value in that rectangle is the p-value (Algorithm \ref{alg:best_scale}).
...
\begin{algorithm}
\caption{Computation of p-value and optimal scales for sample \Mgc. First it locates the smooth regions of the p-value map: we compute the difference of adjacent local p-values within each row, label each difference a smooth change if it is bounded above by a small positive number $\tau$ (i.e., the local p-values have a decreasing trend within the rows), and compute the largest rectangular region of smooth changes. Do it for p-value changes with increasing / decreasing trend within each row / column, and combine the four smooth rectangles into the smooth regions. If the global p-value is among the top p-values, i.e., within $\gamma \%$ of all local p-values, we directly use the global p-value for sample \Mgc; otherwise we compute the largest rectangle within the smooth regions, and take a smaller p-value within the rectangle as the p-value for sample \Mgc~if the area of the rectangle is larger than $\gamma$. Then the largest rectangle that is bounded above by the \Mgc~p-value within the smooth regions is taken as the optimal scale. The running time is $O(n^2)$.}
...
\State $\gamma \rto 0.06$ \Comment{threshold to check significant global p-value and significant region}
...
\If{$\sum_{k,l} R_{kl}==0$} \Comment{if the above returns empty, relax the smooth region constraint}
...
\State $R^{1} = \textsc{LargestRectangle}(P \leq \tau)$ \Comment{largest smooth rectangle with decreasing trend} 
\State $R^{2} = \textsc{LargestRectangle}(P \geq -\tau)$ \Comment{largest smooth rectangle with increasing trend} 
\State $R^{3} = \textsc{LargestRectangle}(P^{'} \leq \tau)$ \Comment{repeat for columns changes}
\State $R^{4} = \textsc{LargestRectangle}(P^{'} \geq -\tau)$
