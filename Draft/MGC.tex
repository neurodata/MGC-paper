...
and define $b^k_{ij}$ similarly, except the rank index is transposed. \jv{I transpose the rank above for symmetry of local correlations}
...
\Linefor{$Z:=A,B$}{$R^{Z}=\textsc{Sort}(\tilde{Z})$} \Comment{proper sorting  assuming no ties}
...
Therefore the number of ``significant'' permutations for $\G^{kl}$ at least equal those for $\G$ under linear dependency, and the p-value of $\G^{kl}$ cannot be less than the p-value of $\G$, in which case the global correlation is the optimal scale for \Mgc. 
\end{proof}
...
\end{appLem}
\begin{proof}
For fixed $k,l$, denote $R^{A}$ as the binary matrix such that $R^{A}_{ij}=1$ if $rank(a_{ij}) \leq k$, $R^{A}_{ij}=0$ otherwise; similarly define $R_{B}$. Then the rank-truncated pairwise comparisons $\mt{a}_{ij}^k$ and $\mt{b}_{ij}^l$ in Equation~\ref{localCoef2} are the entries of $A \circ R^{A}$ and $B \circ {R^{B}}^{T}$ respectively, where $\circ$ denotes the entrywise product.

It follows that the local covariance can be rewritten as
\begin{align*}
z_{kl} \G^{kl}(X,Y) &= \textstyle \sum_{i,j=1}^n a_{ij}^k b_{ij}^l \\
 &= tr((A \circ R_{A})^{T} \times (B \circ R_{B}^{T})) \\
 &= tr((H\tilde{A} \circ R_{A})^{T} \times (\tilde{B}H \circ R_{B}^{T})) \\
\end{align*}
\cs{proof}

As to rank preserving, the column ranks of the Euclidean distance matrix $\tilde{A}$ are the same as the column ranks of $A=H \tilde{A}$, because $H \tilde{A}$ centers each entry of $\tilde{A}$ by column means, while double centering $H \tilde{A} H$ does not always preserve the original ranks. Similarly, the row ranks of $\tilde{B}$ are preserved in $B=\tilde{B}H$ but not $H \tilde{B} H$.
\end{proof}
