...
\cs{ I feel it is necessary to first state the hypothesis testing and some notations before presenting the theorems regarding testing powers; and Theorem~\ref{thm2} is general enough and a good property to be put in the main paper rather than appendix, so I moved them from appendix to here} 
...
Note that for all distributions other than the independent clouds, the empirical powers eventually increase to $1$ as the sample size increases, implying that all methods are consistent except the Mantel test, whose powers stay low in many nonlinear dependencies; and for the independent clouds, all testing powers including MGC are approximately at the type $1$ error level, so no false signal is detected. %In the appendix we also provide the testing powers for MGC by dcorr and MGC by Mantel, which have similar interpretation as MGC by mcorr, with MGC by Mantel being better at certain trigonometric dependencies.
...
\jv{use an enumerate environment for these maybe? the list doesn't look that pretty to me. also, break them up into sub-sections: linear, trig, etc., as you did in the main text. i would use $\mc{U}$ rather than $uniform$.  maybe use a display equation, yah i like that better}
...
\end{equation}
\item Cubic: $\mb{x} \sim uniform(-1,1)^{d_{x}}$, 
\begin{equation}
...
\caption{Same Figures as Figure~\ref{fig:pp}, adding in MGC by dcorr and MGC by Mantel}
\label{fig:pp2}
\end{figure}
