...
% @jovo: "excel in different settings" is semantically weird.
...
% @jovo probably can shorten the above sentence?
...
% * <stefaniejacinto@gmail.com> 2017-01-13T01:10:50.329Z:
% 
% > large p, small n
% @jovo: I added a comma here
% - @sd
% 
% ^.
% @sd: ok
...
Many statistical and machine learning approaches have been developed over the last 120 years to combat the above issues in a wide variety of  settings. Specifically, pairwise comparison-based approaches have been developed for various applications,  ranging from hypothesis testing (e.g., \cite{David1966,Mantel1967,Friedman1983,Schilling1986,Maa1996,SzekelyRizzo2009,SzekelyRizzo2013b,HellerGorfine2013,Dumcke2014}) 
% * <stefaniejacinto@gmail.com> 2017-01-13T01:12:18.177Z:
% 
% > have been utilized  for various purposes
% @jovo: I feel like we don't need this part of the sentence, since in the previous sentence we already say "in a wide variety of settings".
% I suggest:
% "... in a wide variety of settings. Specifically, this includes pairwise comparison-based approaches, ranging from..."
% What do you think?
% -@sd
% 
% ^.
% @sd: made a related change, i think an improvement. #jovo
...
% * <stefaniejacinto@gmail.com> 2017-01-13T01:50:19.447Z:
% 
% > This requires specifying $k$ in advance, and ignores the magnitude of the correlation, meaning that when $k$ is large the resulting test statistic is  uninformative. 
% @jovo: I changed "magnitudes" to "magnitude"
% However this sentence confuses me a bit, so it might benefit from some clarification for someone like me. :) I understand that nearest neighbor methods do require k to be specified in advance, and when k is too large then the method becomes rather useless. But how exactly does the method ignore the magnitude of the correlation? Also, does it only ignore the magnitude of the correlation for large k? Or any k?
% -@sd
% 
% ^.
% @sd: clarified? #jovo
% 
% * <stefaniejacinto@gmail.com> 2017-01-13T01:29:11.764Z:
% 
% > measurement types
% @jovo: I'm a little confused by what we mean here. Is this just another term for the two properties that are being compared?
% -@sd
% 
% ^.
% @sd: clarified #jovo
% * <stefaniejacinto@gmail.com> 2017-01-13T01:16:03.623Z:
% 
% > Dependence tests have developed specifically to address the above issues. 
% @jovo: It feels awkward to reference "the above issues" when the issues are not in the paragraph immediately preceding it. Also, it sounds too similar to the first sentence of the previous paragraph ("Many statistical and machine learning approaches have been developed over the last 120 years to combat the above issues...")
% Before I suggest what to put here instead of this sentence, I think I need some clarification on what we mean by dependence tests. Is this on the same level as the pairwise comparison-based approaches mentioned in the previous paragraph? That is, are we essentially saying the following:
% 1. Many approaches have been developed to combat the issues
% 2. One type is the pairwise comparison-based approach
% 3. Another type is the dependence test
% -@sd
% 
% ^.
% 
% @sd: clarified? #jovo
More recently, these ``local'' tests were discarded in favor of global approaches.  The Distance correlation test (\Dcorr) was proven to work asymptotically for arbitrary nonlinearities \cite{SzekelyRizzo2009}, for large dimensions \cite{SzekelyRizzo2013a}, and structured data \cite{Lyons2013}.
% * <stefaniejacinto@gmail.com> 2017-01-13T01:57:16.649Z:
% 
% > discarded in favor of global approaches
% @jovo: Edited for clarity (previously read "discarded for global approaches")
% -@sd
% 
% ^.
...
% * <stefaniejacinto@gmail.com> 2017-01-13T02:13:48.378Z:
% 
% > like
% @jovo: Deleted typo (extra "like")
% -@sd
% 
% ^.
% @sd: ok #jovo
...
% * <stefaniejacinto@gmail.com> 2017-01-13T02:16:01.052Z:
% 
% > mathematics,
% @jovo: Added comma for flow
% -@sd
% 
% ^.
% @sd: ok #jovo
...
% * <stefaniejacinto@gmail.com> 2017-01-14T03:21:54.799Z:
% 
% > Illustration
% @jovo: Is it possible to make the flow of Figure 1 clearer? I'm not sure what the down arrows are supposed to mean. The label for section (i) should refer to the two graphs above it, but the down arrow implies otherwise. Also, the label for section (ii) seems to be sandwiched between the graphs for two methods of getting the local correlation (i.e. Dcorr vs MGC methods).
% -@sd
% 
% ^.
% @sd: clarified? #jovo
...
% 
\textbf{(i)} Compute all distance pairs. Distances are linearly correlated in the linear setting, whereas they are, in general, not in the spiral setting.  \Dcorr~uses all distances (gray dots) to compute its test statistic, $\GG(\Dcorr)$, which \Dcorr~uses to compute its p-value.
% 
\textbf{(ii)} Find the maximum local correlation between $x$ distances and $y$ distances.  The scale with maximum local correlation is the global scale for the linear setting, where it is a very local scale for the spiral setting (panel titles state the maximal local correlation $\GG(\Mgc)$, and the scales that achieve it, green circles show the distances included in that scale).
% * <stefaniejacinto@gmail.com> 2017-01-21T00:39:37.288Z:
% 
% > maximal
% @jovo: Is there a particular reason for switching between "maximum" earlier in the paragraph and "maximal" here? Might be better to just be consistent?
% -@sd
% 
% ^.
% 
\textbf{(iii)} Compute the p-value and estimate the optimal scales.
The heatmap shows p-values for all scales (computed via a permutation test), the scale with maximal local correlation (green dot), the estimated set of optimal scales  (those in the largest rectangle with significant p-values, green box), and the global scale (gray dot; 
...
% * <stefaniejacinto@gmail.com> 2017-01-14T02:15:56.758Z:
% 
% > grass wetness
% @jovo: I assume you meant grass wetness (this previously said "grass shape").
% -@sd
% 
% ^.
% @sd: ok #jovo
...
% This suggests that global correlation between distances---meaning computing the correlation between all pairs of distances, both small and large---can determine whether these two properties are related.  
% * <stefaniejacinto@gmail.com> 2017-01-14T02:32:17.626Z:
% 
% > pairs of distances
% @jovo: Maybe we could clarify this is a bit more. This part starts off talking about "pairs of observations", and when "distances" is mentioned, in my mind this naturally leads to the idea that the distance being referred to is the distance between the pairs of observations mentioned earlier. But then when we start to mention "pairs of distances", it's not super clear to me which pairs we're talking about. (This is made clear in Figure 1, but perhaps we could find a way to introduce the concept of pairs of distances more smoothly?)
% -@sd
% 
% ^.
% @sd: changed sentence.  clarified? #jovo
...
% * <stefaniejacinto@gmail.com> 2017-01-14T02:45:10.120Z:
% 
% > Here again, when a pair of observations are close to one another in cloud shape, they also tend to be close to one another in grass wetness (see points 1 and 2 again).  However, in this nonlinear relationship,  a large distance between cloud shapes
% @jovo: Is cloud density the same as cloud shape? This is a little confusing since earlier, we only compared cloud density to grass wetness. Now we're comparing cloud shape to grass wetness. If we really intend to use a different variable for Figure 1B, we should mention it above in line 87 or 88.
% -@sd
% 
% ^.
% @sd: was typo, meant density, not shape
...
% * <stefaniejacinto@gmail.com> 2017-01-14T02:50:52.290Z:
% 
% > Use too few and much of the data are ignored; use too many and 
% @jovo: Edited for clarity
% -@sd
% 
% ^.
% * <stefaniejacinto@gmail.com> 2017-01-14T02:49:14.009Z:
% 
% > only the
% @jovo: Removed "to"
% -@sd
...
% * <stefaniejacinto@gmail.com> 2017-01-14T02:52:42.009Z:
% 
% > comparison-based
% @jovo: Added hyphen here
% -@sd
% 
% ^.
% @sd: ok #jovo
\Mgc~explicitly addresses the fundamental trade-off suggested above: as more neighbors are included (that is, as the scale increases), the effective sample size increases; however, 
 more neighbors also means that in nonlinear relationships, the neighbors are less likely to be linearly related, potentially decreasing power.
% * <stefaniejacinto@gmail.com> 2017-01-14T02:58:32.617Z:
% 
% > more neighbors also means the neighbors are less likely to be linearly related, potentially decreasing power.
% @jovo: Does more neighbors increase the chance of the relationships being nonlinear vs linear? I feel if we mentioned nonlinear relationships here, it would link more tightly with what was said in the previous paragraph, i.e. "use too many and nonlinear relationships could be overlooked".
% -@sd
% 
% ^.
% @sd: added caveat of nonlinear relationships.  clarified? #jovo
...
% * <stefaniejacinto@gmail.com> 2017-01-21T01:02:00.755Z:
% 
% > implying a strong nonlinear relationship that is undetected by \Dcorr~but revealed by \Mgc~.
% @jovo: I just rephrased this for better flow
% -@sd
% 
% ^.
% @jovo: explain mitigating the multiple hypothesis testing issue
...
% By virtue of operating on distances, it can handle structured data (such as time-series, shapes, or graphs).  Operating on distances also mitigates the ``large p, small n'' problem because the dimensionality p does not explicitly play a role once the distances are calculated.  
...
% * <stefaniejacinto@gmail.com> 2017-01-21T23:15:33.699Z:
% 
% > obtain a similarity matrix comparing all regions
% @jovo: Deleted "across" (this was previously "...matrix across comparing all regions")
% -@sd
% 
% ^.
