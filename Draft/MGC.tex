...
\begin{tabular}{c  c  c  c}
 $\delta_x$(1,3) & \hspace{1.8em} -0.66 \hspace{1.8em}  & \hspace{1.8em} -0.77 \hspace{1.8em} & \hspace{1.8em} -0.73 \hspace{1.8em}  \\ 
 $\delta_y$(1,3) & -0.52 & -0.41 & -0.19  \\ 
 $\delta_x \times \delta_y$ & \color{green}0.34 & \color{green}0.32 & \color{green}0.14  \\ 
 
\hline


 $\delta_x$(3,9) & -0.46 & -0.39 & 0.04  \\ 
 $\delta_y$(3,9) &  0.44 & 0.40 & 0.22  \\ 
...
\jv{because the largest distance is $4$ before, while now it is $114$, such that MGCScaleVerify can be more trustable }
...
\end{lem}
Our \Mgcd is in fact based on this single-centered \Dcorr, which is more advantageous than \Mgc based on double-centered \Dcorr (shown in Appendix~\ref{appen:mgc}).

\cs{don't we have a lemma here? also, explain why we left center vs right center (ie, for symmetry)}
\jv{moved lemma here. Left center and right center is not for symmetry.}
...
\begin{algorithmic}[1]
\Require The p-value matrix $P \in \Real^{n \times n}$ of all local correlations, a significance level $\alpha$.
\Ensure The approximated \Mgc~optimal scales $(k^{*},l^{*})$, and the approximated \Mgc~p-value $p$.
\Function{MGCScaleVerify}{$P,\alpha$}
\State $[p_{r}]=\textsc{Validate}(P,\alpha)$ \Comment{find consecutive scales of $P$ by row}
\State $[p_{c}]=\textsc{Validate}(P\T,\alpha)$ \Comment{find consecutive scales of $P$ by column}
\State $p \rto \min\{p_{r},p_{c}\}$ \Comment{take the smaller value as the approximated p-value for \Mgc}
\State $[k^{*},l^{*}] \rto \argmin_{\{k,l\}} P_{kl}\leq p$ \Comment{return all optimal scales within the validated range}
\EndFunction
\end{algorithmic}
\begin{algorithmic}[1]
%\Statex
...
\Function{Validate}{$P,\alpha$}
\State Initialize $\psi$ as a matrix of zeros of the same size as $p$, and $\rho$ as a vector of zeros of size $n$.
\State $n_{2} \rto \left\lceil 0.05 \times n \right\rceil$ \Comment{Check adjacent column scales}
\For{$j:=2,\ldots,n$}
\Lineif{$\min{P[:,j]}>\alpha$}{break} %\Comment{check the region size}
\For{$i:=2,\ldots,n$}
\State $\psi[i,j] \rto \mb{I}\{P[i,j]-P[i-1,j] \leq 0\}$ \Comment{Find a region of non-increasing p-values}
\EndFor
\EndFor

\For{$j:=2,\ldots,n$}
\State $j_{1} \rto \min\{2,j-n_{2}\}$
\State $j_{2} \rto \min\{n,j+n_{2}\}$
\State $\rho[j] \rto \max_{\psi[i_{1}:i_{2},j_{1}:j_{2}]=1}{\frac{i_{2}-i_{1}}{n}}$ 
\EndFor

\State $\epsilon \rto \max(0.2,\rho_{n}+3\textsc{Std}(\rho))$

\If{$\max_{i}{\rho_{i}} \geq \epsilon$} 
\State $p \rto \min_{\mbox{ all } i \mbox{ with } \rho_{i} \geq \epsilon}{P[:,i]}$
\Else
\State $\K=\{n\}$
\State $p = \min\{P[n,n],\textsc{Median}(P)\}$ \Comment{use the median or the last p-value instead}
\EndIf
\EndFunction
\end{algorithmic}
\end{algorithm}
