...
Column 3: Double centering yields $A=\{a_{ij}\}$ and $B=\{b_{ij}\}$, from which we compute the element-wise product $A \circ B$.  Double summing over the Hadamard product yields the un-normalized \Mcorr~statistic \cite{SzekelyRizzo2013a}, which is significant only when that double sum is positive. 
