...
Discovering whether certain properties are associated with other properties is fundamental to all science. Identifying the existence of a relationship is a precursor to investigating the structure of the relationship, and whether it has predictive power, or reflects causality.
...
%
First, the \textbf{dimensionality} of individual samples is growing at exponential rates, with genomics and connectomics data, for example, often accruing millions or even billions of dimensions. 
...
% 
Fourth, the dependencies between different properties (or experimental modalities) of data can be highly \textbf{nonlinear}, posing additional obstacles for the analysis of very large datasets.
%
Fifth, because of the near ubiquity of data deluges,  \textbf{computationally efficient} methods for analysis are critical for generating results within acceptable time frames
%
...
A method is needed that has compelling evidence---including extensive simulations, theoretical claims, and real data applications---that  satisfactorily addresses all of these challenges. 
...
\node[xshift=-5.2cm,yshift=-0.6cm] at (current page.east){%
    \includegraphics[width=0.35\textwidth,trim={1.5cm 0 1.5cm 0},clip]{Figures/FigB}};
...
% \jv{sd says: The description for Figure 1 contains inconsistent verb tenses. e.g. "Choose" vs "subtracting". It would be better to be consistent.}
%\cs{I have made them the same, i.e. subtract}
...
% \jv{It's not clear to me when we should be saying "power" vs "powers".} 
%\cs{I have changed to power throughout for consistency}
...
\jv{highlight that we are the only obvious way to determine that the relationship is linear for high dimensional data (otherwise, one needs to compute the linear regression with inverting the covariance matrix which is poorly scaled in high dimensions?}
