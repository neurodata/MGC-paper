...
\cs{providing all local p-values and selecting the optimal neighborhood size.}
% ,  denoted  $(k^{*},l^*)$. 
...
\cs{If the distribution of the data is already known (of course, this is only for simulations), the optimal scale for \Mgc~can be easily determined via simulation.  When the distribution of data is unknown,} 
...
\cs{deleted: A schematic illustrating  how to calculate the \Mgc~statistic is provided in Figure~\ref{fig:A}, with more details provided in Appendix \ref{appen:tests}.}
