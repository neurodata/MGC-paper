...
Thus, existing generalized correlation coefficient based tests  work well in nonlinear and high-dimensional data,  including structured domains. But, empirically, they struggle in various nonlinear settings. We conjectured that their struggles in nonlinear and high-dimensional settings was due to the fact that they were \emph{global methods}, and therefore consider all pairs of points, including non-local ones. They also can not provide insight into the scales of the dependence.

The third step is therefore to compute a local variant of the test.
This locality principle has already reaped considerable benefits in a wide range of data science problems, including for  classification and regression  \cite{Stone1977}, data compression \cite{DaubechiesWaveletBook}, and recommender systems \cite{Sarwar2000}.
...
\Mgc~can be thought of as a sparse or regularized variant of a global correlation test, and therefore faces the same dilemma as all regularized algorithms (including sparse methods, feature selection, and dimensionality reduction): how to efficiently choose the hyper-parameters. \Mgc~requires choosing the locality scales, both to yield a consistent test and to reveal the scales of dependence.  
Many manifold learning algorithms require that the user essentially runs the entire algorithm again from scratch for each different hyper-parameter setting, a pursuit that can be exponentially taxing as the number of hyper-parameters increases.  
Since there are $n \times (n-1)/2$ different local tests, one for each possible combination of $k$ and $l$ (local correlations are symmetric, see Appendix \ref{appen:mgc} for details), the challenge is to find a way to compute all local tests in a computationally efficient fashion that still yields a statistically valid test. 
...
Our estimator therefore does not search over multiple hyperparameters, and therefore does not suffer from multiple hypothesis testing problems \cite{Benjamini1995}.
...
\begin{algorithm}
\caption{Computation of p-value and optimal scales for sample \Mgc. First we locate a smooth region of the p-value map as follows: we compute the difference of adjacent local p-values within each row, label each difference a smooth change if it is bounded above by a small positive number $\tau$ (i.e., the local p-values have a decreasing trend within the rows), and compute the largest rectangular region of smooth changes. Do it for decreasing / increasing p-values within each row / column, combine all four smooth rectangles into one region, then locate the largest rectangle in the region with all p-values less than $0.5$. If the global p-value is among the top p-values, i.e., within $\gamma \%$ of all local p-values, we directly take the global p-value for sample \Mgc; otherwise if the area of the verified smooth region is larger than $\gamma$, we take a small p-value within the region as the p-value for sample \Mgc. Then the the largest rectangle that is bounded above by the \Mgc~p-value within the smooth region is taken as the optimal scale. The running time is $O(n^2)$.}
...
\State $R = \textsc{LargestRectangle}(R \cap (P<0.5))$ \Comment{the verified smooth region}
\EndFunction
\end{algorithmic}
\end{algorithm}
