...
These choices of distances are the only choices in the algorithm. \cs{not sure whether the above sentence is clear and right, you probably mean using Euclidean distance?}
...
Szekely et al. \cite{SzekelyRizzo2013a} further proposed a modified version that we call \Mcorr, which they prove to be unbiased (i.e., of zero mean) even as the dimensions of $\mb{x}$ and $\mb{y}$ increase to infinity, thus consistent for high-dimensional dependencies as well.
...
In real data without sufficient training data, the true distribution is unavailable so we cannot compute the powers to calculate the optimal scales or the optimal test statistic.
...
% However, the global test is quite fragile,  even ignoring a single subject from the global test can render the test non-significant. On the other hand, the \Mgc~multiscale map shows a large region that is quite significant  (Table \ref{t:real}).  
...
Algorithm~\ref{alg:sample_mgc} provides an estimate of the Oracle \Mgc~among all local correlations. When there is a sufficiently large region with significantly large and monotonically changing local correlations, a large local correlation within the region is used as the sample \Mgc~statistic; otherwise we take the global correlation. %More detailed discussions regarding sample \Mgc~is offered in Appendix~\ref{appen:diss}.
...
\State $\beta_{kl} \rto \sum_{j=1}^{r}(\G^{1}_{kl}[j]>\omega_{\alpha}) / r$ \Comment{get the power map}
\EndFor
\State $(k^{*},l^{*}) \rto \arg_{k,l}\max(\{\beta_{kl}\})$ \Comment{find the optimal local scale}
\EndFunction
\end{algorithmic}
\end{algorithm}

\begin{algorithm}
\caption{Compute the sample \Mgc~test statistic. By default, the global distance correlation is always used. But if there exists a sufficiently large region such that all correlations within the region are monotonically increasing or decreasing along the row or column, and are significantly large (governed by the threshold $\epsilon$ computed from negative local correlations), we take a large correlation within the smooth and significant region. The running time is $O(n^2)$.}
...
\If{$\sum_{k,l} R_{kl} \geq 2n$} \Comment{proceed only when there exists sufficiently many significant correlations}
\State $R = \textsc{Smooth}(\{\G^{kl}\},R)$ \Comment{check monotonically changing local correlations} 
...
\For{i:=1,\ldots,4}
\State $R[i] \rto \textsc{Connected}(R[i] \cap R)$ \Comment{largest connected component}
...
\begin{algorithm}
\caption{Compute p-values for all local tests and sample \Mgc, and estimate the optimal scale. This algorithm computes the p-values of all local correlations and sample \Mgc~by the permutation test with $r$ random permutations, which takes $O(rn^2 \log n)$. Then we estimate the optimal scales by the largest rectangle with local p-values less than or equals to the p-value of sample \Mgc. $\pi$ denotes a random permutation, and $\tilde{B}(\pi,\pi)$ denotes the distance matrix with the observation label permuted. In the real data experiment we always set $r=10$,$000$. Note that the p-value computation for any other benchmark follows from the same algorithm by replacing sample \Mgc~with the respective test statistic without the optimal scale estimation.}
\label{alg:pval}
\begin{algorithmic}[1]
