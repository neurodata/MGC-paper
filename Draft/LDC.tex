...
\end{equation}
where $z$ is the product of standard deviations, that is $z=\sigma(a) \sigma(b)$, and $\bar{a}$ and $\bar{b}$ denote the sample means of all $\{a_{ij}\}$ and $\{b_{ij}\}$
...
\end{equation}
where $\bar{a}^{k}$ and $\bar{b}^{l}$ are the truncated means, and $z_{kl}$ is the product of the truncated standard deviations $\sigma(a)^{k}\sigma(b)^{l}$. There are a maximum of $n^2$ different local correlations, one for each possible combinations of $k$ and $l$ (more details for \Mgc are in Appendix \ref{appen:mgc}).
...
\label{appen:mgc}
The generalized correlation coefficient can then be written:
\begin{equation}
\label{generalCoef}
\G= \tfrac{1}{z} {\textstyle \sum_{i,j=1}^n (a_{ij}-\bar{a}) (b_{ij}-\bar{b}) }, 
\end{equation}
where $z$ is the product of standard deviations, that is $z=\sigma(a) \sigma(b)$, and $\bar{a}$ and $\bar{b}$ denote the sample means.
...
\G_{kl}=\dfrac{1}{z_{kl}} \textstyle \sum_{i,j=1}^n (a_{ij}^k-\bar{a}^{k}) (b_{ij}^l-\bar{b}^{l}),
\end{equation}
where $\bar{a}^{k}$ and $\bar{b}^{l}$ are the truncated means, and $z_{kl}$ is the product of the truncated standard deviations $\sigma(a)^{k}\sigma(b)^{l}$. There are a maximum of $n^2$ different local correlations, one for each possible combinations of $k$ and $l$.
tba: a maximum of ... local correlation rather than $n^2$
tba: re-define mean and std properly and unbiased.
tba: origianl dcorr/mcorr centering and our version centering

For \Mantel, it sets $a_{ij}$ and $b_{ij}$ be the entries of $\tilde{A}$ and $\tilde{B}$ directly, and excludes $a_{ii}$ and $b_{ii}$ in the sample means because they are always $0$.

For \Dcorr, $a_{ij}$ and $b_{ij}$ are the respective matrix entry of $A$ and $B$, and the sample means $\bar{a}, \bar{b}$ are automatically $0$. 

In accordance with Equation~\ref{generalCoef}, our version of global \Mcorr sets $a_{ij}=b_{ij}=0$ when $i=j$; otherwise $a_{ij}$ equals the respective matrix entry of $H\tilde{A}-\tilde{A}/n$, and $b_{ij}$ equals the entry of $\tilde{B}H-\tilde{B}/n$. Then the local version of \Mcorr follows by Equation~\ref{localCoef}. This is the local correlation we use for \Mgc throughout the main paper.

The optimal scale exists, may not be unique, and is distribution dependent.
