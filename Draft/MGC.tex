...
We observe that the sample \Mgc~is only significantly inferior to the Oracle \Mgc~when the true optimal scale has a narrow region (i.e., function $16$ and $17$ in both settings, plus function $9$ and $10$ in the high-dimensional setting, see Figure~\ref{f:powermaps} and ~\ref{f:powermaps1}), but otherwise very similar. Optimal scales limited to a narrow region are actually purposely excluded by algorithm~\ref{alg:best_scale} using $\gamma=0.05$, and a smaller $\gamma$ can recover such narrow regions and increase the power of sample \Mgc. However, narrow smooth regions may also occur from a pair of independently generated data by chance, so a reasonable $\gamma$ is necessary to keep the bias minimized, and one should not trust narrow smooth regions without training data or knowing the joint distribution.
