...
\title{\vspace{-2em}\bf Discovering \new{the Nature of} Relationships Across Disparate Data Modalities}
...
\centering
\subfigure{
\includegraphics[width=0.48\textwidth]{Figures/FigHDPowerSummary}
}
\subfigure{
\includegraphics[width=0.48\textwidth]{Figures/Fig1DPowerSummary}
}
...
Let $\bar{\beta}_s(\mathcal{A})$ denote the average power over a wide range of dimensions for a given problem setting $s$ and algorithm $\mc{A}$. The x-axis is the simulation type, and y-axis shows the relative power of existing competitors to \Mgc~, $\bar{\beta}_s(\mathcal{A}) / \bar{\beta}_s(\Mgc)$. The percentages indicate that \Mgc~is significantly more powerful in testing dependency on average, and throughout nearly all settings.
}
\label{f:Summary}
\end{figure}
...
\centering
\subfigure{
\includegraphics[width=0.48\textwidth]{Figures/FigHDPowerSummarySize}
}
\subfigure{
\includegraphics[width=0.48\textwidth]{Figures/Fig1DPowerSummarySize}
}
  \caption{Sample Size of different methods to achieve a power of $0.85$ at type 1 error level $0.05$, for the $20$ different settings under high-dimensional and 1-dimensional settings.  
The x-axis is the simulation type, and y-axis shows the minimal sample size of each method to achieve the required testing power, the smaller the better. The results indicate that \Mgc~is more appealing for finite-sample dependency testing, as data collection can be expensive in many frontiers. 
}
\label{f:Summary2}
\end{figure}
...
The average power across dimensions for each algorithm provides a scalar score per setting. Figure~\ref{f:Summary} shows the \Mgc's average power relative to the benchmarks' for each setting.  
