...
\State $n=size(X,1)$;$n_{s}=floor(n/2)$; \Comment{$n_{s}$ is the sample size of the sub-sample}
\State $ind=1:n_{s}^2$; \Comment{initialize all local scales as the potential optimal scale}
...
\State $p1=PermutationTest(X(per(1:n_{s}),:),Y(per(1:n_{s}),:),ceil(r/s),option)$; \Comment{we use $r/s$ random permutations instead of $r$ to speed up the screening procedure}
\State $p2=PermutationTest(X(per(n_{s}+1:2*n_{s}),:),Y(per(n_{s}+1:2*n_{s}),:),ceil(r/s),option)$;
...
\State $ind=verifyAdjancey(ind)$; \Comment{verify and exclude any isolated scales}
\If{$l1==length(ind)$}
\State $break$; \Comment{stop sub-sampling if the estimated optimal local scale no longer changes}
\EndIf
\If{$length(ind)<n_{s}/20$}
\State $ind=[];$
...
\State $k=k-1; l=l-1$; 
\State $kb=floor(k*(n-1)/(n_{s}-1))$; $ku=floor(k*(n-1)/(n_{s}-1))+1$; 
\State $lb=floor(l*(n-1)/(n_{s}-1))$; $lu=floor(l*(n-1)/(n_{s}-1))+1$; 
...
\State $p=median(p(indN))$; \Comment{take the median as the estimated MGC p-value}
\State \Return $p$, $indN$.
\EndFunction
\end{algorithmic}
\end{algorithm}
...
Furthermore, the screening procedure in Algorithm~\ref{algSplitPerm} can be speed up for large amount of data and large number of random permutations, e.g., instead of setting $n_{s}=\frac{n}{2}$, it usually suffices to use $n_{s}=100$ when the sample size $n$ is too large; and we have used $\frac{r}{s}$ random permutations for each screening rather than $r$ permutations for the whole data. Therefore the screening procedure is approximately equivalent in running time to the MGC permutation test on the whole sample, and Algorithm~\ref{algSplitPerm} has twice the running time of Algorithm~\ref{algPerm} due to the extra optimal scale estimation step.
...
\cs{I prefer to not show the above last paragraph and Figure~\ref{figSimPerm} unless the referee asked; because it kinds of weakens our claims in the main paper. Plus we can save it for the follow-on paper on local scale estimation. But I am also ok if you think keeping it here makes algorithm 4 more convincing. Note that the sub-sampling scheme is very very good and close to true power for all types except trigonometry functions 11-17.}
