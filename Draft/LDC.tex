...
The p-value heat maps of all local tests are provided in Figure~\ref{figReal}(b)(c), where we may observe that there exist many scales that are close to optimal. Indeed, in either hypothesis test, the estimated optimal scale of MGC is not unique, so we randomly pick one optimal scale and report the p-value; but in fact all optimal scales yield exactly the same p-values in this experiment.
...
\end{equation}
If there are more than one optimal neighborhood choices, it suffices to randomly pick one scale; alternatively, we can take all optimal scales and output the mean p-value from the next step.
