...
\end{appLem}
\begin{proof}
For distance covariance, we can re-write it in trace as follows
\begin{align*}
dcov(X,Y) &= \frac{1}{n^2}\sum_{i,j=1}^{n}a_{ij}b_{ij} \\
 &=\frac{1}{n^2} tr(A^{T} \times B) \\
 &=\frac{1}{n^2} tr(H\tilde{A}^{T}H \times H\tilde{B}H) \\
 &=\frac{1}{n^2} tr(\tilde{A}^{T}H \times \tilde{B}H) \\
 &=\frac{1}{n^2} tr((H\tilde{A})^{T} \times (\tilde{B}H)),
\end{align*}
where the second to last equality follows because $H$ is symmetric and idempotent. Therefore, single centering and double centering yields the same distance covariance.
