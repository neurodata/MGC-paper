function [power1, power2, power3, power4]=CorrIndTestNoise(type,n,dim,lim,rep)
...
% n=100; dim=100; lim=10; rep=1000;
% CorrIndTestNoise(1,n,dim,lim,rep);
%
% Type 11-17: seven correlation types in Wikipedia correlation figure /
% Table 3 of HHG, which uses dim=1.
% n=100; dim=1; lim=10; rep=1000;
% CorrIndTestNoise(11,n,dim,lim,rep);
%
% Type 18-20: 3 correlation types in Szeley 2007, which uses dim=5.
% n=100; dim=5; lim=10; rep=1000;
% CorrIndTestNoise(18,n,dim,lim,rep);
...
% Output
power1=zeros(lim+1,1); power2=zeros(lim+1,K);power3=zeros(lim+1,1);power4=zeros(lim+1,1);%powers for dCorr, rankdCorr, modified dCorr, HHG
dCor1A=zeros(rep,lim+1);dCor2A=zeros(rep,K,lim+1);dCor3A=zeros(rep,lim+1);dCor4A=zeros(rep,lim+1);
dCor1N=zeros(rep,lim+1);dCor2N=zeros(rep,K,lim+1);dCor3N=zeros(rep,lim+1);dCor4N=zeros(rep,lim+1);
d=dim;
...
for i=0:lim
    ...
    for r=1:rep
        ...
        dCor1A(r,i+1)=distCorr(C,P);
        dCor2A(r,:,i+1)=rankDCorr(C,P);
        dCor3A(r,i+1)=mDistCorr(C,P);
        dCor4A(r,i+1)=HHG(C,P);
    end
    ...
    for r=1:rep
        ...
        C=squareform(pdist(x));
        P=squareform(pdist(y));
        dCor1N(r,i+1)=distCorr(C,P);
        dCor2N(r,:,i+1)=rankDCorr(C,P);
        dCor3N(r,i+1)=mDistCorr(C,P);
        dCor4N(r,i+1)=HHG(C,P);
    end
end
...
save(filename,'power1','power2','power3','power4','type','n','noise','rep','lim','dim','dCor1N','dCor2N','dCor3N','dCor4N','dCor1A','dCor2A','dCor3A','dCor4A');
...
if display~=0
    ...
    titleStr = strcat('Independence Test for ', titlechar,' with Increasing Noise at n=', num2str(n), ' and m= ',num2str(dim));
    title(titleStr);
