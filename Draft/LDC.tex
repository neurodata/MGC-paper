...
We consider a mixture model: let $\mb{x}=\rho \mb{u} + (1-\rho) \mb{\epsilon}_{1}$, $y=\rho \mb{u} + (1-\rho) \mb{\epsilon}_{2}$, where $\mb{u}$ is uniformly distributed in $(-1,1)$, $\mb{\epsilon}_{1}$ and $\mb{\epsilon}_{2}$ are independently normally distributed with mean $-10$ and $10$ respectively and variance $1$, $\rho$ is Bernoulli distributed with probability $p$. Condition on $\rho$, we repeatedly generate simulating data $(X,Y)$ at $n=100$ and evaluate the testing power at $\alpha=0.05$.

Based on the mixture model, about $pn$ observations in $X$ are the same as the corresponding observations in $Y$; but the remaining observations in $X$ are outliers, which are independent from the corresponding observations in $Y$. One generated sample pair $(X,Y)$ at $p=0.5$ is shown in Figure~\ref{figSim3}(a), and the testing power of all possible local tests by mcorr is shown in Figure~\ref{figSim3}(b)(c)(d) for $p=0.3$,$0.5$,$0.7$ respectively.

We can clearly observe that the local testing powers can be much higher than global mcorr under this outlier model. Furthermore, the optimal scale $(k^{*},l^{*})$ is directly related to the probability $p$: although there exists multiple optimal scales, $k^{*}$ and $l^{*}$ are clearly bounded above by $\min(pn,(1-p)n)$; and a consistent optimal scale appears to be $k^{*}=l^{*}=\min(pn,(1-p)n)-1$, which excludes the distances between observations from different mixture components. Therefore, MGC is able to automatically separate the independent outliers from the dependent observations in testing, which makes it robust against outliers.
...
\item Spiral: Same as circle, but we change $\mb{u}$ and $r$ such that $r=\mb{u}_{d}=\mb{v}$ for all $d$, where $\mb{v} \sim \mc{U}(0,20)$.
