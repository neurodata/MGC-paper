...
\jv{to all science seems too narrow}
Discovering whether certain properties are associated with other properties is fundamental to all science. Identifying the existence of a relationship is a precursor to investigating the structure of the relationship, whether it has predictive power, or even causality.
% 
...
% jv{change this to be the first step is to compute distances}
...
% ---this serves to transform each data point $(\mbx_i,\mby_i)$ to a different representation, so that linear operations on this new representation can correspond to nonlinear operations in the original data (much like kernel machines \cite{scholkopf2002learning}).  
...
% , then its test statistic $\G^*$ equals the local correlation at the optimal scale with the p-value follows from a permutation test. 
...
From the multiscale test statistic map, sample \Mgc~finds the region with the largest test statistic (ignoring sparse noisy scales, Algorithm \ref{alg:sample_mgc}).  The p-value of sample \Mgc, $p(\hat{c}^*)$, follows from a permutation test using the estimated optimal statistic within that region.% is the p-value for the center of that region. 
\cs{do not understand what you mean by p-value for the center of the region, I changed the sentence accordingly}
...
% significantly large and smooth correlations, then uses a large statistic within the region as $\hat{\G}^*$ (Algorithm \ref{alg:sample_mgc}) as an estimator of the optimal statistic $\G^*$. Then we compute the p-value, $p(\hat{\G}^*)$, and reports the largest rectangle within the multiscale p-value map that are no larger than $p(\hat{\G}^*)$ as the optimal scales (Algorithm \ref{alg:pval}). 
Because neither the Oracle \Mgc~nor sample \Mgc~compares multiple p-values, neither suffers from the multiple hypothesis testing problem \cite{Benjamini1995}.
