...
\State $p1=p1-min(p1)$; $p2=p2-min(p2)$; \Comment{normalize the local p-values}
\State $pMean=p1/2+p2/2$; \Comment{mean p-values of the two sub-samples}
\If{$i==1$}
\State $ind=find(pMean-min(pMean)<=0.1)$; \Comment{find all scales that is close to the minimum p-value}
\Else
\State $ind=ind(pMean(ind)-min(pMean)<=0.1)$; \Comment{verify if the scale is still close to the minimum p-value for a different random split}
\EndIf
\State $Diff1=p1-pMean$; $Diff2=p2-pMean$;
\State $ind=ind(mean(Diff1(ind)>Diff1)<0.2)$; \Comment{check if the scale produces similar p-values}
...
In Figure~\ref{figSimPerm} we compare the theoretical testing power of MGC\{dcorr\} from Figure~\ref{fig:1D} to its testing power based on the sub-sampling scheme and the permutation test at $n=50$ for the low-dimensional simulation, using the global dcorr as the benchmark. We generate $100$ pairs of dependent data, calculate the MGC p-value by Algorithm~\ref{algSplitPerm}, and reject the independent hypothesis when the p-value is less than $0.05$. We observe that MGC by sub-sampling is slightly inferior to the true MGC power, but is still significantly better than the global methods. 
\cs{I prefer to not show the above last paragraph and Figure~\ref{figSimPerm} unless the referee asked; because it kinds of weakens our claims in the main paper.}
