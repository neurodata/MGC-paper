...
Whereas \Mcorr, the global test, has very low power (magenta dot in multiscale p-value map) and therefore yields a non-significant p-value ($0.187$),  there are many local scales that achieve nearly perfect power, so \Mgc~obtains both a highly significant p-value ($\approx 0.001$) and reveals the scales of dependency. 
...
\cs{I slightly rephrased the next paragraph for sample mgc}
The fifth step is to estimate the optimal scales and report a valid p-value. The optimal scales for any given distribution and finite sample size are defined as the scales that maximize power (the probability of correctly rejecting a false null hypothesis).  In simulations, or datasets for which we have a known model or extensive training data, we can find the optimal scales by maximizing the testing powers; ``Oracle \Mgc'' chooses the optimal scale in this fashion  (Algorithm \ref{alg:power}), and the p-value follows by locating the optimal scale in the p-value map (Algorithm \ref{alg:pval}).
In real data settings, the power of a test cannot be known because the true distribution is unavailable, so  we design a ``sample \Mgc'' (or just \Mgc) tailored for local \Mcorr, which locates one local correlation to use within all local correlations (Algorithm \ref{alg:sample_mgc}). The resulting p-value by sample \Mgc~is then compared to the p-value map, for which the largest rectangle with same or less local p-values are taken as the estimated optimal scale. Clearly, neither the Oracle \Mgc~or sample \Mgc~suffers from multiple hypothesis testing problems \cite{Benjamini1995}.
...
For nearly linear dependencies (1-5), the best neighborhood choice always includes the largest scale, i.e., $k=l=n$. For all strongly nonlinear dependencies (6-19), Oracle \Mgc~chooses smaller scales for $x$ or $y$. This implies that a global optimal scale indicates that the underlying dependency is close to linear, otherwise the dependency is strongly non-linear.
...
%
Note that the local structures can be similarly identified by the p-value map or the local correlation map, and there always exists a large smooth region of local scales that are nearly equally significant in the multiscale maps. These are two important observations that we utilize to design the sample \Mgc~in algorithm~\ref{alg:sample_mgc}, which performs very well in estimating the p-value and optimal scales.
...
In this work we proved that there exist local scales that improve upon the global scale, and demonstrated that sample \Mgc~yields tests with power close to the optimal power, and often better power than \Hhg~and \Mcorr.  But  theory connecting Oracle \Mgc~and  sample \Mgc~will yield further insight.
...
Algorithm~\ref{alg:power} estimates the testing powers of all local statistics using a known or estimated joint distribution, which can be used to accurately calculate the optimal scales and the testing power for Oracle \Mgc. 
Algorithm~\ref{alg:pval} computes the p-values of all local correlation by the random permutation test, which also includes the optimal scale estimation by sample \Mgc.
Algorithm~\ref{alg:sample_mgc} locates a suitable local correlation among all local correlations for sample \Mgc. When there is a sufficiently large region with significant local correlations, we pick a large local correlation within the region; otherwise the global correlation is used instead. More detailed discussions regarding sample \Mgc~is offered in Appendix~\ref{appen:diss}.
...
\State $\gamma \rto 0.03$ %\Comment{threshold to check significant global p-value and significant region}
\If{$\sum_{k,l} (\G^{kl}>\gamma) \leq 2n$} 
\State $\hat{\G}^{*} \rto \G_{nn}$ \Comment{use the global correlation if too few significant local statistics}
\Else
\State $\Omega \rto \arg_{k,l}(\G^{kl}\geq \textsc{Quantile}(\G^{kl},99))$ %\Comment{get the optimal scales}
