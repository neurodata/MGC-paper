...
More specifically,  we propose Multiscale Graph Correlation (\Mgc), which can be thought of as the love child of distance correlations with graph-based tests, by considering all scales and choosing the best one.  \Mgc~therefore  naturally inherits the advantages of the global distance correlation tests---such as the consistency---while also enjoying the properties of graph-based tests to exclude non-local comparisons, efficiently for all definitions of local, and choosing the optimal neighborhood sizes. \Mgc~performs as well or better than the existing literature on each of the seven above mentioned design goals. More specifically, in a comprehensive simulation setting, \Mgc~is able to achieve superior finite-sample testing powers for data sets of nonlinearity, high-dimensionality, noise, and small sample size, compared to existing popular methods. Moreover,  in real data experiments, we show that \Mgc~is able to uncover the underlying dependency when testing between related data sets,  and does not inflate the false positive rate when the dependencies are not there. Thus, we expect \Mgc to find values in a wide range of applications.  To facilitate this, we provide open source MATLAB and R implementations of \Mgc~at \website.
